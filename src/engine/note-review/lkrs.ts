...
export function computeLkrsLoadFactor(totalActiveNotes: number, settings: LkrsSettings): number {
...
export function scaleLkrsIntervalDays(stepIntervalDays: number, loadFactor: number): number {
  return Math.max(1, stepIntervalDays * clampMinOne(loadFactor));
}

export function initialLkrsDueTime(
  noteId: string,
  now: number,
...
export function reviewWithLkrs(
  state: LkrsNoteState,
  now: number,
  settings: LkrsSettings,
