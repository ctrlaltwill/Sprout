...
export class LaunchNoticeModal extends Modal {
    ...
    onOpen() {
        ...
        body.createEl("p", {
            text: tx("ui.launchNotice.body", "Thank you for using LearnKit. I have thoroughly enjoyed building this over the last few months, and I am excited to release version 1.1.0."),
        });
        body.createEl("p", {
            text: tx("ui.launchNotice.body.versionSummary", "Version 1.1.0 is our biggest update so far. It introduces companion, our AI learning companion, with a bring-your-own-key model. This model allows it to be included for all users and not locked behind subscriptions or paywalls."),
        });
        ...
        body.createEl("p", {
            text: tx("ui.launchNotice.security.body", "Your API keys are stored in local plugin data files ({configPath}). If you use Git in your vault, make sure plugin data files are ignored in .gitignore before pushing to GitHub. If you do not sync that folder with Git, this is not an issue.", { configPath }),
        });
    ...
    async persistPreference() {
        if (this.persisted || !this.doNotShowAgain)
            return;
        this.persisted = true;
