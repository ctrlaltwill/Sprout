 ...
 */
import { Modal, Notice, MarkdownView, TFile, setIcon } from "obsidian";
import { log } from "../core/logger";
import { placePopover, queryFirst, setCssProps } from "../core/ui";
import { normaliseVaultPath, extFromMime, bestEffortAttachmentPath, writeBinaryToVault, setVisible, focusFirstField, hasClozeToken, formatPipeField, createModalCardEditor, setModalTitle, scopeModalToWorkspace, } from "./modal-utils";
...
export class CardCreatorModal extends Modal {
    ...
    constructor(app, plugin, forcedType) {
        super(app);
     ...
     */
    async handleImagePaste(ev, textarea) {
     ...
     */
    async savePendingImages(sourceFile, textContent) {
        ...
        for (const [placeholder, imageInfo] of this.pendingImages) {
            try {
                ...
                updatedContent = updatedContent.replace(new RegExp(`!\\[\\[${placeholder.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\]\\]`, "g"), `![[${actualPath}]]`);
            }
            catch (e) {
    ...
    onOpen() {
        ...
        if (headerEl) {
            ...
            if (titleEl)
                titleEl.setText(this.tx("ui.cardCreator.title", "Add flashcard"));
            const existingCloseBtn = headerEl.querySelector(":scope > .learnkit-card-creator-close-btn");
            if (existingCloseBtn)
        ...
        const existingFooter = this.modalEl.querySelector(":scope > .learnkit-card-creator-footer");
        if (existingFooter)
        ...
        let cardEditor = null;
        let currentType = this.forcedType || "basic";
        ...
        const isTypeMenuOption = (type) => type === "basic" || type === "reversed" || type === "cloze" || type === "mcq" || type === "oq";
        let updateTypeMenuLabel = () => { };
        const setType = (next) => {
            currentType = next;
            if (next === "reversed")
                typeSel.value = "basic";
            else
                typeSel.value = next === "mcq" ? "mcq" : next === "cloze" ? "cloze" : "basic";
            renderCardEditor();
            syncVisibility();
            updateTypeMenuLabel();
        };
        ...
        const closeTypeMenu = () => {
            ...
            try {
                ...
                log.swallow("remove type menu wrapper", e);
            }
            typeMenuOpen = false;
        };
        ...
        const buildTypeMenu = () => {
            while (typeMenu.firstChild)
                typeMenu.removeChild(typeMenu.firstChild);
        ...
        typeMenuBtn.addEventListener("pointerdown", (ev) => {
            if (ev.button !== 0)
                return;
            ev.preventDefault();
            ev.stopPropagation();
        ...
        const renderCardEditor = () => {
            ...
            try {
                ...
                const formatLocationPath = (p) => {
                    if (!p)
                        return "";
                    const cleaned = p.replace(/\.md$/i, "");
                    return cleaned.split("/").filter(Boolean).join(" / ");
                };
                const addLocationField = (root) => {
                    ...
                    const groupsWrapper = children.find((child) => {
                        ...
                        return lbl && ((_a = lbl.textContent) === null || _a === void 0 ? void 0 : _a.trim()) === "Groups";
                    });
        ...
        this._ioPasteHandler = (ev) => { void handleIoPaste(ev); };
        ...
        typeSel.addEventListener("change", () => {
            const next = (typeSel.value || "basic");
            setType(next);
        });
        renderCardEditor();
        syncVisibility();
        // ── Helpers ──────────────────────────────────────────────────────────────
        const getActiveMarkdownFile = () => {
        ...
        const insertTextAtCursorOrAppend = async (active, textToInsert, forcePersist = false) => {
            var _a;
            const view = this.app.workspace.getActiveViewOfType(MarkdownView);
            if (((_a = view === null || view === void 0 ? void 0 : view.file) === null || _a === void 0 ? void 0 : _a.path) === active.path && view.editor) {
                ...
                if (forcePersist) {
                    try {
                        const saveable = view;
                        if (typeof (saveable === null || saveable === void 0 ? void 0 : saveable.save) === "function")
                            await saveable.save();
                    }
                    catch (_b) {
                        // ignore
        ...
        addBtn.onclick = async () => {
            ...
            try {
                ...
                const getValue = (key) => { var _a, _b; return String((_b = (_a = (cardEditor.inputEls)[key]) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : "").trim(); };
                ...
                const requireNonEmpty = (val, message) => {
                ...
                if (type === "basic" || type === "reversed") {
                    if (!requireNonEmpty(questionVal, "Basic requires a question."))
                        return;
                    if (!requireNonEmpty(answerVal, "Basic requires an answer."))
                        return;
                    if (titleVal)
                        block.push(...formatPipeField("T", titleVal));
                    block.push(...formatPipeField(currentType === "reversed" ? "RQ" : "Q", questionVal));
                    block.push(...formatPipeField("A", answerVal));
                }
                else if (type === "cloze") {
                    if (!requireNonEmpty(questionVal, "Cloze requires text with at least one {{cN::...}} token."))
                        return;
                    ...
                    if (!requireNonEmpty(questionVal, "Multiple choice requires a stem."))
                        return;
                    const mcqValues = (_a = cardEditor.getMcqOptions) === null || _a === void 0 ? void 0 : _a.call(cardEditor);
                    const corrects = ((mcqValues === null || mcqValues === void 0 ? void 0 : mcqValues.corrects) || [])
                        .map((x) => (typeof x === "string" ? x.trim() : typeof x === "number" ? String(x).trim() : ""))
                        .filter(Boolean);
                    const wrongs = ((mcqValues === null || mcqValues === void 0 ? void 0 : mcqValues.wrongs) || [])
                        .map((x) => (typeof x === "string" ? x.trim() : typeof x === "number" ? String(x).trim() : ""))
                        .filter(Boolean);
                    ...
                    for (const wrong of wrongs)
                        block.push(...formatPipeField("O", wrong));
                    for (const c of corrects)
                        block.push(...formatPipeField("A", c));
                }
                else if (type === "oq") {
                    if (!requireNonEmpty(questionVal, "Ordering requires a question."))
                        return;
                    const steps = ((_b = cardEditor.getOqSteps) === null || _b === void 0 ? void 0 : _b.call(cardEditor)) || [];
                    if (steps.length < 2) {
                    ...
                    for (let i = 0; i < steps.length; i++) {
                        block.push(...formatPipeField(String(i + 1), steps[i]));
                    }
                }
                else {
                ...
                if (infoVal)
                    block.push(...formatPipeField("I", infoVal));
                if (groupsVal)
                    block.push(...formatPipeField("G", groupsVal));
                block.push("");
    ...
    onClose() {
        ...
        const footerEl = this.modalEl.querySelector(":scope > .learnkit-card-creator-footer");
        if (footerEl)
            footerEl.remove();
        const closeBtn = this.modalEl.querySelector(":scope > .modal-header .learnkit-card-creator-close-btn");
        if (closeBtn)
            closeBtn.remove();
