...
import { previewApkg, importFromApkg } from "../../platform/integrations/anki/anki-import";
...
export class AnkiImportModal extends Modal {
    ...
    mkNavBtn(parent, label, icon, opts) {
        const btn = parent.createEl("button", {
            cls: "learnkit-btn-toolbar learnkit-btn-toolbar inline-flex items-center gap-2 h-9 px-3 text-sm",
            attr: { type: "button", "aria-label": (opts === null || opts === void 0 ? void 0 : opts.tooltip) || label },
        });
    ...
    renderFileStep(root) {
        ...
        fileInput.onchange = async () => {
            ...
            if (pickSpan)
                pickSpan.textContent = this.tx("ui.anki.import.action.changeFile", "Change file");
            try {
        ...
        nextBtn.onclick = async () => {
            ...
            if (nextSpan)
                nextSpan.textContent = this.tx("ui.anki.import.status.scanning", "Scanning") + "\u2026";
            try {
                ...
                nextBtn.disabled = false;
                if (nextSpan)
                    nextSpan.textContent = common.next;
            }
        };
    }
    // ── Step 2: Preview ─────────────────────────────────────────────────────────
    renderPreviewStep(root) {
        var _a, _b;
        const common = txCommon((_b = (_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.general) === null || _b === void 0 ? void 0 : _b.interfaceLanguage);
        if (!this.preview)
            return;
        ...
        if (p.ioCount > 0)
            addStat("image-off", "Image Occlusion (skipped)", p.ioCount);
        if (p.otherCount > 0)
            addStat("help-circle", "Other (as Basic)", p.otherCount);
        if (p.mediaCount > 0)
            addStat("image", "Media files", p.mediaCount);
    ...
    renderOptionsStep(root) {
        ...
        importBtn.onclick = () => {
            ...
            const opts = {
                ...
                groupMapping: groupDropdown.getValue(),
                duplicateStrategy: dupeDropdown.getValue(),
            ...
            const unknowns = ((_a = this.preview) === null || _a === void 0 ? void 0 : _a.unknownModels) || [];
    ...
    renderFieldMappingStep(root, opts) {
        var _a, _b, _c;
        const common = txCommon((_b = (_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.general) === null || _b === void 0 ? void 0 : _b.interfaceLanguage);
        const unknowns = ((_c = this.preview) === null || _c === void 0 ? void 0 : _c.unknownModels) || [];
        root.empty();
        ...
        for (let mi = 0; mi < unknowns.length; mi++) {
            ...
            const renderMapControls = () => {
                ...
                typeDropdown.onChange((val) => { st.importAs = val; });
                const makeFieldDropdown = (parent, label, currentIdx, allowNone, onChange) => {
                    const f = this.mkField(parent, label);
                    const opts = [];
                    if (allowNone)
                        opts.push({ value: "-1", label: "— None —" });
    ...
    async renderProgressStep(root, opts) {
        var _a, _b;
        const common = txCommon((_b = (_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.general) === null || _b === void 0 ? void 0 : _b.interfaceLanguage);
        root.empty();
        ...
        const setProgress = async (pct, phase) => {
    ...
    renderResultStep(root, result) {
        ...
        if (result.ioSkipped > 0)
            addRow("IO cards skipped", result.ioSkipped);
        if (result.otherSkipped > 0)
            addRow("Custom types skipped", result.otherSkipped);
        if (result.duplicates > 0)
            addRow("Duplicates skipped", result.duplicates);
