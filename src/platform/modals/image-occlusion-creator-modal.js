...
import { buildToolbar, buildCanvasContainer, buildFooter, } from "../../platform/image-occlusion/io-modal-ui";
import { autoDetectTextMasks } from "../../platform/image-occlusion/io-ocr";
import { resolveImageFile as resolveIoImageFile, mimeFromExt, } from "../../platform/image-occlusion/io-helpers";
import { setModalTitle, scopeModalToWorkspace, } from "./modal-utils";
import { rotateImageData, cropImageData, burnTextBoxesIntoImageData, clampTextBgOpacity, textBgCss, } from "../../platform/image-occlusion/io-image-ops";
...
export class ImageOcclusionCreatorModal extends Modal {
    ...
    static openForParent(plugin, parentId, opts) {
        ...
        m.onCloseCallback = opts === null || opts === void 0 ? void 0 : opts.onClose;
        m.open();
        return m;
    }
    // ── Modal lifecycle ───────────────────────────────────────────────────────
    onOpen() {
        ...
        if (headerEl) {
            ...
            if (titleEl)
                titleEl.setText(headerTitle);
            const existingCloseBtn = headerEl.querySelector(":scope > .learnkit-io-creator-close-btn");
            if (existingCloseBtn)
                existingCloseBtn.remove();
            const legacyHeaderClose = headerEl.querySelector(":scope > .modal-close-button");
            if (legacyHeaderClose)
                legacyHeaderClose.remove();
        ...
        const legacyRootClose = this.modalEl.querySelector(":scope > .modal-close-button");
        if (legacyRootClose)
            legacyRootClose.remove();
        const existingFooter = this.modalEl.querySelector(":scope > .learnkit-io-footer");
        if (existingFooter)
        ...
        titleField.appendChild(attachFlagPreviewOverlay(titleInput, 60, 150, {
        ...
        if (this.canvasContainerEl)
            this.canvasContainerEl.classList.add("learnkit-io-canvas", "learnkit-io-canvas");
        if (this.canvasContainerEl)
            this.canvasContainerEl.tabIndex = 0;
        if (this.stageEl)
            this.stageEl.classList.add("learnkit-io-stage", "learnkit-io-stage");
        if (this.viewportEl)
        ...
        this.onDocPaste = (ev) => { void handlePaste(ev); };
        ...
        infoField.appendChild(attachFlagPreviewOverlay(infoInput, 60, 150, {
            preferInlineControlHeight: true,
            deferMeasuredHeightUntilInteraction: true,
        }));
        ...
        const initialGroups = editing
            ? (_c = (((_b = (_a = this.plugin.store) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.cards) || {})[String(this.editParentId)]) === null || _c === void 0 ? void 0 : _c.groups
            : null;
        ...
        if (editing && this.editParentId) {
            const cardsMap = (((_e = (_d = this.plugin.store) === null || _d === void 0 ? void 0 : _d.data) === null || _e === void 0 ? void 0 : _e.cards) || {});
            const parent = cardsMap[String(this.editParentId)];
            if (parent && String(parent.type) === "io") {
                if (this.titleInput)
                    this.titleInput.value = String(parent.title || "");
                if (this.infoInput)
                    this.infoInput.value = String(parent.info || "");
                const ioMap = ((_g = (_f = this.plugin.store) === null || _f === void 0 ? void 0 : _f.data) === null || _g === void 0 ? void 0 : _g.io) || {};
                const def = ioMap[String(this.editParentId)] || null;
                const imageRef = String(parent.imageRef || (def === null || def === void 0 ? void 0 : def.imageRef) || "").trim();
                this.editImageRef = imageRef || null;
                const rects = Array.isArray(def === null || def === void 0 ? void 0 : def.rects) ? def.rects : [];
                this.rects = rects.map((r) => {
                    var _a, _b, _c, _d;
                    const rectIdRaw = r.rectId;
                    const rectId = typeof rectIdRaw === "string"
                        ? rectIdRaw
                    ...
                    return {
                        rectId,
                        normX: Number((_a = r.x) !== null && _a !== void 0 ? _a : 0) || 0,
                        normY: Number((_b = r.y) !== null && _b !== void 0 ? _b : 0) || 0,
                        normW: Number((_c = r.w) !== null && _c !== void 0 ? _c : 0) || 0,
                        normH: Number((_d = r.h) !== null && _d !== void 0 ? _d : 0) || 0,
        ...
        const saveIo = async (maskMode) => {
            ...
            try {
                const shouldClose = await saveIoCard({
                    ...
                    titleVal: String(((_a = this.titleInput) === null || _a === void 0 ? void 0 : _a.value) || "").trim(),
                    groupsVal: String(((_c = (_b = this.groupsField) === null || _b === void 0 ? void 0 : _b.hiddenInput) === null || _c === void 0 ? void 0 : _c.value) || "").trim(),
                    infoVal: String(((_d = this.infoInput) === null || _d === void 0 ? void 0 : _d.value) || "").trim(),
        ...
        let defaultMode = ((_j = (_h = this.plugin.settings) === null || _h === void 0 ? void 0 : _h.imageOcclusion) === null || _j === void 0 ? void 0 : _j.defaultMaskMode) || "solo";
        if (editing && this.editParentId) {
            const ioMap = ((_l = (_k = this.plugin.store) === null || _k === void 0 ? void 0 : _k.data) === null || _l === void 0 ? void 0 : _l.io) || {};
            const def = ioMap[String(this.editParentId)] || null;
        ...
        requestAnimationFrame(() => {
            var _a, _b;
            (_b = ((_a = this.canvasContainerEl) !== null && _a !== void 0 ? _a : this.viewportEl)) === null || _b === void 0 ? void 0 : _b.focus();
        });
    }
    // ── Tool management ───────────────────────────────────────────────────────
    setTool(tool) {
        ...
        setActive((_a = this.btnRectTool) !== null && _a !== void 0 ? _a : undefined, tool === "occlusion-rect");
        setActive((_b = this.btnTransform) !== null && _b !== void 0 ? _b : undefined, tool === "transform");
        setActive((_c = this.btnText) !== null && _c !== void 0 ? _c : undefined, tool === "text");
        setActive((_d = this.btnCrop) !== null && _d !== void 0 ? _d : undefined, tool === "crop");
        this.updateCursor();
    ...
    updatePlaceholderVisibility() {
        if (!this.placeholderEl || !this.viewportEl)
            return;
        if (this.ioImageData) {
    ...
    updateCanvasHeightForImage() {
        ...
        if (!width || !this.stageW || !this.stageH)
            return;
        const maxHeight = 450;
    ...
    async loadImageToCanvas() {
        if (!this.ioImageData || !this.imgEl)
            return;
    ...
    async loadExistingImageForEdit(imageRef, sourceNotePath) {
        var _a, _b;
        try {
            ...
            const data = await ((_b = (_a = this.app.vault).readBinary) === null || _b === void 0 ? void 0 : _b.call(_a, file));
            if (!data)
                throw new Error("readBinary not available");
    ...
    // ── Viewport / transform ──────────────────────────────────────────────────
    fitToViewport() {
        if (!this.viewportEl || !this.stageEl)
            return false;
        ...
        if (!vw || !vh || vw < 2 || vh < 2 || !this.stageW || !this.stageH)
            return false;
    ...
    fitToViewportWhenReady(maxAttempts = 5) {
    ...
    applyTransform() {
        if (!this.stageEl)
            return;
        setCssProps(this.stageEl, "--learnkit-io-stage-transform", `translate(${this.t.tx}px, ${this.t.ty}px) scale(${this.t.scale})`);
    }
    ...
    setupCanvasEvents() {
        ...
        this.viewportEl.addEventListener("mousedown", (e) => {
            ...
            const isOnRect = !!(targetEl === null || targetEl === void 0 ? void 0 : targetEl.closest("[data-rect-id]"));
            const isOnText = !!(targetEl === null || targetEl === void 0 ? void 0 : targetEl.closest("[data-text-id]"));
            const isOnForm = !!(targetEl === null || targetEl === void 0 ? void 0 : targetEl.closest("input,textarea,button,select"));
        ...
        this.viewportEl.addEventListener("mouseup", (e) => {
            if (panning) {
                panning = false;
                if (this.viewportEl) {
    ...
    syncTextBgOpacityInput() {
        if (!this.textBgOpacityInput)
            return;
    ...
    openTextInput(stageX, stageY, dims) {
        var _a, _b, _c;
        if (!this.viewportEl)
            return;
        if (!this.ioImageData) {
        ...
        this.clearTextInput(false);
        const defaultW = (_a = dims === null || dims === void 0 ? void 0 : dims.w) !== null && _a !== void 0 ? _a : 220;
        const defaultH = (_b = dims === null || dims === void 0 ? void 0 : dims.h) !== null && _b !== void 0 ? _b : 64;
        ...
        if (box) {
            ...
            this.textBgOpacity = clampTextBgOpacity((_c = box.bgOpacity) !== null && _c !== void 0 ? _c : this.textBgOpacity);
            if (this.textColorInput)
                this.textColorInput.value = this.textColor;
            if (this.textBgInput)
                this.textBgInput.value = this.textBgColor !== "transparent" ? this.textBgColor : "#ffffff";
    ...
    async burnTextBoxesIntoImage() {
        if (!this.ioImageData || this.textBoxes.length === 0)
            return;
    ...
    async finalizeCropRect(x1, y1, x2, y2) {
        const left = Math.min(x1, x2);
        const top = Math.min(y1, y2);
        const width = Math.abs(x2 - x1);
        const height = Math.abs(y2 - y1);
    ...
    updatePreview(x1, y1, x2, y2, shape = "rect") {
        if (!this.overlayEl)
            return;
        if (!this.previewEl) {
    ...
    getNextGroupNumber() {
        const maxGroup = this.rects.reduce((acc, r) => {
            var _a;
            const n = Number.parseInt(String((_a = r.groupKey) !== null && _a !== void 0 ? _a : "").trim(), 10);
    ...
    saveHistory() {
        ...
        if (this.historyIndex === -1) {
            const base = { rects: [], texts: [], image: snapshot.image };
    ...
    renderRects() {
        ...
        renderOverlay({
            ...
            preserveEls: new Set([this.previewEl, this.cropPreviewEl]),
            rects: this.rects,
            ...
            cb: {
                ...
                editTextBox: (textBox) => {
                    ...
                    this.textBgOpacity = clampTextBgOpacity((_a = textBox.bgOpacity) !== null && _a !== void 0 ? _a : this.textBgOpacity);
                    if (this.textColorInput)
                        this.textColorInput.value = this.textColor;
                    if (this.textBgInput)
                        this.textBgInput.value = this.textBgColor !== "transparent" ? this.textBgColor : "#ffffff";
    ...
    onClose() {
        ...
        try {
            ...
            log.swallow("IO modal onClose callback", e);
        }
        if (this.onDocPaste) {
