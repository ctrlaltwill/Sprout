...
export function getShortcutGroups(locale) {
    const tx = (token, fallback) => t(locale, token, fallback);
    return [
        {
...
export class KeyboardShortcutsModal extends Modal {
    ...
    onOpen() {
        ...
        const defaultClose = this.modalEl.querySelector(":scope > .modal-close-button");
        if (defaultClose)
            defaultClose.remove();
        if (headerEl) {
            ...
            close.setAttribute("aria-label", t((_d = (_c = this.plugin.settings) === null || _c === void 0 ? void 0 : _c.general) === null || _d === void 0 ? void 0 : _d.interfaceLanguage, "ui.common.close", "Close"));
        ...
        const buildColumn = (heading, sections, host) => {
        ...
        buildColumn(tx("ui.shortcuts.colStudy", "Study"), studyGroups, columns);
        if (actionGroup) {
            buildColumn(tx("ui.shortcuts.colActions", "Actions"), [actionGroup], columns);
        }
