...
import { createCardEditor, } from "../card-editor/card-editor";
import { setCssProps } from "../core/ui";
// ──────────────────────────────────────────────────────────────────────────────
// DOM helpers
// ──────────────────────────────────────────────────────────────────────────────
/** Renders a styled "danger callout" box (used by ParseErrorModal). */
export function mkDangerCallout(parent, text) {
...
export function setDisabledUnder(root, disabled) {
    const els = root.querySelectorAll("input, textarea, select, button");
    els.forEach((el) => {
        try {
 ...
 */
export function parkBehind(modalEl, behind) {
    if (behind) {
...
export function typeLabelBrowser(t) {
    const ty = String(t !== null && t !== void 0 ? t : "").toLowerCase();
    if (ty === "basic")
        return "Basic";
    if (ty === "reversed" || ty === "reversed-child")
        return "Basic (Reversed)";
    if (ty === "cloze" || ty === "cloze-child")
        return "Cloze";
    if (ty === "mcq")
        return "Multiple choice";
    if (ty === "io" || ty === "io-child")
 ...
 */
export function createModalCardEditor(config) {
...
export function focusFirstField(el) {
    ...
    (_a = first === null || first === void 0 ? void 0 : first.focus) === null || _a === void 0 ? void 0 : _a.call(first);
}
// ──────────────────────────────────────────────────────────────────────────────
...
export async function ensureParentFolder(app, vaultPath) {
    var _a;
    const parts = vaultPath.split("/").filter(Boolean);
    if (parts.length <= 1)
        return;
    const parent = parts.slice(0, -1).join("/");
    const adapter = (_a = app.vault) === null || _a === void 0 ? void 0 : _a.adapter;
    if (!(adapter === null || adapter === void 0 ? void 0 : adapter.exists) || !(adapter === null || adapter === void 0 ? void 0 : adapter.mkdir))
        return;
...
export async function writeBinaryToVault(app, vaultPath, data) {
    const vault = app.vault;
    const path = normaliseVaultPath(vaultPath);
    await ensureParentFolder(app, path);
    const existing = vault.getAbstractFileByPath(path);
    if (existing instanceof TFile) {
        if (typeof (vault === null || vault === void 0 ? void 0 : vault.modifyBinary) === "function") {
            await vault.modifyBinary(existing, data);
            return;
        }
    }
    if (typeof (vault === null || vault === void 0 ? void 0 : vault.createBinary) === "function") {
        await vault.createBinary(path, data);
        return;
    }
    const adapter = vault === null || vault === void 0 ? void 0 : vault.adapter;
    if (adapter === null || adapter === void 0 ? void 0 : adapter.writeBinary) {
 ...
 */
export function bestEffortAttachmentPath(plugin, active, baseName, type = "io") {
    var _a, _b, _c;
    const folderRaw = type === "card"
        ? ((_a = plugin.settings.storage.cardAttachmentFolderPath) !== null && _a !== void 0 ? _a : "")
        : ((_b = plugin.settings.storage.imageOcclusionFolderPath) !== null && _b !== void 0 ? _b : "");
    const folder = normaliseFolderPath(folderRaw);
    if (folder)
        return normaliseVaultPath(`${folder}${baseName}`);
    const fm = plugin.app.fileManager;
    if (fm === null || fm === void 0 ? void 0 : fm.getAvailablePathForAttachment) {
        try {
            const p = fm.getAvailablePathForAttachment(baseName, active.path);
            if (p && p.length)
                return normaliseVaultPath(p);
        }
        catch (_d) {
            // fall through
        }
    }
    const parent = ((_c = active.parent) === null || _c === void 0 ? void 0 : _c.path) ? String(active.parent.path) : "";
...
export function setModalTitle(modal, title) {
...
export function hasClozeToken(s) {
 ...
 */
export function createThemedDropdown(options, initialValue, extraCls, config) {
    var _a, _b, _c, _d, _e, _f, _g;
    let currentValue = (_b = initialValue !== null && initialValue !== void 0 ? initialValue : (_a = options[0]) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : "";
    let changeCb = null;
    const fullWidth = (_c = config === null || config === void 0 ? void 0 : config.fullWidth) !== null && _c !== void 0 ? _c : true;
    const buttonSize = (_d = config === null || config === void 0 ? void 0 : config.buttonSize) !== null && _d !== void 0 ? _d : "md";
    const buttonJustify = (_e = config === null || config === void 0 ? void 0 : config.buttonJustify) !== null && _e !== void 0 ? _e : (fullWidth ? "between" : "start");
    const container = document.createElement("div");
    container.className = [
        "sprout relative inline-flex",
        fullWidth ? "w-full" : "",
        extraCls !== null && extraCls !== void 0 ? extraCls : "",
        (_f = config === null || config === void 0 ? void 0 : config.containerClassName) !== null && _f !== void 0 ? _f : "",
    ].filter(Boolean).join(" ");
    // Trigger button
    ...
    btn.className = [
        ...
        (_g = config === null || config === void 0 ? void 0 : config.buttonClassName) !== null && _g !== void 0 ? _g : "",
    ].filter(Boolean).join(" ");
    ...
    const updateLabel = () => {
        ...
        btnText.textContent = (_a = opt === null || opt === void 0 ? void 0 : opt.label) !== null && _a !== void 0 ? _a : currentValue;
    };
    let isOpen = false;
    const close = () => {
        btn.setAttribute("aria-expanded", "false");
    ...
    const buildMenu = () => {
        ...
        for (const opt of options) {
            ...
            const activate = () => {
                ...
                updateLabel();
                close();
                changeCb === null || changeCb === void 0 ? void 0 : changeCb(currentValue);
            };
 ...
 */
export function scopeModalToWorkspace(modal) {
    var _a, _b;
    ...
    const activeLeaf = (_b = (_a = document.querySelector(".mod-root .workspace-leaf.mod-active")) !== null && _a !== void 0 ? _a : document.querySelector(".mod-root .workspace-leaf:not([style*='display: none'])")) !== null && _b !== void 0 ? _b : document.querySelector(".workspace-leaf.mod-active");
    const leafContent = activeLeaf === null || activeLeaf === void 0 ? void 0 : activeLeaf.querySelector(".workspace-leaf-content");
