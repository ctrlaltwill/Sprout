...
function normalizeBrandIconSvg(rawSvg) {
    const cleaned = rawSvg
        .replace(/<\?xml[^>]*\?>/gi, "")
        .replace(/<!DOCTYPE[^>]*>/gi, "")
        .trim();
    const forceThemeColor = (value) => {
        return value
            ...
            .replace(/style\s*=\s*(['"])([\s\S]*?)\1/gi, (_m, quote, styleText) => {
    ...
    return themed.replace(/<svg\b([^>]*)>/i, (_match, attrs) => {
        let nextAttrs = attrs;
        if (!/\baria-hidden\s*=/.test(nextAttrs))
            nextAttrs += ' aria-hidden="true"';
        if (!/\bfill\s*=/.test(nextAttrs))
