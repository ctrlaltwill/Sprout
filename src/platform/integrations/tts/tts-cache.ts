 ...
 */
export function getTtsCacheDirPath(configDir: string, pluginId: string): string {
...
export function ttsCacheKey(
  text: string,
  provider: SproutSettings["audio"]["ttsProvider"],
  voiceId: string,
  model: string,
  cacheId?: string,
  languageTag = "",
...
async function ensureCacheDir(adapter: AdapterLike, dirPath: string): Promise<boolean> {
  try {
    if (await adapter.exists(dirPath)) return true;
...
export async function clearTtsCache(
  ...
  if (!adapter.list || !adapter.remove) return -1;
  try {
    if (!(await adapter.exists(cacheDirPath))) return 0;
 ...
 */
export async function deleteTtsCacheForCardIds(
  adapter: AdapterLike,
  cacheDirPath: string,
