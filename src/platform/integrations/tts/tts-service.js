...
function clampSpeechValue(value) {
    return Math.max(0.5, Math.min(2.0, Number.isFinite(value) ? value : 1.0));
}
/**
 * Apply local playback tuning for externally generated audio.
 *
 * HTMLAudioElement can change playback speed, but it cannot independently retune
 * the rendered audio clip without also affecting speed. Preserve `rate` as true
 * playback speed and only use `preservesPitch` to avoid accidental chipmunking
 * when the user is not explicitly asking for a pitch-shifted external clip.
 */
export function applyExternalPlaybackSettings(audioEl, settings) {
    const rate = clampSpeechValue(settings.rate);
    const pitch = clampSpeechValue(settings.pitch);
    audioEl.playbackRate = rate;
    const pitchAware = audioEl;
    const preservePitch = Math.abs(pitch - 1.0) < 0.01;
    if ("preservesPitch" in pitchAware)
        pitchAware.preservesPitch = preservePitch;
    if ("mozPreservesPitch" in pitchAware)
        pitchAware.mozPreservesPitch = preservePitch;
    if ("webkitPreservesPitch" in pitchAware)
        pitchAware.webkitPreservesPitch = preservePitch;
    return { playbackRate: rate, pitch };
}
...
function dumpVoiceTable(voices) {
...
if (typeof window !== "undefined") {
    // ── sproutTtsDebug() ──────────────────────────────────────────
    window.sproutTtsDebug = (on = true) => {
        _ttsDebug = !!on;
        if (_ttsDebug) {
            log.debug("[TTS] Debug ON — voice selection will be logged for speak() calls.");
            void ensureVoicesLoaded().then((voices) => dumpVoiceTable(voices)).catch((error) => {
...
export function getScriptLanguageGroups() {
    return [
        {
            key: "arabic",
 ...
 */
function resolveDetectedLanguage(detected, scriptLangs) {
    if (!scriptLangs)
        return detected;
...
const PRIMARY_LANG_DEFAULT_LOCALE = {
    en: "en-US",
    es: "es-ES",
    fr: "fr-FR",
    de: "de-DE",
...
function normaliseLocaleTag(tag) {
    const raw = String(tag !== null && tag !== void 0 ? tag : "").trim();
    if (!raw)
        return "";
...
function languageFromFlagCode(flagCode, fallbackLanguage) {
    var _a;
    const code = String(flagCode !== null && flagCode !== void 0 ? flagCode : "").trim().toLowerCase();
    if (!code)
        return null;
    if (FLAG_CODE_TO_LOCALE[code])
        return FLAG_CODE_TO_LOCALE[code];
    if (code.includes("-")) {
    ...
    return (_a = PRIMARY_LANG_DEFAULT_LOCALE[code]) !== null && _a !== void 0 ? _a : null;
}
function spokenLanguageLabel(lang) {
    var _a;
    const primary = normaliseLocaleTag(lang).split("-")[0] || "";
    return (_a = SPOKEN_LANGUAGE_LABELS[primary]) !== null && _a !== void 0 ? _a : lang;
}
/**
 ...
 */
function voiceQualityScore(v, targetLang) {
    var _a, _b, _c, _d;
    let score = 0;
    const name = v.name.toLowerCase();
    const uri = ((_a = v.voiceURI) !== null && _a !== void 0 ? _a : "").toLowerCase();
    const platform = detectPlatform();
    ...
    if (targetLang) {
        ...
        if (platform === "mac" || platform === "ios") {
            prefs = (_b = APPLE_VOICE_PREFS[full]) !== null && _b !== void 0 ? _b : APPLE_VOICE_PREFS[primary];
        }
        else if (platform === "windows") {
            prefs = (_c = WINDOWS_VOICE_PREFS[full]) !== null && _c !== void 0 ? _c : WINDOWS_VOICE_PREFS[primary];
        }
        else if (platform === "android") {
            prefs = (_d = ANDROID_VOICE_PREFS[full]) !== null && _d !== void 0 ? _d : ANDROID_VOICE_PREFS[primary];
        }
        if (prefs) {
    ...
    if (platform === "mac" || platform === "ios") {
        if (name.includes("premium") || uri.includes("premium"))
            score += 60;
        ...
        if (name.includes("personal"))
            score += 40;
        if (name.includes("compact") || uri.includes("compact"))
            score -= 30;
    ...
    if (platform === "windows") {
        ...
        if (name.includes("online"))
            score += 40;
        if (name.includes("microsoft") && !name.includes("desktop"))
            score += 20;
    ...
    if (platform === "android") {
        ...
        if (name.includes("google"))
            score += 30;
        if (name.includes("network") || uri.includes("network"))
            score += 25;
...
function pickVoice(lang, preferredURI) {
    ...
    const scoreFor = (v) => voiceQualityScore(v, lang);
    ...
    if (exact.length) {
        ...
        if (winnerScore < QUALITY_FLOOR && partial.length) {
            ...
            if (partialBestScore >= QUALITY_FLOOR) {
                ttsLog(`pickVoice("${lang}") — exact winner "${winner.name}" scored ${winnerScore} (below ${QUALITY_FLOOR}). ` +
                    `Using higher-quality regional voice "${partialBest.name}" (score ${partialBestScore}) instead.`);
                if (_ttsDebug) {
                    log.debug("[TTS] Exact + partial ranked voices", [...sorted, ...partialSorted].map((v) => ({ name: v.name, lang: v.lang, score: scoreFor(v), uri: v.voiceURI })));
                }
                return partialBest;
            }
        }
        ...
        if (_ttsDebug) {
            log.debug("[TTS] Exact-match ranked voices", sorted.map((v) => ({ name: v.name, lang: v.lang, score: scoreFor(v), uri: v.voiceURI })));
        }
        return winner;
    }
    if (partial.length) {
        ...
        if (_ttsDebug) {
            log.debug("[TTS] Partial-match ranked voices", sorted.map((v) => ({ name: v.name, lang: v.lang, score: scoreFor(v), uri: v.voiceURI })));
        }
        return winner;
    }
    ttsLog(`pickVoice("${lang}") — no matching voices found. Falling back to speechSynthesis default.`);
    if (_ttsDebug && scoredAll.length) {
        log.debug("[TTS] All voices scored", scoredAll
            .sort((a, b) => b.score - a.score)
...
function replaceSymbolsForSpeech(text) {
    let result = text;
...
function stripToPlainText(md) {
    ...
    text = text.replace(/!\[\[([^\]|]+)(?:\|([^\]]*))?\]\]/g, (_match, _path, alt) => {
        const altText = (alt !== null && alt !== void 0 ? alt : "").trim();
        return altText ? ` ${altText} ` : " ";
    });
    ...
    text = text
        ...
        .replace(/\[\[([^\]|]*?)(?:\|([^\]]*))?\]\]/g, (_match, target, alias) => alias !== null && alias !== void 0 ? alias : target) // wikilinks
...
function buildFlagSpeechSegments(rawText, fallbackLang, settings, autoDetect) {
    var _a, _b;
    const flagEnabled = settings["useFlagsForVoiceSelection"];
    if (flagEnabled === false)
        return [];
    const sourceText = String(rawText !== null && rawText !== void 0 ? rawText : "");
    const matches = getCircleFlagTokenMatches(sourceText);
    if (!matches.length)
        return [];
    const firstLang = languageFromFlagCode((_b = (_a = matches[0]) === null || _a === void 0 ? void 0 : _a.code) !== null && _b !== void 0 ? _b : "", fallbackLang);
 ...
 */
export function prepareClozeTextForTts(clozeText, reveal, targetIndex, defaultLanguage) {
    const blankWord = getBlankWord(defaultLanguage);
    const re = /\{\{c(\d+)::([\s\S]*?)\}\}/g;
    const result = clozeText.replace(re, (_match, idxStr, content) => {
 ...
 * Handles the hint syntax `{{c1::answer::hint}}` by stripping the hint portion.
 */
export function extractClozeAnswerForTts(clozeText, targetIndex) {
    const re = /\{\{c(\d+)::([\s\S]*?)\}\}/g;
    const answers = [];
    let match;
...
export class TtsService {
    ...
    _setSpeaking(value) {
        ...
        for (const fn of this._speakingListeners)
            fn(value);
    }
    _clearResumeKeepAliveInterval() {
    ...
    shouldAutoDetectLanguage(settings) {
        const raw = settings["autoDetectLanguage"];
     ...
     */
    speak(text, lang, settings, autoDetect = true, bypassEnabled = false, cacheId) {
        var _a;
        // Respect the master TTS toggle (unless bypassed for test playback)
        if (!bypassEnabled && !settings.enabled)
            return;
        if (!text.trim())
            return;
        ...
        const provider = (_a = settings.ttsProvider) !== null && _a !== void 0 ? _a : "browser";
        if (provider !== "browser") {
        ...
        const includeLanguageLabel = Boolean(settings["speakFlagLanguageLabel"]);
        ...
        const speakSegmentAt = (index) => {
            ...
            const preferredForThisSegment = !segment.fromFlag && preferredVoiceURI && segmentLang.toLowerCase() === defaultLang.toLowerCase()
                ? preferredVoiceURI
                : undefined;
            ...
            utterance.onstart = () => {
                ...
                if (_ttsDebug) {
                    ...
                    log.debug("[TTS] Speaking started", {
                        ...
                        voiceURI: (_a = actualVoice === null || actualVoice === void 0 ? void 0 : actualVoice.voiceURI) !== null && _a !== void 0 ? _a : "n/a",
                        lang: utterance.lang,
                        rate: utterance.rate,
            ...
            utterance.onerror = (ev) => {
                ...
                if (_ttsDebug) {
                    log.error("[TTS] Utterance error", {
                        ...
                        utteranceVoice: (_b = (_a = utterance.voice) === null || _a === void 0 ? void 0 : _a.name) !== null && _b !== void 0 ? _b : "<default>",
                        segmentIndex: index,
                    });
                }
                speakSegmentAt(index + 1);
            };
            ttsLog(`speak() — segment ${index + 1}/${segments.length}, lang="${segmentLang}", fromFlag=${segment.fromFlag}, text="${spokenText.slice(0, 80)}…"`);
            setTimeout(() => {
                if (session !== this._speakSession)
                    return;
     ...
     */
    _speakExternal(text, lang, settings, cacheId) {
        ...
        const flagEnabled = settings["useFlagsForVoiceSelection"] !== false;
        let effectiveLang;
        ...
        const doSpeak = async () => {
            try {
                ...
                // Check cache first
                if (cacheEnabled) {
                    const key = ttsCacheKey(plainText, settings.ttsProvider, settings.ttsVoiceId || "", settings.ttsModel || "", cacheId, effectiveLang);
                ...
                await audioEl.play();
                ttsLog(`▶ Playing external TTS audio (${(audioData.byteLength / 1024).toFixed(1)} KB, rate=${profile.playbackRate.toFixed(2)}, pitch=${profile.pitch.toFixed(2)})`);
            }
            catch (e) {
                if (session !== this._speakSession)
                    return;
     ...
     */
    speakBasicCard(text, settings, cacheId) {
     ...
     */
    speakClozeCard(clozeText, reveal, targetIndex, settings, cacheId) {
        let prepared;
        ...
        if (isClozeOnly) {
            ...
            prepared = prepareClozeTextForTts(clozeText, reveal, targetIndex, settings.defaultLanguage);
        }
     ...
     */
    speakMcqCard(stem, options, displayOrder, reveal, correctIndices, settings, cacheId) {
    ...
    speakMcqStem(stem, settings, cacheId) {
        if (!stem.trim())
            return;
    ...
    speakMcqOptions(options, displayOrder, settings, cacheId) {
    ...
    speakMcqAnswer(options, displayOrder, correctIndices, settings, cacheId) {
     ...
     */
    speakOqFront(question, steps, settings, cacheId) {
    ...
    speakOqQuestion(question, settings, cacheId) {
        if (!question.trim())
            return;
    ...
    speakOqSteps(steps, settings, cacheId) {
     ...
     */
    speakOqAnswer(steps, pass, settings, cacheId) {
 ...
 */
export function formatMcqTextForTts(stem, options, displayOrder, reveal, correctIndices) {
    var _a, _b;
    ...
    if (!reveal) {
        ...
        const numbered = order.map((origIdx, displayIdx) => { var _a; return `${displayIdx + 1}. ${(_a = options[origIdx]) !== null && _a !== void 0 ? _a : ""}`; });
    ...
    if (correctSet.size <= 1) {
        ...
        const correctOrig = (_a = correctIndices[0]) !== null && _a !== void 0 ? _a : 0;
        const displayNum = order.indexOf(correctOrig) + 1;
        const answerText = (_b = options[correctOrig]) !== null && _b !== void 0 ? _b : "";
        return displayNum > 0
    ...
    const answers = order
        ...
        .map(({ origIdx, displayIdx }) => { var _a; return `${displayIdx + 1}. ${(_a = options[origIdx]) !== null && _a !== void 0 ? _a : ""}`; });
    return `The answers are: ${answers.join(". ")}`;
}
/** Format just the MCQ options as a numbered list for TTS. */
export function formatMcqOptionsForTts(options, displayOrder) {
    const order = displayOrder.length === options.length ? displayOrder : options.map((_, i) => i);
    const numbered = order.map((origIdx, displayIdx) => { var _a; return `${displayIdx + 1}. ${(_a = options[origIdx]) !== null && _a !== void 0 ? _a : ""}`; });
    return numbered.filter(Boolean).join(". ");
}
/** Format just the MCQ answer (correct option(s)) for TTS. */
export function formatMcqAnswerForTts(options, displayOrder, correctIndices) {
    var _a, _b;
    const order = displayOrder.length === options.length ? displayOrder : options.map((_, i) => i);
    const correctSet = new Set(correctIndices);
    if (correctSet.size <= 1) {
 ...
 */
export function formatOqAnswerForTts(steps, pass) {
...
function _restoreTtsTooltip(btn) {
 ...
 */
export function markTtsButtonActive(btn) {
 ...
 */
export function markTtsFieldActive(container, field) {
    const btn = container.querySelector(`.learnkit-tts-replay-btn[data-tts-field="${field}"]`);
    if (!btn)
        return false;
 ...
 */
export function bindTtsPlayingState(btn) {
