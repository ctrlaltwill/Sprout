...
export class TtsProviderError extends Error {
  ...
  constructor(message: string, provider: TtsProvider, status?: number, detail?: string) {
    super(message);
...
export function ttsProviderBaseUrl(audio: SproutSettings["audio"]): string {
  ...
  if (provider === "custom") {
    ...
    if (!isValidHttpUrl(override)) {
      throw new TtsProviderError(
        "Invalid custom TTS endpoint URL: must start with https:// or http://",
        "custom",
      );
    }
  ...
  if (provider === "elevenlabs") return "https://api.elevenlabs.io/v1";
  if (provider === "openai") return "https://api.openai.com/v1";
  if (provider === "google-cloud") return "https://texttospeech.googleapis.com/v1";
  return "";
}

export function ttsProviderApiKey(
  provider: TtsProvider,
  apiKeys: SproutSettings["audio"]["ttsApiKeys"],
): string {
  if (provider === "elevenlabs") return String(apiKeys.elevenlabs || "").trim();
  if (provider === "openai") return String(apiKeys.openai || "").trim();
  if (provider === "google-cloud") return String(apiKeys["google-cloud"] || "").trim();
  return String(apiKeys.custom || "").trim();
}

// ── Provider display labels ─────────────────────────────────────

export function formatTtsProviderLabel(provider: TtsProvider): string {
  if (provider === "browser") return "System (Web Speech API)";
...
export async function requestExternalTts(params: {
  ...
  if (!baseUrl) {
    throw new TtsProviderError(
      provider === "custom"
        ...
        : `No base URL for TTS provider "${provider}"`,
      provider,
    );
  }
  ...
  if (!apiKey) {
    throw new TtsProviderError(
      `No API key configured for TTS provider "${formatTtsProviderLabel(provider)}"`,
      provider,
    );
  }

  if (provider === "elevenlabs") return requestElevenLabsTts(baseUrl, apiKey, text, lang, audio);
  if (provider === "openai") return requestOpenAiTts(baseUrl, apiKey, text, lang, audio);
  if (provider === "google-cloud") return requestGoogleCloudTts(baseUrl, apiKey, text, lang, audio);
  ...
  try {
    ...
    if (response.status < 200 || response.status >= 300) {
      throw new TtsProviderError(
        ...
        "elevenlabs",
        response.status,
        typeof response.text === "string" ? response.text.slice(0, 500) : undefined,
      );
    }
...
async function requestOpenAiTts(
  ...
  if (lang) {
    ...
    if (primary && primary !== "en") {
      const langName = OPENAI_LANG_NAMES[primary] ?? lang;
  ...
  try {
    ...
    if (response.status < 200 || response.status >= 300) {
      throw new TtsProviderError(
        `OpenAI TTS request failed (HTTP ${response.status})`,
        "openai",
        response.status,
        typeof response.text === "string" ? response.text.slice(0, 500) : undefined,
      );
    ...
    if (e instanceof TtsProviderError) throw e;
    throw new TtsProviderError(
      `OpenAI TTS request failed: ${e instanceof Error ? e.message : String(e)}`,
      "openai",
    );
  }
}
...
async function requestGoogleCloudTts(
  baseUrl: string,
  apiKey: string,
  text: string,
  lang: string,
  ...
  try {
    ...
    if (response.status < 200 || response.status >= 300) {
      throw new TtsProviderError(
        `Google Cloud TTS request failed (HTTP ${response.status})`,
        "google-cloud",
        response.status,
        typeof response.text === "string" ? response.text.slice(0, 500) : undefined,
      );
    ...
    if (e instanceof TtsProviderError) throw e;
    throw new TtsProviderError(
      `Google Cloud TTS request failed: ${e instanceof Error ? e.message : String(e)}`,
      "google-cloud",
    );
  }
}
...
async function requestCustomTts(
  baseUrl: string,
  apiKey: string,
  text: string,
  lang: string,
  ...
  try {
    ...
    if (response.status < 200 || response.status >= 300) {
      throw new TtsProviderError(
        `Custom TTS request failed (HTTP ${response.status})`,
        "custom",
        response.status,
        typeof response.text === "string" ? response.text.slice(0, 500) : undefined,
      );
    ...
    if (e instanceof TtsProviderError) throw e;
    throw new TtsProviderError(
      `Custom TTS request failed: ${e instanceof Error ? e.message : String(e)}`,
      "custom",
    );
  }
}
