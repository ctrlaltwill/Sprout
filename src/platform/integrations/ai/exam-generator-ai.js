...
function looksLikePathOnlyLine(line) {
    ...
    if (/^\[\[[^\]]+\]\]$/.test(value))
        return true;
    if (/^[\w .&()'/-]+(?:\.md)?$/i.test(value) && (value.includes("/") || /\.md$/i.test(value)))
        return true;
    return false;
}
function isNonEducationalLine(line) {
    const value = String(line || "").trim();
    if (!value)
        return false;
    if (NON_EDUCATIONAL_PATTERNS.some((re) => re.test(value)))
        return true;
    if (looksLikePathOnlyLine(value))
        return true;
    return false;
}
function filterEducationalContent(raw) {
    ...
    for (const line of lines) {
        ...
        if (inTocBlock) {
            if (trimmed.length === 0)
                continue;
            if (looksLikePathOnlyLine(trimmed))
                continue;
            if (/^(?:[-*+]\s+|\d+\.\s+)/.test(trimmed))
                continue;
            if (trimmed.split(/\s+/).length <= 6 && !/[.!?]/.test(trimmed))
                continue;
        }
        if (isNonEducationalLine(line))
            continue;
        kept.push(line);
    }
...
function educationalSignalScore(raw) {
    ...
    for (const line of lines) {
        ...
        if (words.length >= 8 && /[.!?]|:/.test(value))
            sentenceLikeLines += 1;
        if (words.length <= 5 && !/[.!?]/.test(value))
            shortTitleLike += 1;
...
function isEducationallyUseful(raw) {
    return educationalSignalScore(raw) >= 4;
}
function clip(text, maxChars = MAX_NOTE_CHARS) {
...
function asStringArray(value) {
    if (!Array.isArray(value))
        return [];
    return value.map((v) => String(v !== null && v !== void 0 ? v : "").trim()).filter((v) => v.length > 0);
}
...
const EXPLICIT_MULTI_SELECT_PROMPT_PATTERNS = [
...
const EXPLICIT_MULTI_SELECT_INSTRUCTION_PATTERNS = [
    /\bselect\s+all\s+that\s+apply\b/i,
    /\ball\s+that\s+apply\b/i,
    /\b(?:choose|select|pick|identify)\s+(?:the\s+)?(?:two|three|four|five|six|\d+)\b/i,
];
function normaliseMcqSelectionMode(value) {
    const mode = asText(value).trim().toLowerCase();
    if (!mode)
        return null;
    if (["single", "single-select", "single select", "single_best", "single-best", "one"].includes(mode))
        return "single";
...
function getMcqSelectionMode(row) {
...
function promptSignalsMultiSelect(prompt) {
    return EXPLICIT_MULTI_SELECT_PROMPT_PATTERNS.some((re) => re.test(prompt));
}
function promptHasExplicitMultiSelectInstruction(prompt) {
    return EXPLICIT_MULTI_SELECT_INSTRUCTION_PATTERNS.some((re) => re.test(prompt));
}
function ensureExplicitMultiSelectPrompt(prompt) {
    const trimmed = String(prompt || "").trim();
    if (!trimmed)
        return trimmed;
    if (promptHasExplicitMultiSelectInstruction(trimmed))
        return trimmed;
    return `Select all that apply: ${trimmed}`;
}
function normaliseQuestion(candidate, index, fallbackPath, issues) {
    const fail = (reason) => {
        issues === null || issues === void 0 ? void 0 : issues.push(`Question ${index + 1}: ${reason}`);
        return null;
    };
    const row = toRecord(candidate);
    if (!row)
        return fail("item was not an object.");
    const typeRaw = asText(row.type).trim().toLowerCase();
    const type = typeRaw === "saq" ? "saq" : typeRaw === "mcq" || typeRaw === "msq" ? "mcq" : null;
    if (!type)
        return fail(`unsupported question type "${typeRaw || "(empty)"}".`);
    const prompt = asText(row.prompt).trim();
    if (!prompt)
        return fail("prompt was empty.");
    ...
    if (type === "mcq") {
        ...
        if (options.length < 2)
            return fail("MCQ requires at least 2 options.");
        ...
        if (!Number.isFinite(idx))
            return fail("single-select MCQ requires correctIndex.");
...
function allowedQuestionTypesForRequest(requestedType) {
    if (requestedType === "mixed")
        return new Set(["mcq", "saq"]);
    return new Set([requestedType]);
}
function normaliseGeneratedQuestions(payload, questionCount, fallbackPath, requestedType) {
    const obj = toRecord(payload);
    if (!obj || !Array.isArray(obj.questions))
        return { questions: [], issues: ["Response did not contain a questions array."] };
    const out = [];
    const issues = [];
    const seen = new Set();
...
function allocateQuestionCounts(total, chunks) {
    if (chunks <= 0)
        return [];
    if (total <= 0)
        return Array.from({ length: chunks }, () => 1);
...
function dedupeQuestions(questions, questionCount) {
...
async function requestExamChunk(params) {
    const { settings, systemPrompt, notes, difficulty, requestedType, questionCount, fallbackPath, idPrefix, attachedFileDataUrls, } = params;
    const maxAttempts = 2;
    let lastError = null;
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        const userPrompt = JSON.stringify({
            ...
            strictRequirements: {
                ...
                allowedQuestionTypes: requestedType === "mixed"
                    ? ["mcq", "msq", "saq"]
                    : requestedType === "mcq"
                        ? ["mcq", "msq"]
                        : [requestedType],
...
export async function generateExamQuestions(params) {
    ...
    const systemPrompt = [
        ...
        ...(config.includeFlashcards
            ...
            : []),
        ...(((_a = config.customInstructions) === null || _a === void 0 ? void 0 : _a.trim())
            ? [
    ...
    const fallbackPath = ((_b = notesForGeneration[0]) === null || _b === void 0 ? void 0 : _b.path) || "";
    let questions = [];
    if (notesForGeneration.length <= EXAM_CHUNK_SIZE) {
        ...
        for (let i = 0; i < chunks.length; i += 1) {
            ...
            const chunkQuestions = await requestExamChunk({
                ...
                fallbackPath: ((_c = chunkNotes[0]) === null || _c === void 0 ? void 0 : _c.path) || fallbackPath,
                idPrefix: `chunk${i + 1}`,
                attachedFileDataUrls,
            });
...
export async function gradeSaqAnswer(params) {
    ...
    if (!obj)
        throw new Error("SAQ marker returned invalid result.");
...
export async function gradeFullTest(params) {
    ...
    const obj = toRecord(parsed);
    if (!obj)
        throw new Error("Test grader returned invalid result.");
    const rawResults = Array.isArray(obj.results) ? obj.results : [];
    const results = rawResults.map((r) => {
        const rec = toRecord(r);
        if (!rec)
            return { questionNumber: 0, correct: false, scorePercent: 0, feedback: "No result." };
...
export async function suggestTestName(params) {
    ...
    const systemPrompt = "Suggest a short (2-5 word) descriptive test name based on the topics covered by the questions below. " +
