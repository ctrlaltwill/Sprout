...
import { buildStudyAssistantNoteContext, resolveStudyAssistantAttachmentRefs, } from "./study-assistant-note-context";
import { generateStudyAssistantChatReply, generateStudyAssistantSuggestions, parseEditProposal, } from "./study-assistant-generator";
import { providerApiKey, providerSupportsNativeDocumentAttachment, } from "./study-assistant-provider";
const DEFAULT_NOTE_PATH = "test.md";
const DEFAULT_NOTE_PATH_CANDIDATES = [
    DEFAULT_NOTE_PATH,
    `Test/${DEFAULT_NOTE_PATH}`,
];
const DEFAULT_ATTACHMENT_REFS = {
    docx: "test.docx",
    pptx: "test.pptx",
    pdf: "test.pdf",
    image: "test.jpg",
};
const DEFAULT_FLASHCARD_TYPES = ["basic", "cloze", "mcq", "oq"];
...
function cloneStudyAssistantSettings(settings) {
    if (typeof structuredClone === "function")
        return structuredClone(settings);
    return JSON.parse(JSON.stringify(settings));
}
function formatFallbackText(template, vars) {
...
function txFallback(_token, fallback, vars) {
    return formatFallbackText(fallback, vars);
}
function normalizeScenarioSettings(settings, scenario) {
...
function resolveModelSpecs(host, options) {
...
function resolveScenarioList(options) {
    if (Array.isArray(options.scenarios) && options.scenarios.length)
        return options.scenarios;
    return DEFAULT_SCENARIOS;
}
function resolveNoteFile(host, notePath) {
    ...
    if (!fallbackPath)
        resolved = host.app.workspace.getActiveFile();
    if (!(resolved instanceof TFile) || String(resolved.extension || "").toLowerCase() !== "md") {
...
function extractMarkdownSections(text) {
    ...
    for (const line of String(text || "").split(/\r?\n/)) {
        ...
        if (heading === null || heading === void 0 ? void 0 : heading[1]) {
        ...
        if (!currentKey)
            continue;
        (_a = sections.get(currentKey)) === null || _a === void 0 ? void 0 : _a.push(line);
    }
    return sections;
}
...
function validateMarkdownSections(reply, sections) {
    const text = String(reply || "").trim();
    if (!text)
        return { ok: false, detail: "Empty reply." };
    const parsed = extractMarkdownSections(text);
    for (const section of sections) {
        const lines = parsed.get(section.heading.toLowerCase());
        if (!lines)
            return { ok: false, detail: `Missing section: ${section.heading}` };
...
function validateJsonObject(reply, rules) {
    var _a, _b;
    ...
    for (const rule of rules) {
        ...
        if (!Array.isArray(value) || value.length < ((_a = rule.minItems) !== null && _a !== void 0 ? _a : 1)) {
            return {
                ok: false,
                detail: `Field ${rule.key} did not contain at least ${(_b = rule.minItems) !== null && _b !== void 0 ? _b : 1} item(s).`,
            };
        }
    }
    return { ok: true, detail: "Reply matched expected JSON structure." };
}
function combineValidationResults(primary, secondary) {
    if (!primary.ok)
        return primary;
    if (!secondary.ok)
        return secondary;
    if (!secondary.detail || secondary.detail === primary.detail)
        return primary;
    return {
        ok: true,
...
function validateExpectedEvidence(reply, label, evidence, minMatches) {
...
function buildScenarioPlans(scenarios) {
    const plans = [];
...
function validateChatLikeResult(plan, reply) {
    ...
    if (plan.feature === "chat" && plan.scenario === "linked-note-context") {
        return combineValidationResults(validateMarkdownSections(reply, [
    ...
    if (plan.scenario === "explicit-docx" || plan.scenario === "docx-fallback") {
        return combineValidationResults(validateJsonObject(reply, [
...
function attachmentRouteFromUnknown(error) {
    if (!error || typeof error !== "object")
        return "none";
    const route = error.attachmentRoute;
    return route === "native" || route === "retry-fallback" || route === "forced-fallback" || route === "none"
        ? route
        : "none";
}
function attachmentRoutingSummary(provider, imageDataUrls, attachedFileDataUrls, attachmentRoute) {
    ...
    for (const raw of attachedFileDataUrls) {
        ...
        if (forceDocumentFallback)
            fallbackTextCount += 1;
        else if (providerSupportsNativeDocumentAttachment(provider, mimeType))
            nativeCount += 1;
        else
            fallbackTextCount += 1;
    }
    ...
    if (nativeCount > 0)
        parts.push(`native:${nativeCount}`);
    if (fallbackTextCount > 0)
        parts.push(`fallback-text:${fallbackTextCount}`);
...
function logRunResult(result, title = "LearnKit AI compatibility matrix") {
    ...
    for (const row of result.rows) {
        ...
        if (row.attachmentRefs.length)
            console.debug("Attachment refs:", row.attachmentRefs);
        if (row.payloadPreview)
            console.debug("Payload preview:\n", row.payloadPreview);
        if (row.rawResponseText)
...
async function runScenarioPlan(params) {
    ...
    const resolvedAttachments = ((_a = plan.attachmentKinds) === null || _a === void 0 ? void 0 : _a.length)
    ...
    const finalizeRow = (row) => ({
        ...
        attachmentRouting: attachmentRoutingSummary(modelSpec.provider, context.imageDataUrls, context.attachedFileDataUrls, row.attachmentRoute),
    ...
    try {
        ...
        if (plan.feature === "edit") {
            ...
            const validation = validateEditProposal(proposal.edits, context.noteContent, mentionsFrontmatter(plan.userMessage));
            return finalizeRow({
...
export async function runStudyAssistantMatrix(host, options = {}) {
    ...
    if (options.printToConsole !== false && modelSpecs.length > 1)
        logRunResult(result, "LearnKit AI compatibility matrix (all models complete)");
    if (options.printToConsole !== false && modelSpecs.length <= 1)
        logRunResult(result);
    return result;
}
export function createStudyAssistantMatrixConsoleApi(host) {
    const api = {
        lastResult: null,
