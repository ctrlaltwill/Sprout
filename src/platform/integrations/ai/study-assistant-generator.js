...
function combineAttachmentRoutes(...routes) {
    if (routes.includes("retry-fallback"))
        return "retry-fallback";
    if (routes.includes("forced-fallback"))
        return "forced-fallback";
    if (routes.includes("native"))
        return "native";
    return "none";
}
...
function extractTaggedEditProposal(text) {
    const raw = String(text || "");
...
function trimTrailingMarkerFragment(text, marker) {
...
export function extractVisibleEditResponseText(rawText) {
...
function topicsLikelyEquivalent(a, b) {
...
function hasClozeDeletion(value) {
    return /\{\{c\d+::[\s\S]+?\}\}/i.test(String(value || ""));
}
function buildExistingFlashcardTopics(noteContent) {
    ...
    for (const line of lines) {
        ...
        if (!row)
            continue;
        if (!["Q", "RQ", "MCQ", "OQ", "CQ"].includes(row.key))
            continue;
        out.push(row.value);
    }
    const deduped = [];
    for (const topic of out) {
        if (!topic)
            continue;
        if (deduped.some((existing) => topicsLikelyEquivalent(existing, topic)))
            continue;
        deduped.push(topic);
    }
    return deduped.slice(0, 60);
}
function extractSuggestionTopics(s) {
    const out = [];
...
function userExplicitlyAllowsRepeatTopics(input) {
    const text = `${coerceString(input.userRequestText)}\n${coerceString(input.customInstructions)}`.toLowerCase();
    if (!text)
        return false;
    if (/\b(do not|don't|avoid|without)\b[\s\S]{0,30}\b(repeat|duplicate|same topics?)\b/i.test(text))
        return false;
    const allowPatterns = [
...
export function parseUserRequestOverrides(text) {
    var _a, _b;
    ...
    while ((match = countTypePattern.exec(t)) !== null) {
        ...
        if (mapped) {
            result.count = ((_a = result.count) !== null && _a !== void 0 ? _a : 0) + n;
            result.exactCountRequested = true;
            perTypeCounts.set(mapped, ((_b = perTypeCounts.get(mapped)) !== null && _b !== void 0 ? _b : 0) + n);
            if (!detectedTypes.includes(mapped))
                detectedTypes.push(mapped);
        }
    }
    ...
    if (!detectedTypes.length) {
        while ((match = typeOnlyPattern.exec(t)) !== null) {
            ...
            typeKey = typeKey
                ...
                .replace(/s$/, "");
            const mapped = TYPE_ALIASES[typeKey];
            if (mapped && !detectedTypes.includes(mapped))
                detectedTypes.push(mapped);
        }
    }
    ...
    if (result.count != null)
        result.count = Math.max(1, Math.floor(result.count));
    if (detectedTypes.length)
        result.types = detectedTypes;
    if (perTypeCounts.size > 0)
        result.perTypeCounts = perTypeCounts;
    result.topic = extractRequestedTopic(t);
    return result;
}
function extractRequestedTopic(value) {
    ...
    for (const re of patterns) {
        ...
        if (!topic)
            continue;
        if (/^(topic|note|this topic|this note)$/i.test(topic))
            continue;
        return topic;
    }
    return "";
}
function userExplicitlyRequestsExternalKnowledge(input) {
    const text = `${coerceString(input.userRequestText)}\n${coerceString(input.customInstructions)}`.toLowerCase();
    if (!text)
        return false;
    return [
...
function buildGenerationIntent(input, overrides) {
    var _a, _b;
    const baseTarget = Math.max(1, Math.min(10, Math.round(Number(input.targetSuggestionCount) || 5)));
    const requestedCount = (_a = overrides.count) !== null && _a !== void 0 ? _a : baseTarget;
    return {
        requestedCount,
        exactCountRequested: !!overrides.exactCountRequested,
        requestedTypes: ((_b = overrides.types) === null || _b === void 0 ? void 0 : _b.length) ? [...new Set(overrides.types)] : null,
...
function suggestionTextForRelevance(s) {
...
function topicRelevanceScore(topic, suggestion) {
...
function topicRelevanceThreshold(topic) {
...
function validateAndRepairSuggestions(intent, suggestions, noteContent) {
    var _a;
    const out = [];
    ...
    for (const suggestion of suggestions) {
        if (((_a = intent.requestedTypes) === null || _a === void 0 ? void 0 : _a.length) && !intent.requestedTypes.includes(suggestion.type))
            continue;
        if (!intent.allowExternalKnowledge && suggestion.sourceOrigin === "external")
            continue;
        ...
        if (intent.avoidRepeatingExistingTopics && suggestionTopics.length > 0) {
            const duplicatesExisting = suggestionTopics.some((topic) => existingTopics.some((existing) => topicsLikelyEquivalent(topic, existing)));
        ...
        const duplicatesOutput = out.some((existingSuggestion) => {
            ...
            if (!existingSuggestionTopics.length || !suggestionTopics.length)
                return false;
            return suggestionTopics.some((topic) => existingSuggestionTopics.some((existingTopic) => topicsLikelyEquivalent(topic, existingTopic)));
        });
        if (duplicatesOutput)
            continue;
        out.push(suggestion);
...
function resolveSuggestionsWithFallback(intent, suggestions, noteContent) {
    var _a;
    const strict = validateAndRepairSuggestions(intent, suggestions, noteContent);
    if (strict.length > 0 || suggestions.length === 0)
        return strict;
...
function complexityScoreForSuggestion(s) {
    var _a;
    ...
    return (((_a = typeWeight[s.type]) !== null && _a !== void 0 ? _a : 1)
        + promptTokens * 0.06
        + answerTokens * 0.03
        + optionsCount * 0.07
        + stepsCount * 0.08
...
function assignDifficultyLevels(suggestions) {
...
function isLikelyOpenEndedPrompt(value) {
...
function isReversedPairSafe(question, answer) {
    const q = question.trim();
    const a = answer.trim();
    if (!q || !a)
        return false;
    // Reversed cards should be compact, atomic mappings in both directions.
    if (q.length > 80 || a.length > 56)
        return false;
    if (wordCount(q) > 10 || wordCount(a) > 8)
        return false;
    if (isLikelyOpenEndedPrompt(q))
        return false;
    return true;
}
function downgradeReversedRowsToBasic(rows) {
...
function normalizeIoMaskMode(value) {
    const v = coerceString(value).toLowerCase();
    if (v === "solo" || v === "all")
        return v;
    return undefined;
}
function normalizeIoGroupKey(value, fallbackIndex) {
    ...
    if (m === null || m === void 0 ? void 0 : m[0])
        return String(Math.max(1, parseInt(m[0], 10)));
    return String(fallbackIndex);
}
function normalizeIoAssessmentImageRef(value) {
...
function normalizeIoImageKind(value) {
    const raw = coerceString(value).toLowerCase().replace(/[\s_]+/g, "-");
    if (!raw)
        return undefined;
    if (raw.includes("annotated") && raw.includes("photo"))
        return "annotated-photo";
    if (raw.includes("diagram") || raw.includes("schematic") || raw.includes("illustration") || raw.includes("anatomy"))
        return "diagram";
    if (raw.includes("flow") || raw.includes("pathway"))
        return "flowchart";
    if (raw.includes("chart") || raw.includes("graph"))
        return "chart";
    if (raw.includes("table"))
        return "table";
    if (raw.includes("map"))
        return "map";
    if (raw.includes("screen") || raw.includes("screenshot") || raw.includes("ui"))
        return "screenshot";
    if (raw.includes("scan") || raw.includes("xray") || raw.includes("ct") || raw.includes("mri") || raw.includes("ultrasound"))
        return "clinical-photo";
    if (raw.includes("photo") || raw.includes("picture") || raw.includes("image"))
        return "photo";
    return raw;
}
function normalizeIoStudyValue(value) {
    const raw = coerceString(value).toLowerCase().replace(/[\s_]+/g, "-");
    if (!raw)
        return undefined;
    if (["high", "high-yield", "strong", "useful", "study-worthy"].includes(raw))
        return "high";
    if (["medium", "moderate", "possible", "borderline"].includes(raw))
        return "medium";
    if (["low", "weak"].includes(raw))
        return "low";
    if (["none", "decorative", "skip", "not-useful", "not-study-worthy"].includes(raw))
        return "none";
    return undefined;
}
function normalizeIoPlacementConfidence(value) {
...
function normalizeIoAssessment(value) {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m, _o, _p, _q, _r, _s;
    if (!value || typeof value !== "object")
        return undefined;
    const rec = value;
    const targetLabels = toStringArray((_b = (_a = rec.targetLabels) !== null && _a !== void 0 ? _a : rec.targets) !== null && _b !== void 0 ? _b : rec.labels)
        .map((label) => label.trim())
        .filter(Boolean)
        .slice(0, 12);
    const assessment = {
        imageRef: normalizeIoAssessmentImageRef((_e = (_d = (_c = rec.imageRef) !== null && _c !== void 0 ? _c : rec.ref) !== null && _d !== void 0 ? _d : rec.image) !== null && _e !== void 0 ? _e : rec.ioSrc),
        imageKind: normalizeIoImageKind((_h = (_g = (_f = rec.imageKind) !== null && _f !== void 0 ? _f : rec.kind) !== null && _g !== void 0 ? _g : rec.imageType) !== null && _h !== void 0 ? _h : rec.visualType),
        studyValue: normalizeIoStudyValue((_l = (_k = (_j = rec.studyValue) !== null && _j !== void 0 ? _j : rec.usefulness) !== null && _k !== void 0 ? _k : rec.yield) !== null && _l !== void 0 ? _l : rec.studyUsefulness),
        placementConfidence: normalizeIoPlacementConfidence((_p = (_o = (_m = rec.placementConfidence) !== null && _m !== void 0 ? _m : rec.confidence) !== null && _o !== void 0 ? _o : rec.maskConfidence) !== null && _p !== void 0 ? _p : rec.localizationConfidence),
        targetLabels: targetLabels.length ? targetLabels : undefined,
        usefulnessReason: coerceString((_r = (_q = rec.usefulnessReason) !== null && _q !== void 0 ? _q : rec.reason) !== null && _r !== void 0 ? _r : rec.justification) || undefined,
    };
    if (!assessment.imageRef && !assessment.imageKind && !assessment.studyValue && assessment.placementConfidence === undefined && !((_s = assessment.targetLabels) === null || _s === void 0 ? void 0 : _s.length) && !assessment.usefulnessReason) {
        return undefined;
    }
    return assessment;
}
function isHighYieldIoAssessment(value) {
    var _a;
    if (!value)
        return false;
    if (value.studyValue !== "high")
        return false;
    if (((_a = value.placementConfidence) !== null && _a !== void 0 ? _a : 0) < MIN_IO_PLACEMENT_CONFIDENCE)
        return false;
    if (!Array.isArray(value.targetLabels) || value.targetLabels.length === 0)
        return false;
    if (value.imageKind && LOW_VALUE_IO_IMAGE_KINDS.has(value.imageKind))
        return false;
    return true;
}
function normalizeAllowedIoImageRefs(input) {
...
function normalizeIoTargetLabel(value) {
    return String(value || "")
        .toLowerCase()
        .replace(/[-_]+/g, " ")
...
function tokenizeIoTargetLabel(value) {
    const normalized = normalizeIoTargetLabel(value);
    return normalized ? normalized.split(" ").filter(Boolean) : [];
}
function normalizeOcrIoRegion(value) {
    var _a;
    if (!value || typeof value !== "object")
        return null;
    const rec = value;
    ...
    if (w <= 0 || h <= 0)
        return null;
    const confidence = toFiniteNumber((_a = rec.confidence) !== null && _a !== void 0 ? _a : rec.score);
    return {
        text,
...
function buildIoOcrRegionMap(input) {
    const descriptors = Array.isArray(input.imageDescriptors) ? input.imageDescriptors : [];
    const out = new Map();
    const seenByRef = new Map();
    for (const descriptor of descriptors) {
        const imageRef = normalizeIoAssessmentImageRef(descriptor === null || descriptor === void 0 ? void 0 : descriptor.ref);
        if (!imageRef)
            continue;
        const rawRegions = Array.isArray(descriptor === null || descriptor === void 0 ? void 0 : descriptor.ocrTextRegions) ? descriptor.ocrTextRegions : [];
...
function scoreIoTargetLabelMatch(label, regionText) {
    ...
    let score = f1;
    if (normalizedRegion.includes(normalizedLabel))
        score = Math.min(0.96, score + 0.04);
    else if (normalizedLabel.includes(normalizedRegion))
        score = Math.min(0.9, score + 0.02);
    return clamp01(score);
}
function ocrRegionArea(region) {
    return Math.max(0, region.w) * Math.max(0, region.h);
}
function isPlausibleIoOcrRegion(region) {
    ...
    if (touchesEdge && (region.w > 0.28 || region.h > 0.16))
        return false;
    return true;
}
function dedupeIoOcclusions(rects) {
    const seen = new Set();
    const out = [];
...
function applyOcrTextRegionsToIoSuggestion(suggestion, ocrRegionsByImageRef) {
    ...
    if (!(ocrRegions === null || ocrRegions === void 0 ? void 0 : ocrRegions.length))
        return suggestion;
    const targetLabels = Array.isArray((_a = suggestion.ioAssessment) === null || _a === void 0 ? void 0 : _a.targetLabels)
    ...
    const usedRegionIndexes = new Set();
    const matches = [];
    for (const label of labels) {
        let bestIndex = -1;
        let bestScore = 0;
        for (let idx = 0; idx < ocrRegions.length; idx += 1) {
            if (usedRegionIndexes.has(idx))
                continue;
            const region = ocrRegions[idx];
            if (!isPlausibleIoOcrRegion(region))
                continue;
            ...
            if (score === bestScore && bestIndex >= 0) {
                ...
                const regionConfidence = (_b = region.confidence) !== null && _b !== void 0 ? _b : 0;
                const currentBestConfidence = (_d = (_c = ocrRegions[bestIndex]) === null || _c === void 0 ? void 0 : _c.confidence) !== null && _d !== void 0 ? _d : 0;
    ...
    const placementConfidence = Math.max((_f = (_e = suggestion.ioAssessment) === null || _e === void 0 ? void 0 : _e.placementConfidence) !== null && _f !== void 0 ? _f : 0, clamp01(averageScore * 0.7 + coverage * 0.3));
    ...
    if (Array.isArray(nextSuggestion.noteRows) && nextSuggestion.noteRows.length) {
        nextSuggestion.noteRows = upsertIoRows(nextSuggestion.noteRows, extractIoImageKey(nextSuggestion), nextOcclusions, nextSuggestion.ioMaskMode);
    }
    return nextSuggestion;
}
function applyOcrTextRegionsToSuggestions(suggestions, input) {
    const ocrRegionsByImageRef = buildIoOcrRegionMap(input);
    if (!ocrRegionsByImageRef.size)
        return suggestions;
    return suggestions
        .map((suggestion) => applyOcrTextRegionsToIoSuggestion(suggestion, ocrRegionsByImageRef))
        .filter((suggestion) => !!suggestion);
}
function allowRuntimeSuggestion(suggestion, canUseVisionForIo, allowedIoImageRefs) {
    if (suggestion.type !== "io")
        return true;
    if (!canUseVisionForIo)
        return false;
    const imageRef = normalizeIoAssessmentImageRef(extractIoImageKey(suggestion));
    if (!imageRef)
        return false;
    if (allowedIoImageRefs.size > 0 && !allowedIoImageRefs.has(imageRef))
        return false;
    return true;
}
function extractIoImageKey(suggestion) {
    var _a;
    const direct = coerceString(suggestion.ioSrc);
    if (direct)
        return direct;
    const assessedRef = coerceString((_a = suggestion.ioAssessment) === null || _a === void 0 ? void 0 : _a.imageRef);
    if (assessedRef)
        return assessedRef;
    const rows = Array.isArray(suggestion.noteRows) ? suggestion.noteRows : [];
    for (const rowText of rows) {
        const row = parseDelimitedRow(String(rowText || ""));
        if ((row === null || row === void 0 ? void 0 : row.key) === "IO")
            return coerceString(row.value);
    }
    return "";
}
function canonicalizeIoSuggestionSrc(raw) {
    const refs = extractIoImageRefs(raw);
    if (refs.length !== 1)
        return null;
    return `![[${refs[0]}]]`;
}
function upsertIoRows(rows, ioSrc, ioOcclusions, ioMaskMode) {
    ...
    if (ioSrc)
        keptRows.unshift(`IO | ${ioSrc} |`);
    return ensureIoMaskRows(keptRows, ioOcclusions, ioMaskMode);
}
function mergeIoSuggestionsByImage(suggestions) {
    var _a, _b, _c, _d, _e, _f, _g, _h;
    const out = [];
    const mergedByImage = new Map();
    for (const suggestion of suggestions) {
        if (suggestion.type !== "io") {
            out.push(suggestion);
            continue;
        ...
        if (!existing) {
            const normalizedOcclusions = dedupeIoOcclusions((Array.isArray(suggestion.ioOcclusions) ? suggestion.ioOcclusions : [])
                .map((rect, idx) => {
                var _a;
                return ({
                    ...rect,
                    rectId: coerceString((_a = rect.rectId) !== null && _a !== void 0 ? _a : rect.id) || `r${idx + 1}`,
                    groupKey: String(idx + 1),
                });
            }));
            const nextSuggestion = {
            ...
            if (Array.isArray(nextSuggestion.noteRows) && nextSuggestion.noteRows.length) {
                nextSuggestion.noteRows = upsertIoRows(nextSuggestion.noteRows, extractIoImageKey(nextSuggestion), normalizedOcclusions, nextSuggestion.ioMaskMode);
            }
        ...
        existing.ioAssessment = existing.ioAssessment || suggestion.ioAssessment
            ? {
                ...
                ...(suggestion.ioAssessment || {}),
                imageRef: normalizeIoAssessmentImageRef((_a = existing.ioAssessment) === null || _a === void 0 ? void 0 : _a.imageRef)
                    || normalizeIoAssessmentImageRef((_b = suggestion.ioAssessment) === null || _b === void 0 ? void 0 : _b.imageRef)
                    || normalizeIoAssessmentImageRef(imageKey),
                placementConfidence: Math.max((_d = (_c = existing.ioAssessment) === null || _c === void 0 ? void 0 : _c.placementConfidence) !== null && _d !== void 0 ? _d : 0, (_f = (_e = suggestion.ioAssessment) === null || _e === void 0 ? void 0 : _e.placementConfidence) !== null && _f !== void 0 ? _f : 0) || undefined,
                targetLabels: Array.from(new Set([
                    ...(((_g = existing.ioAssessment) === null || _g === void 0 ? void 0 : _g.targetLabels) || []),
                    ...(((_h = suggestion.ioAssessment) === null || _h === void 0 ? void 0 : _h.targetLabels) || []),
                ])).filter(Boolean),
            }
            : undefined;
        if (Array.isArray(existing.noteRows) && existing.noteRows.length) {
            existing.noteRows = upsertIoRows(existing.noteRows, extractIoImageKey(existing), mergedOcclusions, existing.ioMaskMode);
        }
    }
    return out;
}
function toIoOcclusionsArray(value) {
    ...
    for (let i = 0; i < rawItems.length; i += 1) {
        ...
        if (!item || typeof item !== "object")
            continue;
        const rec = item;
        const xRaw = toFiniteNumber((_a = rec.x) !== null && _a !== void 0 ? _a : rec.left);
        const yRaw = toFiniteNumber((_b = rec.y) !== null && _b !== void 0 ? _b : rec.top);
        const wRaw = toFiniteNumber((_c = rec.w) !== null && _c !== void 0 ? _c : rec.width);
        const hRaw = toFiniteNumber((_d = rec.h) !== null && _d !== void 0 ? _d : rec.height);
        if (xRaw === null || yRaw === null || wRaw === null || hRaw === null)
            continue;
        if (wRaw <= 0 || hRaw <= 0)
            continue;
        const rectId = coerceString((_e = rec.rectId) !== null && _e !== void 0 ? _e : rec.id) || `r${i + 1}`;
        const groupKey = normalizeIoGroupKey((_g = (_f = rec.groupKey) !== null && _f !== void 0 ? _f : rec.label) !== null && _g !== void 0 ? _g : rec.name, i + 1);
        const shapeRaw = coerceString(rec.shape).toLowerCase();
        const shape = shapeRaw === "circle" ? "circle" : "rect";
        out.push({
            rectId,
            x: clamp01(xRaw),
            y: clamp01(yRaw),
...
function rowHasKey(rows, key) {
    ...
    return rows.some((line) => re.test(String(line !== null && line !== void 0 ? line : "")));
}
function ensureIoMaskRows(rows, ioOcclusions, ioMaskMode) {
...
function sanitizeSuggestion(raw) {
    ...
    let suggestion = {
        ...
        ioOcclusions: toIoOcclusionsArray((_c = (_b = (_a = rec.ioOcclusions) !== null && _a !== void 0 ? _a : rec.occlusions) !== null && _b !== void 0 ? _b : rec.ioMasks) !== null && _c !== void 0 ? _c : rec.masks),
        ioMaskMode: normalizeIoMaskMode((_e = (_d = rec.ioMaskMode) !== null && _d !== void 0 ? _d : rec.maskMode) !== null && _e !== void 0 ? _e : rec.mode),
        ioAssessment: normalizeIoAssessment((_g = (_f = rec.ioAssessment) !== null && _f !== void 0 ? _f : rec.imageAssessment) !== null && _g !== void 0 ? _g : rec.ioAnalysis),
    ...
    if (suggestion.type === "cloze") {
        ...
        if (!hasDeletionInClozeText && !hasDeletionInClozeRows)
            return null;
        if (hasNoteRows && (!clozeRows.length || hasInvalidQaRows))
            return null;
    }
    if (suggestion.type === "mcq") {
        if (!hasNoteRows) {
            ...
            if (!suggestion.options || suggestion.options.length < 2)
                return null;
            if (!suggestion.correctOptionIndexes || suggestion.correctOptionIndexes.length < 1)
                return null;
        }
    }
    ...
    if (suggestion.type === "io") {
        ...
        if (!ioSrc)
            return null;
        if (!Array.isArray(suggestion.ioOcclusions) || suggestion.ioOcclusions.length === 0)
            return null;
        const ioImageRef = normalizeIoAssessmentImageRef(ioSrc);
        const ioAssessment = suggestion.ioAssessment;
        if (!isHighYieldIoAssessment(ioAssessment))
            return null;
        ...
        if (hasNoteRows) {
            suggestion = {
                ...suggestion,
                noteRows: upsertIoRows(suggestion.noteRows || [], ioSrc, suggestion.ioOcclusions || [], suggestion.ioMaskMode),
            };
        }
    }
    return suggestion;
...
function findStreamingSuggestionArrayStart(rawText) {
...
function extractStreamingJsonArrayItems(rawText) {
    const value = String(rawText || "");
...
function parseStreamingSuggestions(rawText) {
...
function modelLikelySupportsVision(settings) {
...
function buildSystemPrompt(customInstructions, canUseVisionForIo) {
...
function buildChatUserPrompt(input) {
    ...
    const historyBlock = ((_a = input.conversationHistory) === null || _a === void 0 ? void 0 : _a.length)
...
function buildUserPrompt(input, canUseVisionForIo, overrides) {
    var _a, _b;
    const baseTarget = Math.max(1, Math.min(10, Math.round(Number(input.targetSuggestionCount) || 5)));
    const exactTarget = overrides.exactCountRequested && overrides.count != null
        ? Math.max(1, Math.min(20, overrides.count))
        : null;
    const target = exactTarget !== null && exactTarget !== void 0 ? exactTarget : baseTarget;
    const minCount = exactTarget !== null && exactTarget !== void 0 ? exactTarget : Math.max(1, target - 1);
    const maxCount = exactTarget !== null && exactTarget !== void 0 ? exactTarget : Math.min(10, target + 1);
    const effectiveTypes = ((_a = overrides.types) === null || _a === void 0 ? void 0 : _a.length)
    ...
    const imageDescriptors = Array.isArray(input.imageDescriptors)
        ...
        : [];
    const visionDescriptorRefs = new Set((Array.isArray(input.imageDataUrls) ? input.imageRefs.slice(0, input.imageDataUrls.filter(Boolean).length) : input.imageRefs)
        .map((ref) => normalizeIoAssessmentImageRef(ref))
        .filter((ref) => !!ref));
    const preview = {
        notePath: input.notePath,
    ...
    const optionalRowsInstructions = [];
    ...
    if ((_b = overrides.types) === null || _b === void 0 ? void 0 : _b.length) {
...
export async function generateStudyAssistantSuggestions(params) {
    ...
    if (exactRequested && ((_a = overrides.count) !== null && _a !== void 0 ? _a : 0) > 20) {
    ...
    const baseTarget = Math.max(1, Math.min(10, Math.round(Number(input.targetSuggestionCount) || 5)));
    const target = exactRequested ? Math.max(1, Math.min(20, (_b = overrides.count) !== null && _b !== void 0 ? _b : 1)) : ((_c = overrides.count) !== null && _c !== void 0 ? _c : baseTarget);
    const maxAllowed = exactRequested ? target : Math.min(10, target + 1);
    const effectiveTypes = ((_d = overrides.types) === null || _d === void 0 ? void 0 : _d.length)
        ? [...new Set(overrides.types)]
        : input.enabledTypes;
    ...
    resolvedConversationId = (_e = firstResponse.conversationId) !== null && _e !== void 0 ? _e : resolvedConversationId;
    ...
    if (intent.exactCountRequested && suggestions.length < target) {
        ...
        resolvedConversationId = (_f = refillResponse.conversationId) !== null && _f !== void 0 ? _f : resolvedConversationId;
        ...
        const refillSuggestions = applyOcrTextRegionsToSuggestions(parseSuggestions(refillRawResponseText), refillInput)
            .filter((s) => allowRuntimeSuggestion(s, canUseVisionForIo, allowedIoImageRefs))
            .filter((s) => effectiveTypes.includes(s.type));
        suggestions = resolveSuggestionsWithFallback(intent, [...suggestions, ...refillSuggestions], input.noteContent).slice(0, target);
    }
...
export async function generateStudyAssistantSuggestionsStreaming(params) {
    ...
    const payloadPreview = `System prompt:\n${systemPrompt}\n\nUser prompt:\n${userPrompt}`;
    let resolvedConversationId = input.conversationId;
    let attachmentRoute = "none";
    let emittedSuggestionCount = 0;
    const filterSuggestions = (suggestions) => applyOcrTextRegionsToSuggestions(suggestions, input)
    ...
    const emitResolvedSuggestions = (baseSuggestions, streamedSuggestions, limit) => {
        const resolved = resolveSuggestionsWithFallback(intent, [...baseSuggestions, ...filterSuggestions(streamedSuggestions)], input.noteContent).slice(0, limit);
    ...
    const runStreamingPass = async (args) => {
        ...
        resolvedConversationId = (_a = response.conversationId) !== null && _a !== void 0 ? _a : resolvedConversationId;
        attachmentRoute = combineAttachmentRoutes(attachmentRoute, response.attachmentRoute);
        const resolvedSuggestions = resolveSuggestionsWithFallback(intent, [...args.baseSuggestions, ...filterSuggestions(parseSuggestions(response.text))], input.noteContent).slice(0, args.limit);
...
export async function generateStudyAssistantChatReply(params) {
    ...
    return {
        ...
        conversationId: (_a = response.conversationId) !== null && _a !== void 0 ? _a : input.conversationId,
    };
}
...
export function parseEditProposal(rawText) {
