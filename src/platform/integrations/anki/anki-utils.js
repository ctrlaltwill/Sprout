 ...
 */
import { ANKI_CARD_TYPE_NEW, ANKI_CARD_TYPE_LEARNING, ANKI_CARD_TYPE_REVIEW, ANKI_CARD_TYPE_RELEARNING, ANKI_QUEUE_SUSPENDED, ANKI_QUEUE_NEW, ANKI_QUEUE_LEARNING, ANKI_QUEUE_REVIEW, ANKI_EASE_AGAIN, ANKI_EASE_HARD, ANKI_EASE_GOOD, ANKI_EASE_EASY, } from "./anki-constants";
// ── Deck ↔ Group path conversion ──────────────────────────────────────────────
/** Convert Anki deck name to Sprout group path: `"A::B::C"` → `"A/B/C"`. */
export function ankiDeckToGroupPath(deck) {
    return String(deck !== null && deck !== void 0 ? deck : "")
        .split("::")
        .map((s) => s.trim())
        .filter(Boolean)
        .join("/");
...
export function groupPathToAnkiDeck(group) {
    return String(group !== null && group !== void 0 ? group : "")
        .split("/")
        .map((s) => s.trim())
        .filter(Boolean)
        .join("::");
 ...
 */
export function deckToFolderAndFile(deck, root) {
    const r = root.replace(/\/+$/, "");
    const parts = String(deck !== null && deck !== void 0 ? deck : "")
        .split("::")
        .map((s) => s.trim())
        .filter(Boolean);
    if (parts.length === 0) {
...
export function generateAnkiId() {
    const now = Date.now();
...
export function sproutRatingToAnkiEase(rating) {
    switch (rating) {
        case "again":
        case "fail":
...
export function ankiEaseToSproutRating(ease) {
    switch (ease) {
...
export function sproutStageToAnkiTypeQueue(stage) {
    switch (stage) {
        case "new":
...
export function ankiTypeQueueToSproutStage(type, queue) {
 ...
 */
export function fsrsDifficultyToAnkiFactor(difficulty) {
    if (difficulty === undefined || !Number.isFinite(difficulty))
        return 0;
 ...
 */
export function ankiFactorToFsrsDifficulty(factor) {
    if (!factor || factor <= 0)
        return 5.0;
...
export function sproutDueToAnkiReviewDue(dueMs, collectionCrtSec) {
 ...
 */
export function computeFieldChecksum(field) {
    let hash = 0x811c9dc5;
...
export function sproutGroupsToAnkiTags(groups) {
...
export function ankiTagsToSproutGroups(tags) {
    return String(tags !== null && tags !== void 0 ? tags : "")
        .trim()
        .split(/\s+/)
        .filter(Boolean)
