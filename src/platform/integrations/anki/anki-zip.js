 ...
 */
export function packApkg(sqliteBytes, media = new Map()) {
    const files = {};
 ...
 */
export function unpackApkg(apkgBytes) {
    const filesRaw = unzipSync(apkgBytes);
    ...
    const dbKey = Object.keys(files).find((k) => k.endsWith(".anki2") || k.endsWith(".anki21") || k === "collection.anki2");
    if (!dbKey) {
    ...
    if (mediaManifestBytes) {
        try {
            const manifest = JSON.parse(strFromU8(mediaManifestBytes));
