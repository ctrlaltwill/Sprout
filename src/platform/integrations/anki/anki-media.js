...
export function collectMediaRefs(fields) {
    const refs = new Set();
...
export async function resolveVaultMedia(app, refs, fromNotePaths) {
    var _a;
    const media = new Map();
    const readBinary = (_a = app.vault.readBinary) === null || _a === void 0 ? void 0 : _a.bind(app.vault);
    if (!readBinary)
        return media;
...
export function rewriteFieldForAnki(field, _mediaNames) {
    if (!field)
        return field;
    let text = field;
    ...
    text = text.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (_match, _alt, path) => {
...
export function rewriteFieldForSprout(html, mediaNameMap) {
    if (!html)
        return html;
    let text = html;
    // <img src="filename.png"> → ![[vault-path.png]]
    text = text.replace(new RegExp(ANKI_IMG_RE.source, "gi"), (_match, src) => {
        const vaultPath = mediaNameMap.get(src) || src;
        return `![[${vaultPath}]]`;
    });
    // Anki sound marker → ![[file.mp3]]
    text = text.replace(new RegExp(ANKI_SOUND_RE.source, "g"), (_match, src) => {
        const vaultPath = mediaNameMap.get(src) || src;
        return `![[${vaultPath}]]`;
    });
    return text;
...
export async function saveMediaToVault(app, media, targetFolder) {
    var _a;
    const nameMap = new Map();
    if (media.size === 0)
        return nameMap;
    const createBinary = (_a = app.vault.createBinary) === null || _a === void 0 ? void 0 : _a.bind(app.vault);
    if (!createBinary)
        return nameMap;
    ...
    for (const [filename, bytes] of media) {
        ...
        try {
            if (!app.vault.getAbstractFileByPath(vaultPath)) {
                await createBinary(vaultPath, bytes.buffer);
            }
