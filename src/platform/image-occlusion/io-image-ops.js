...
export function clampTextBgOpacity(value) {
...
export function textBgCss(color, opacity) {
    const c = String(color || "").trim();
    if (!c || c === "transparent")
        return "transparent";
    const o = clampTextBgOpacity(opacity !== null && opacity !== void 0 ? opacity : 1);
...
export async function loadImageElement(imageData) {
...
// ── Rotate ──────────────────────────────────────────────────────────────────
export async function rotateImageData(imageData, direction, rects, textBoxes) {
    ...
    const newImageData = { mime: imageData.mime, data };
    const dstW = srcH;
    const dstH = srcW;
...
// ── Crop ────────────────────────────────────────────────────────────────────
export async function cropImageData(imageData, sx, sy, sw, sh, rects, textBoxes) {
    const img = await loadImageElement(imageData);
    if (!img)
        return null;
    const srcW = img.naturalWidth;
...
export async function burnTextBoxesIntoImageData(imageData, textBoxes) {
    var _a;
    ...
    for (const t of textBoxes) {
        ...
        if (w <= 2 || h <= 2 || !t.text.trim())
            continue;
        ctx.save();
        ctx.beginPath();
        ctx.rect(x, y, w, h);
        ctx.clip();
        const bg = textBgCss(t.bgColor, (_a = t.bgOpacity) !== null && _a !== void 0 ? _a : 1);
...
export async function drawTextOnImageData(imageData, text, stageX, stageY, fontSize, textColor) {
    const img = await loadImageElement(imageData);
    if (!img)
        return null;
    const canvas = document.createElement("canvas");
