...
function isMobileLikePlatform() {
    var _a;
    if (Platform.isMobileApp || Platform.isIosApp || Platform.isAndroidApp)
        return true;
    if (typeof document !== "undefined" && ((_a = document.body) === null || _a === void 0 ? void 0 : _a.classList.contains("is-mobile")))
        return true;
    return false;
}
function getPlatformShortcut(key) {
    if (isMobileLikePlatform())
        return null;
    return Platform.isMacOS ? `⌘${key}` : `Ctrl+${key}`;
}
/** Build the IO-editor toolbar and return element references. */
export function buildToolbar(parent, cb) {
    ...
    const createIconBtn = (iconParent, iconName, tooltip, onClick, opts = {}) => {
    ...
    const btnRectTool = createIconBtn(toolbarGroup, "square", "Add Mask", () => cb.onSetTool("occlusion-rect"), { label: "Add Mask" });
    const btnAutoMask = createIconBtn(toolbarGroup, "wand-sparkles", findShortcut ? `Auto-Mask (${findShortcut})` : "Auto-Mask", () => cb.onAutoMask(), { label: "Auto-Mask" });
    const btnResetMasks = createIconBtn(toolbarGroup, "eraser", "Reset Masks", () => cb.onResetMasks(), { label: "Reset Masks", disabled: true });
...
export function buildCanvasContainer(parent, defaults, cb) {
...
export function buildFooter(parent, cb, defaultMode = "solo") {
    var _a, _b;
    ...
    const triggerLabel = trigger.createEl("span", { cls: "", text: (_b = (_a = options.find((o) => o.value === selectedMode)) === null || _a === void 0 ? void 0 : _a.label) !== null && _b !== void 0 ? _b : "Hide group" });
    ...
    const placeModeMenu = () => placePopover({
        trigger,
        panel: menuList,
        popoverEl: popover,
    ...
    const openMenu = () => {
        ...
        onDocPointerDown = (ev) => {
            ...
            if (!t)
                return;
            if (dropRoot.contains(t) || popover.contains(t))
                return;
            closeMenu();
        };
        window.setTimeout(() => {
...
// ── Image-limit dialog ──────────────────────────────────────────────────────
export function buildImageLimitDialog(parent, onDelete) {
...
// ── Header ──────────────────────────────────────────────────────────────────
export function buildHeader(parent, title, onClose) {
