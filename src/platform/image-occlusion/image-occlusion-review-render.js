...
function installSmoothZoomInteractions(host, zoomLayer) {
    let scale = 1;
    let offsetX = 0;
    let offsetY = 0;
    ...
    const applyTransform = () => {
        ...
        const maxScaleByViewport = Math.max(1, Math.min(maxViewportWidth / baseWidth, maxViewportHeight / baseHeight));
    ...
    zoomLayer.addEventListener("click", (ev) => {
        const target = ev.target;
        if (target === null || target === void 0 ? void 0 : target.closest(".learnkit-zoom-close"))
            return;
        ev.preventDefault();
        ev.stopPropagation();
    ...
    host.addEventListener("pointermove", (ev) => {
        if (!isDragging || pointerId !== ev.pointerId)
            return;
...
export function isIoRevealableType(card) {
...
export function renderImageOcclusionReviewInto(args) {
    ...
    const widgetMode = ((_a = containerEl === null || containerEl === void 0 ? void 0 : containerEl.dataset) === null || _a === void 0 ? void 0 : _a.sproutIoWidget) === "1";
    ...
    const openZoomModal = () => {
        const modal = new (class extends Modal {
            onOpen() {
                ...
                (_a = queryFirst(this.modalEl, ".modal-header")) === null || _a === void 0 ? void 0 : _a.remove();
                (_b = queryFirst(this.modalEl, ".modal-close-button")) === null || _b === void 0 ? void 0 : _b.remove();
                ...
                const closeIfOutsideSurface = (ev) => {
                    const target = ev.target;
                    if (!target)
                        return;
                    if (target.closest(".learnkit-zoom-surface"))
                        return;
                    if (target.closest(".learnkit-zoom-close"))
                        return;
                    this.close();
                };
                ...
                closeBtn.setAttribute("aria-label", t((_d = (_c = plugin.settings) === null || _c === void 0 ? void 0 : _c.general) === null || _d === void 0 ? void 0 : _d.interfaceLanguage, "ui.common.close", "Close"));
                closeBtn.setAttribute("data-learnkit-expand-collapse", "true");
                closeBtn.classList.add("learnkit-btn-toolbar", "learnkit-btn-toolbar", "learnkit-btn-filter", "learnkit-btn-filter", "h-7", "px-3", "text-sm", "inline-flex", "items-center", "gap-2", "learnkit-zoom-close", "learnkit-zoom-close");
    ...
    const revealMode = ((_c = (_b = plugin.settings) === null || _b === void 0 ? void 0 : _b.imageOcclusion) === null || _c === void 0 ? void 0 : _c.revealMode) === "all" ? "all" : "group";
    let targetIds = null;
    let targetGroup = null;
    if (card.type === "io-child") {
        const rectIds = Array.isArray(card.rectIds) ? card.rectIds.map((v) => String(v)) : [];
        if (rectIds.length > 0)
            targetIds = new Set(rectIds);
        else if (card.groupKey)
            targetGroup = String(card.groupKey || "");
    }
    const renderMasks = showAllMasks && card.type === "io-child" ? occlusions : masksToShow;
    const isTargetRect = (rect) => {
        const rectId = String(rect.rectId || "");
        const rectGroup = String(rect.groupKey || "");
        if (card.type !== "io-child")
            return true;
        if (!targetIds && !targetGroup)
            return true;
        if (targetIds)
            return targetIds.has(rectId);
        return rectGroup === targetGroup;
    };
    const revealGroupOnly = reveal &&
        card.type === "io-child" &&
        maskMode === "all" &&
        revealMode === "group";
    ...
    if (masksForOverlay.length > 0) {
        ...
        const cleanupOverlayListeners = () => {
            ...
            if (resizeRafId)
                cancelAnimationFrame(resizeRafId);
            resizeRafId = 0;
            overlayResizeObserver === null || overlayResizeObserver === void 0 ? void 0 : overlayResizeObserver.disconnect();
            overlayResizeObserver = null;
            detachedObserver === null || detachedObserver === void 0 ? void 0 : detachedObserver.disconnect();
