...
import type { IOParentDef, IORect, IOMaskMode } from "./ImageOcclusionTypes";
import {
  isMaskMode,
  makeRectId,
  nextAutoGroupKey,
...
function stripEmbedSyntax(raw: string): string {
...
function resolveImageFile(app: App, sourceNotePath: string, imageRef: string): TFile | null {
...
export class ImageOcclusionEditor extends Modal {
  ...
  static openForParent(plugin: SproutPlugin, parentId: string, opts?: IoEditorOpenOpts) {
    const m = new ImageOcclusionEditor(plugin.app, plugin, parentId, opts);
    m.open();
    return m;
  }
