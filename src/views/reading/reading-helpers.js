...
export const FIELD_START_RE = { test: (s) => FIELD_START_READING_RE().test(s), exec: (s) => FIELD_START_READING_RE().exec(s) };
/** Match a string against the field-start regex. Works as a drop-in for `str.match(FIELD_START_RE)`. */
function matchFieldStart(s) {
    return s.match(FIELD_START_READING_RE());
}
function isKnownReadingFieldKey(key) {
...
function matchKnownFieldStart(s) {
...
export function clean(s) {
    return (s !== null && s !== void 0 ? s : "").replace(INVIS_RE, "");
}
/**
 ...
 */
export function unescapePipeText(s) {
    return unescapeDelimiterText(s);
}
export function normalizeMathSignature(s) {
    let out = (s !== null && s !== void 0 ? s : "").toLowerCase();
...
export function processMarkdownFeatures(text) {
    ...
    result = result.replace(/!\[\[([^\]|]+?)(?:\|([^\]]*?))?\]\]/g, (_match, target, alt) => {
...
export function splitAtPipeTerminator(line) {
 ...
 */
export function extractLaTeXFromMathJax(mathEl) {
 ...
 */
export function extractRawTextFromParagraph(el) {
 ...
 */
export function extractTextWithLaTeX(el) {
 ...
 */
export function extractCardFromSource(sourceContent, anchorId) {
    ...
    const cardLines = [lines[anchorLineIndex]];
    ...
    for (let i = anchorLineIndex + 1; i < lines.length; i++) {
        ...
        if (matchKnownFieldStart(trimmed)) {
            ...
            const initialContent = (fm === null || fm === void 0 ? void 0 : fm[2]) || '';
...
export function parseSproutCard(text) {
    ...
    if (fields.G) {
        ...
        const groups = allGroups.split(',').map((s) => s.trim()).filter((s) => s.length > 0);
        fields.G = groups;
    }
...
export function saveField(fields, fieldName, content) {
    if (!fields[fieldName])
        fields[fieldName] = [];
    ...
    if (joined.length > 0 && Array.isArray(arr))
        arr.push(joined);
}
/* -----------------------
...
export function buildCardContentHTML(card) {
    let contentHTML = '';
...
export function buildMCQSectionHTML(question, options, answers) {
    ...
    function shuffleAndInsertAnswers(opts, ans) {
        ...
        arr.forEach((opt, idx) => {
            if (ansSet.has(opt.trim()))
                answerIdxs.add(idx);
        });
        return { options: arr, answerIdxs };
    }
    const shuffled = ansArr.length > 0 ? shuffleAndInsertAnswers(options, ansArr) : { options, answerIdxs: new Set() };
...
export function buildOQSectionHTML(question, steps) {
    ...
    const answerSection = buildCollapsibleSectionHTML('Sequence', `.${aId}`, `<div class="learnkit-oq-answer-list">${stepsHTML}</div>`, undefined, 'learnkit-section-answer');
    return `
    <div class="learnkit-card-section learnkit-section-question">
      <div class="learnkit-section-label">Question</div>
      <div class="learnkit-section-content learnkit-text-muted learnkit-p-spacing-none">${processMarkdownFeatures(question)}</div>
...
export function buildBasicSectionHTML(question, answer) {
...
export function buildIOSectionHTML(_ioContent) {
    ...
    const answerSection = buildCollapsibleSectionHTML('Answer (full image)', `.learnkit-io-answer-${Math.random().toString(36).slice(2, 8)}`, `<div class="learnkit-io-answer-wrap learnkit-p-spacing-none" id="${ioAnswerId}"></div>`, ioAnswerId, 'learnkit-section-answer');
    return `
...
export function buildInfoSectionHTML(_infoContent) {
...
export function buildCollapsibleSectionHTML(label, targetSelector, innerHtml, _markdownId, sectionClass = '') {
 ...
 */
export function parseMarkdownToElements(text) {
    const elements = [];
 ...
 */
export function createMarkdownElement(data) {
 ...
 */
export function checkForNonCardContent(el) {
 ...
 */
export function containsNonCardContent(el) {
