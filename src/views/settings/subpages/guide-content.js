...
const tx = (token, fallback) => t("en", token, fallback);
function convertDocsLinksForObsidianGuide(markdown) {
    return String(markdown !== null && markdown !== void 0 ? markdown : "")
        .replace(/\[([^\]]+)\]\(\.\/([^)#]+?)\.md(#[^)]+)?\)/g, (_m, text, target) => {
        const label = String(text !== null && text !== void 0 ? text : "").trim();
        const page = String(target !== null && target !== void 0 ? target : "").trim();
        if (!page)
            return _m;
...
export async function loadGuidePages(app, pluginDir) {
    const pagesFromRepoRaw = [];
    ...
    if (pagesFromRepoRaw.length)
        return pagesFromRepoRaw;
    const pagesFromPluginDir = [];
    if (pluginDir) {
        for (const fileName of PREFERRED_GUIDE_FILES) {
    ...
    if (pagesFromPluginDir.length)
        return pagesFromPluginDir;
...
export function orderGuidePagesByNavigation(pages) {
    if (!pages.length)
        return [];
...
export function getGuidePageDisplayLabel(pageKey) {
    var _a;
    return (_a = GUIDE_LABEL_MAP[pageKey]) !== null && _a !== void 0 ? _a : pageKey.replace(/-/g, " ");
}
export function getGuideTooltipLabel(pageKey) {
    if (pageKey === "What-LearnKit-Is")
        return "What LearnKit Is";
    return getGuidePageDisplayLabel(pageKey);
}
export function getGuidePageIcon(pageKey) {
    var _a;
    return (_a = GUIDE_ICON_MAP[pageKey]) !== null && _a !== void 0 ? _a : "file-text";
}
