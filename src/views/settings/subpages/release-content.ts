...
export async function readSupportMarkdown(app: App, pluginDir?: string): Promise<string> {
  try {
    ...
    return await app.vault.adapter.read(`${pluginDir ?? `${configDir}/plugins/sprout`}/site/docs/Support-Sprout.md`);
  } catch {
    return `# About Sprout
...
You can support Sprout by starring the repo, sharing it with friends, or sponsoring development. Because Sprout is open source, the goal is simple: powerful learning tools without paywalls.
