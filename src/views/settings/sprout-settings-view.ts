...
import { initAOS, refreshAOS, cascadeAOSOnLoad } from "../../platform/core/aos-loader";
...
export class SproutSettingsView extends ItemView {
  ...
  render() {
    ...
    title.textContent = tx("ui.view.settings.title", "Settings");
    ...
    const topTabs = [
      { id: "guide", label: tx("ui.topTabs.guide", "Guide"), icon: "book-open" },
      {
        id: "about",
        label: isMobile
          ? tx("ui.topTabs.about.releases", "Releases")
          : tx("ui.topTabs.about.releaseNotes", "Release Notes"),
        icon: "sprout-brand",
      },
    ...
    for (const tab of topTabs) {
      ...
      const tooltipMap: Record<string, string> = {
        guide: tx("ui.topTabs.tooltip.guide", "Open guide"),
        about: tx("ui.topTabs.tooltip.about", "Open release notes"),
  ...
  private _renderActiveTabContent() {
    ...
    try {
      ...
      const settingsSubTabs: Array<{ id: string; label: string; paneTitle?: string; method: Exclude<RenderMethodName, "renderAboutTab" | "renderGuideTab"> }> = [
        ...
        { id: "study", label: tx("ui.settings.subTabs.study", "Study"), method: "renderStudyTab" },
      ];
      ...
      if (typeof renderMethod === "function") {
        ...
        msg.textContent = tx("ui.settings.error.unknownTab", "Unknown tab: {tab}", { tab });
  ...
  private _enhanceAboutPage(body: HTMLElement) {
    ...
    if (linkedinLink) {
      ...
      if (storyUl) {
        ...
        githubBtn.textContent = tx("ui.settings.about.link.github", "GitHub →");
        ...
        linkedinBtn.textContent = tx("ui.settings.about.link.linkedin", "LinkedIn →");
