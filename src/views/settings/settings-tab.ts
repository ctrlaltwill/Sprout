...
} from "./config/reading-presets";
import { BACKUP_ROWS_PER_PAGE_OPTIONS } from "./config/backup-constants";
import { createSettingsNoticeLines, type SettingsNoticeLines } from "./config/notice-lines";
import type {
...
export class SproutSettingsTab extends PluginSettingTab {
  ...
  private renderBackupsSection(wrapper: HTMLElement) {
...

    {
      ...
      const summaryLabel = (stats: BackupSchedulingStats) => {
      ...
      const scan = async () => {
        ...
        try {
          ...
          const rows: Array<{ stats: DataJsonBackupStats; ok: boolean; integrity: BackupIntegrityState }> = [];
          for (const e of entries) {
            ...
            const integrityState: BackupIntegrityState = !integrity.ok
              ? "invalid"
              : integrity.verified
                ? "verified"
                : "legacy";
  ...
  private renderAppearanceSection(wrapper: HTMLElement): void {
    ...
    const resolveThemeAccentHex = (): string => {
      const toHexFromRgb = (input: string): string | null => {
      ...
      const normaliseCssColorToHex = (input: string): string | null => {
    ...
    new Setting(wrapper)
      ...
      .addColorPicker((picker) => {
        ...
        setAccentPickerValue = (hex: string) => {
          picker.setValue(hex);
        };
  ...
  private renderReadingViewSection(wrapper: HTMLElement): void {
    ...
    this._addSimpleSelect(readingThemeSetting.controlEl, {
      ...
      onChange: (value) => {
        void applyPreset(value as "flashcards" | "markdown");
      },
    });
    ...
    if (rv.activeMacro === "markdown") {
      const addMarkdownFieldToggle = (
        name: string,
        desc: string,
        key: keyof typeof activeCfg.fields,
      ) => {
        new Setting(wrapper)
          .setName(name)
          .setDesc(desc)
  ...
  private renderStudySection(wrapper: HTMLElement): void {
    ...
    new Setting(wrapper)
      .setName(this._tx("ui.settings.sections.analyticsCoaching", "Analytics coaching"))
      .setHeading();
    ...
    new Setting(wrapper)
      ...
      .addToggle((t) => {
        ...
        t.setValue(current);
        t.onChange(async (v) => {
          this.plugin.settings.study.analyticsIncludePracticeNoteReview = v;
          await this.plugin.saveAll();
        });
      });
    ...
    new Setting(wrapper)
      ...
      .then((s) => {
        this._addSimpleSelect(s.controlEl, {
          options: [
            { value: "folder-group-topic", label: "Folder > Group > Topic" },
            { value: "group-topic", label: "Group > Topic" },
            { value: "topic", label: "Topic only" },
          ],
          value: this.plugin.settings.study.analyticsTopicHierarchy ?? "folder-group-topic",
          onChange: (v) => {
            void (async () => {
              this.plugin.settings.study.analyticsTopicHierarchy = v as "folder-group-topic" | "group-topic" | "topic";
              await this.plugin.saveAll();
            })();
          },
        });
    ...
    new Setting(wrapper)
      .setName(this._tx("ui.settings.study.analytics.threshold.red.name", "Mastery red threshold"))
      .setDesc(this._tx("ui.settings.study.analytics.threshold.red.desc", "Scores at or below this value are marked weak (red)."))
      .addText((t) => {
        t.setValue(String(this.plugin.settings.study.analyticsMasteryThresholds?.redMax ?? 59));
        t.onChange(async (v) => {
          const next = clamp(toNonNegInt(v, 59), 0, 98);
          const current = this.plugin.settings.study.analyticsMasteryThresholds ?? { redMax: 59, yellowMax: 79 };
          current.redMax = next;
          if (current.yellowMax <= next) current.yellowMax = Math.min(99, next + 1);
          this.plugin.settings.study.analyticsMasteryThresholds = current;
          await this.plugin.saveAll();
        });
      });

    new Setting(wrapper)
      .setName(this._tx("ui.settings.study.analytics.threshold.yellow.name", "Mastery yellow threshold"))
      .setDesc(this._tx("ui.settings.study.analytics.threshold.yellow.desc", "Scores above red and at or below this value are marked medium (yellow)."))
      .addText((t) => {
        t.setValue(String(this.plugin.settings.study.analyticsMasteryThresholds?.yellowMax ?? 79));
        t.onChange(async (v) => {
          const red = Number(this.plugin.settings.study.analyticsMasteryThresholds?.redMax ?? 59);
          const next = clamp(toNonNegInt(v, 79), Math.max(1, red + 1), 100);
          const current = this.plugin.settings.study.analyticsMasteryThresholds ?? { redMax: 59, yellowMax: 79 };
          current.yellowMax = next;
          this.plugin.settings.study.analyticsMasteryThresholds = current;
          await this.plugin.saveAll();
        });
      });
    ...
    new Setting(wrapper)
      ...
      .addText((t) => {
        ...
        t.onChange(async (v) => {
          this.plugin.settings.study.analyticsFocusTopicCount = clamp(toNonNegInt(v, 7), 1, 20);
          await this.plugin.saveAll();
        });
      });
    ...
    let gatekeeperBehaviourHeading: Setting | null = null;
    ...
    let gatekeeperBypassHeading: Setting | null = null;
    ...
    const refreshRemindersVisibility = (): void => {
      ...
      startupDelaySetting?.setDisabled(!launchEnabled);
      repeatIntervalSetting?.setDisabled(!routineEnabled);
      gatekeeperFrequencySetting?.setDisabled(!gatekeeperEnabled);
      gatekeeperDueQuestionsSetting?.setDisabled(!gatekeeperEnabled);
      gatekeeperScopeSetting?.setDisabled(!gatekeeperEnabled);
      gatekeeperPauseSetting?.setDisabled(!gatekeeperEnabled);
      gatekeeperBypassSetting?.setDisabled(!gatekeeperEnabled);
      gatekeeperBypassWarningSetting?.setDisabled(!gatekeeperEnabled || !gatekeeperAllowSkip);
    };
  ...
  private renderNoteReviewSection(wrapper: HTMLElement): void {
    this.plugin.settings.noteReview ??= {
      algorithm: "fsrs",
    ...
    new Setting(wrapper)
      ...
      .setDesc(
        this._tx(
          "ui.settings.noteReview.avoidFolderNotes.desc",
    ...
    const renderAlgorithmFields = () => {
      ...
      new Setting(dynamicHost)
        ...
        .addText((t) =>
          t.setValue(String(noteCfg.reviewsPerDay ?? 10)).onChange(async (v) => {
      ...
      new Setting(dynamicHost)
        ...
        .addText((t) =>
          t.setValue((noteCfg.reviewStepsDays ?? [1, 7, 30, 365]).join(",")).onChange(async (v) => {
            const vals = parsePositiveNumberListCsv(v);
      ...
      new Setting(dynamicHost)
        ...
        .addToggle((t) =>
          t.setValue(noteCfg.fillFromFutureWhenUnderLimit !== false).onChange(async (v) => {
      ...
      .addToggle((toggle) =>
        toggle.setValue(!!this.plugin.settings.studyAssistant.enabled).onChange(async (value) => {
          ...
          this.queueSettingsNotice(
            ...
            this._tx("ui.settings.studyAssistant.notice.enabled", "Companion: {state}", {
    ...
    for (const type of cardTypes) {
      ...
      const typeDesc = type.key === "io"
        ? this._tx(
          ...
          "Turn off to skip this card type. Image occlusion needs a vision-capable model, and generated masks may still need manual adjustment in the flashcard editor.",
        )
  ...
  private renderStorageSection(wrapper: HTMLElement): void {
    ...
    this.plugin.settings.storage.vaultSync ??= clonePlain(DEFAULT_SETTINGS.storage.vaultSync);
    ...
    const vaultSyncFolderSetting = new Setting(wrapper)
      ...
      .setDesc(
        this._tx(
          "ui.settings.storage.vaultSync.folder.desc",
          "Vault folder where .db copies are stored for syncing.",
        ),
      )
      .addText((t) => {
        const allFolders = listVaultFolders(this.app);
        ...
        const commitVaultSyncFolder = async (rawValue: string, fromPick: boolean) => {
          const prev = String(vaultSync.folderPath ?? "");
