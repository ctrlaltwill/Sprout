 ...
 */
function localDayIndex(ts, timeZone) {
...
export class SproutAnalyticsView extends ItemView {
    constructor(leaf, plugin) {
        super(leaf);
        this._activeSection = "flashcards";
        this._header = null;
        this._rootEl = null;
        ...
        this._examAttemptsCache = [];
        this._examAttemptsHydrated = false;
    ...
    getDisplayText() {
        var _a, _b;
        return t((_b = (_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.general) === null || _b === void 0 ? void 0 : _b.interfaceLanguage, "ui.view.analytics.title", "Analytics");
    }
    getIcon() {
        return "chart-spline";
    }
    ...
    async onClose() {
        ...
        try {
            ...
            log.swallow("dispose header", e);
        }
        this._header = null;
        try {
            (_c = this._heatmapRoot) === null || _c === void 0 ? void 0 : _c.unmount();
        }
        catch (e) {
            log.swallow("unmount heatmap root", e);
        }
        this._heatmapRoot = null;
        try {
            (_d = this._stagePieRoot) === null || _d === void 0 ? void 0 : _d.unmount();
        }
        catch (e) {
            log.swallow("unmount stage pie root", e);
        }
        this._stagePieRoot = null;
        try {
            (_e = this._answerPieRoot) === null || _e === void 0 ? void 0 : _e.unmount();
        }
        catch (e) {
            log.swallow("unmount answer pie root", e);
        }
        this._answerPieRoot = null;
        try {
            (_f = this._futureDueRoot) === null || _f === void 0 ? void 0 : _f.unmount();
        }
        catch (e) {
            log.swallow("unmount future due root", e);
        }
        this._futureDueRoot = null;
        try {
            (_g = this._newCardsRoot) === null || _g === void 0 ? void 0 : _g.unmount();
        }
        catch (e) {
            log.swallow("unmount new cards root", e);
        }
        this._newCardsRoot = null;
        try {
            (_h = this._stackedButtonsRoot) === null || _h === void 0 ? void 0 : _h.unmount();
        }
        catch (e) {
            log.swallow("unmount stacked buttons root", e);
        }
        this._stackedButtonsRoot = null;
        try {
            (_j = this._stabilityDistributionRoot) === null || _j === void 0 ? void 0 : _j.unmount();
        }
        catch (e) {
            log.swallow("unmount stability distribution root", e);
        }
        this._stabilityDistributionRoot = null;
        try {
            (_k = this._forgettingCurveRoot) === null || _k === void 0 ? void 0 : _k.unmount();
        }
        catch (e) {
            log.swallow("unmount forgetting curve root", e);
        }
        this._forgettingCurveRoot = null;
        try {
            (_l = this._testsAnalyticsRoot) === null || _l === void 0 ? void 0 : _l.unmount();
        }
        catch (e) {
            log.swallow("unmount tests analytics root", e);
        }
        this._testsAnalyticsRoot = null;
        try {
            (_m = this._noteReviewAnalyticsRoot) === null || _m === void 0 ? void 0 : _m.unmount();
        }
        catch (e) {
            log.swallow("unmount note-review analytics root", e);
        }
        this._noteReviewAnalyticsRoot = null;
        this._hasPlayedEntryAnimation = false;
        resetAOS();
    ...
    render() {
        ...
        this._rootEl = root;
        root.classList.add("learnkit-view-content", "learnkit-view-content", "learnkit-analytics-root", "learnkit-analytics-root", "w-full");
        ...
        (_a = this.setTitle) === null || _a === void 0 ? void 0 : _a.call(this, tx("ui.view.analytics.title", "Analytics"));
        if (!this._header) {
        ...
        const rowStep = 50;
        const applyAos = (_el, _delay, _animation = "fade-up") => {
        ...
        const applyRootAos = (el, delay, animation = "fade-up") => {
        ...
        subtitle.textContent = tx("ui.analytics.header.subtitle", "See trends, forecast due cards, and tune your study plan.");
        const filtersWrap = document.createElement("div");
        filtersWrap.className = "flex flex-wrap items-center gap-2 ml-auto max-md:ml-0 learnkit-analytics-filter-buttons";
        const mkFilterBtn = (section, label, iconName) => {
        ...
        const now = Date.now();
        const events = (_g = (_f = (_e = this.plugin.store).getAnalyticsEvents) === null || _f === void 0 ? void 0 : _f.call(_e)) !== null && _g !== void 0 ? _g : [];
        const allCards = (_k = (_j = (_h = this.plugin.store).getAllCards) === null || _j === void 0 ? void 0 : _j.call(_h)) !== null && _k !== void 0 ? _k : [];
        const cards = allCards.filter(c => !isParentCard(c));
        const parentIds = new Set(allCards.filter(c => isParentCard(c)).map(c => String(c.id)));
        const states = (_l = this.plugin.store.data.states) !== null && _l !== void 0 ? _l : {};
        const reviewLog = (_m = this.plugin.store.data.reviewLog) !== null && _m !== void 0 ? _m : [];
        ...
        for (const ev of events) {
            ...
            const entry = (_o = dayMap.get(idx)) !== null && _o !== void 0 ? _o : { count: 0, totalMs: 0 };
            entry.count += 1;
        ...
        for (const [stId, st] of Object.entries(states || {})) {
            ...
            dueByDay.set(idx, ((_p = dueByDay.get(idx)) !== null && _p !== void 0 ? _p : 0) + 1);
        }
        ...
        while ((_q = dayMap.get(cursor)) === null || _q === void 0 ? void 0 : _q.count) {
            currentStreak += 1;
            cursor -= 1;
        }
        let longestStreak = 0;
        const sortedDays = Array.from(dayMap.keys())
            .filter((k) => { var _a, _b; return ((_b = (_a = dayMap.get(k)) === null || _a === void 0 ? void 0 : _a.count) !== null && _b !== void 0 ? _b : 0) > 0; })
            .sort((a, b) => a - b);
        let run = 0;
        let prev = Number.NaN;
        for (const d of sortedDays) {
            if (!Number.isFinite(prev) || d === prev + 1)
                run += 1;
            else
                run = 1;
        ...
        if (showFlashcards) {
            this._renderKpiCards(body, applyAos, kpiBaseDelay, kpiStep, currentStreak, longestStreak, avgCardsPerDay, avgTimePerDayMinutes, prevAvgCardsPerDay, prevAvgTimePerDayMinutes, prevActiveDaysReviews, prevActiveDaysTime, streakMatches, streakNote, todayIndex, dayMap, tx);
        }
        if (showTests) {
        ...
        this._renderCharts(body, applyAos, animationsEnabled, heatmapDelay, heroRowDelay, graphsGridDelay, stabilityRowDelay, events, cards, states, reviewLog, timezone, todayIndex, now, dayMap, dueByDay, parentIds, showFlashcards, showTests, showNotes, tx);
        if (showFlashcards) {
            this._renderStatsTable(body, applyAos, animationsEnabled, statsCardDelay, rowBaseDelay, rowStep, events, reviewLog, timezone, todayIndex, dayMap, dueByDay, parentIds, tx);
        }
        if (shouldAnimateEntry) {
            titleStrip.classList.remove("aos-animate");
            contentShell.classList.remove("aos-animate");
            window.requestAnimationFrame(() => {
                if (!titleStrip.isConnected || !contentShell.isConnected)
                    return;
    ...
    _renderKpiCards(body, applyAos, kpiBaseDelay, kpiStep, currentStreak, longestStreak, avgCardsPerDay, avgTimePerDayMinutes, prevAvgCardsPerDay, prevAvgTimePerDayMinutes, prevActiveDaysReviews, prevActiveDaysTime, streakMatches, streakNote, todayIndex, dayMap, tx) {
        var _a, _b;
        const kpiRow = document.createElement("div");
        kpiRow.className = "learnkit-ana-grid grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4";
        body.appendChild(kpiRow);
        const makeBadge = (opts) => {
            ...
            if (opts.bg)
                setCssProps(badge, "--learnkit-badge-bg", opts.bg);
            else if (!opts.className)
                setCssProps(badge, "--learnkit-badge-bg", "var(--theme-accent)");
            if (opts.color)
                setCssProps(badge, "--learnkit-badge-color", opts.color);
            if (opts.border)
                setCssProps(badge, "--learnkit-badge-border", opts.border);
            if (opts.live) {
        ...
        const formatTrend = (current, previous, previousDaysWithData) => {
        ...
        const makeCard = (title, value, badge, note) => {
            ...
            if (cardIndex > 3)
                cardIndex = 3; // Only first 4 get delays
        ...
        makeCard(tx("ui.analytics.card.longestStreak", "Longest streak"), tx("ui.analytics.card.daysSuffix", "{count} day{suffix}", { count: longestStreak, suffix: longestStreak === 1 ? "" : "s" }), longestBadge, streakNote);
        const includesToday = ((_b = (_a = dayMap.get(todayIndex)) === null || _a === void 0 ? void 0 : _a.count) !== null && _b !== void 0 ? _b : 0) > 0;
        const currentBadge = includesToday
            ...
            : null;
        const currentNote = currentStreak > 0
            ? (streakMatches
        ...
        makeCard(tx("ui.analytics.card.currentStreak", "Current streak"), tx("ui.analytics.card.daysSuffix", "{count} day{suffix}", { count: currentStreak, suffix: currentStreak === 1 ? "" : "s" }), currentBadge, currentNote);
        ...
        makeCard(tx("ui.analytics.card.dailyTime", "Daily time"), dailyTimeValue, timeBadge, tx("ui.analytics.note.last7Days", "Last 7 days"));
        const cardsBadge = buildTrendBadge(formatTrend(avgCardsPerDay, prevAvgCardsPerDay, prevActiveDaysReviews));
        makeCard(tx("ui.analytics.card.dailyCards", "Daily cards"), `${Math.round(avgCardsPerDay)}`, cardsBadge, tx("ui.analytics.note.last7Days", "Last 7 days"));
    }
    _renderTestKpiCards(body, events, timezone, todayIndex, tx) {
        const kpiRow = document.createElement("div");
        ...
        const formatTrend = (cur, prev, hasData) => {
            if (!hasData || prev <= 0)
                return { value: 0, text: "0%", dir: 0 };
        ...
        makeCard(tx("ui.analytics.card.testAttempts", "Attempts"), `${attempts7d}`, buildTrendBadge(formatTrend(attempts7d, prevAttempts7d, prevAttempts7d > 0)), tx("ui.analytics.note.last7Days", "Last 7 days"));
        makeCard(tx("ui.analytics.card.testAvgScore", "Avg score"), attempts7d > 0 ? `${avgScore7d.toFixed(1)}%` : "\u2014", buildTrendBadge(formatTrend(avgScore7d, prevAvgScore7d, prevAttempts7d > 0)), tx("ui.analytics.note.last7Days", "Last 7 days"));
        makeCard(tx("ui.analytics.card.testRecentScore", "Most recent score"), lastAt > 0 ? `${lastScore.toFixed(1)}%` : "\u2014", null, lastAt > 0 ? tx("ui.analytics.note.latestAttempt", "Latest attempt") : tx("ui.analytics.note.noAttemptsYet", "No attempts yet"));
    }
    async _renderNoteKpiCards(body, events, timezone, todayIndex, tx) {
        var _a, _b, _c, _d;
        const kpiRow = document.createElement("div");
        kpiRow.className = "learnkit-ana-grid grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4";
        body.appendChild(kpiRow);
        ...
        for (const ev of events) {
            ...
            const entry = (_a = noteDayMap.get(idx)) !== null && _a !== void 0 ? _a : { count: 0 };
            entry.count += 1;
        ...
        let currentStreak = 0;
        let cursor = todayIndex;
        while ((_b = noteDayMap.get(cursor)) === null || _b === void 0 ? void 0 : _b.count) {
            currentStreak += 1;
            cursor -= 1;
        }
        ...
        const sortedDays = Array.from(noteDayMap.keys())
            .filter((k) => { var _a, _b; return ((_b = (_a = noteDayMap.get(k)) === null || _a === void 0 ? void 0 : _a.count) !== null && _b !== void 0 ? _b : 0) > 0; })
            .sort((a, b) => a - b);
        let run = 0;
        let prev = Number.NaN;
        for (const d of sortedDays) {
        ...
        catch ( /* graceful fallback */_e) { /* graceful fallback */ }
        const formatTrend = (cur, prev, hasData) => {
            if (!hasData || prev <= 0)
                return { value: 0, text: "0%", dir: 0 };
            const raw = ((cur - prev) / prev) * 100;
        ...
        makeCard(tx("ui.analytics.card.noteLongestStreak", "Longest streak"), tx("ui.analytics.card.daysSuffix", "{count} day{suffix}", { count: longestStreak, suffix: longestStreak === 1 ? "" : "s" }), longestBadge, streakNote);
        // Card 2: Current streak
        const includesToday = ((_d = (_c = noteDayMap.get(todayIndex)) === null || _c === void 0 ? void 0 : _c.count) !== null && _d !== void 0 ? _d : 0) > 0;
        const currentBadge = includesToday
            ? makeBadge({ text: tx("ui.analytics.badge.live", "Live"), live: true, className: "learnkit-trend-badge learnkit-live-badge learnkit-live-badge-dark" })
            : null;
        const currentNote = currentStreak > 0
            ? (streakMatches
                ...
                : includesToday
                    ...
                    : tx("ui.analytics.note.streakActive", "Streak active."))
            : tx("ui.analytics.note.noReviews", "No reviews yet");
        makeCard(tx("ui.analytics.card.noteCurrentStreak", "Current streak"), tx("ui.analytics.card.daysSuffix", "{count} day{suffix}", { count: currentStreak, suffix: currentStreak === 1 ? "" : "s" }), currentBadge, currentNote);
        // Card 3: Notes reviewed
        makeCard(tx("ui.analytics.card.noteReviewed", "Notes reviewed"), `${reviewed7d}`, buildTrendBadge(formatTrend(reviewed7d, prevReviewed7d, prevReviewed7d > 0)), tx("ui.analytics.note.last7Days", "Last 7 days"));
        // Card 4: Scheduled next 7 days
        makeCard(tx("ui.analytics.card.noteScheduledNext7", "Scheduled next 7 days"), `${scheduledNext7d}`, null, tx("ui.analytics.note.upcoming", "Upcoming"));
    }
    _renderCharts(body, applyAos, animationsEnabled, heatmapDelay, heroRowDelay, graphsGridDelay, stabilityRowDelay, events, cards, states, reviewLog, timezone, todayIndex, now, dayMap, dueByDay, parentIds, showFlashcards, showTests, showNotes, tx) {
        const renderChartMountFallback = (host, chartName, error) => {
        ...
        const mountChartRoot = (host, chartName, renderFn) => {
            try {
        ...
        if (showFlashcards) {
            ...
            this._heatmapRoot = mountChartRoot(heatmapHost, tx("ui.analytics.chart.reviewHeatmap", "Review Calendar Heatmap"), (rootNode) => {
                rootNode.render(React.createElement(ChartErrorBoundary, { chartName: tx("ui.analytics.chart.reviewHeatmap", "Review Calendar Heatmap") }, React.createElement(ReviewCalendarHeatmap, {
                    revlog: reviewEvents,
                    timezone,
                    rangeDays: 365,
                    filters: {},
            ...
            this._stagePieRoot = mountChartRoot(stagePieHost, tx("ui.analytics.chart.cardStageDistribution", "Card Stage Distribution"), (rootNode) => {
                rootNode.render(React.createElement(ChartErrorBoundary, { chartName: tx("ui.analytics.chart.cardStageDistribution", "Card Stage Distribution") }, React.createElement(StagePieCard, { cards, states, enableAnimations: animationsEnabled })));
            });
            ...
            const futureCards = cards.map((c) => {
                var _a, _b;
                const stateData = states[String(c.id)] || {};
                const due = stateData.due;
                const stage = stateData.stage;
                return {
                    id: String(c.id),
                    due: Number(due !== null && due !== void 0 ? due : 0) || null,
                    suspended: String(stage !== null && stage !== void 0 ? stage : "") === "suspended",
                    stage: String(stage !== null && stage !== void 0 ? stage : "new"),
                    type: String((_a = c === null || c === void 0 ? void 0 : c.type) !== null && _a !== void 0 ? _a : ""),
                    sourceNotePath: String((_b = c === null || c === void 0 ? void 0 : c.sourceNotePath) !== null && _b !== void 0 ? _b : ""),
                    groups: Array.isArray(c === null || c === void 0 ? void 0 : c.groups) ? c.groups : [],
                };
            });
            this._futureDueRoot = mountChartRoot(futureDueHost, tx("ui.analytics.chart.studyForecast", "Study Forecast"), (rootNode) => {
                rootNode.render(React.createElement(ChartErrorBoundary, { chartName: tx("ui.analytics.chart.studyForecast", "Study Forecast") }, React.createElement(FutureDueChart, {
                    cards: futureCards,
                    nowMs: now,
            ...
            this._stackedButtonsRoot = mountChartRoot(stackedButtonsHost, tx("ui.analytics.chart.answerButtons", "Answer Buttons"), (rootNode) => {
                rootNode.render(React.createElement(ChartErrorBoundary, { chartName: tx("ui.analytics.chart.answerButtons", "Answer Buttons") }, React.createElement(StackedReviewButtonsChart, {
                    events,
                    cards,
                    timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
                    days: 30,
            ...
            this._newCardsRoot = mountChartRoot(newCardsHost, tx("ui.analytics.chart.newCardsPerDay", "New Cards Per Day"), (rootNode) => {
                rootNode.render(React.createElement(ChartErrorBoundary, { chartName: tx("ui.analytics.chart.newCardsPerDay", "New Cards Per Day") }, React.createElement(NewCardsPerDayChart, {
                    cards,
                    timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
                    days: 30,
                    enableAnimations: animationsEnabled,
            ...
            this._stabilityDistributionRoot = mountChartRoot(stabilityDistributionHost, tx("ui.analytics.chart.stabilityDistribution", "Stability Distribution"), (rootNode) => {
                rootNode.render(React.createElement(ChartErrorBoundary, { chartName: tx("ui.analytics.chart.stabilityDistribution", "Stability Distribution") }, React.createElement(StabilityDistributionChart, {
                    cards: cards.map(c => { var _a; return ({ ...c, groups: (_a = c.groups) !== null && _a !== void 0 ? _a : undefined }); }),
                    states,
                    enableAnimations: animationsEnabled,
                })));
            });
            ...
            this._forgettingCurveRoot = mountChartRoot(forgettingCurveHost, tx("ui.analytics.chart.forgettingCurve", "Forgetting Curve"), (rootNode) => {
                var _a;
                rootNode.render(React.createElement(ChartErrorBoundary, { chartName: tx("ui.analytics.chart.forgettingCurve", "Forgetting Curve") }, React.createElement(ForgettingCurveChart, {
                    cards,
                    states,
                    reviewLog,
                    scheduler: (_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.scheduling,
                    nowMs: now,
                    enableAnimations: animationsEnabled,
                })));
            });
        ...
        if (showTests) {
            ...
            this._testsAnalyticsRoot = mountChartRoot(testsHost, tx("ui.analytics.chart.testsPerformance", "Tests Performance"), (rootNode) => {
                rootNode.render(React.createElement(ChartErrorBoundary, { chartName: tx("ui.analytics.chart.testsPerformance", "Tests Performance") }, React.createElement(TestsAnalyticsCard, {
                    events: events.filter((ev) => ev && ev.kind === "exam-attempt"),
        ...
        if (showNotes) {
            ...
            this._noteReviewAnalyticsRoot = mountChartRoot(noteReviewHost, tx("ui.analytics.chart.noteReviewActivity", "Note Review Activity"), (rootNode) => {
                var _a, _b;
                rootNode.render(React.createElement(ChartErrorBoundary, { chartName: tx("ui.analytics.chart.noteReviewActivity", "Note Review Activity") }, React.createElement(NoteReviewAnalyticsCard, {
                    events: events.filter((ev) => ev && ev.kind === "note-review"),
                    includePracticeDefault: ((_b = (_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.study) === null || _b === void 0 ? void 0 : _b.analyticsIncludePracticeNoteReview) !== false,
                    timezone,
                })));
            });
        }
    }
    _renderStatsTable(body, applyAos, animationsEnabled, statsCardDelay, rowBaseDelay, rowStep, events, reviewLog, timezone, todayIndex, dayMap, dueByDay, parentIds, tx) {
        ...
        for (const ev of events) {
            ...
            if (ev.kind !== "review")
                continue;
            const idx = localDayIndex(at, timezone);
            const row = (_a = ratingsByDay.get(idx)) !== null && _a !== void 0 ? _a : { again: 0, hard: 0, good: 0, easy: 0 };
        ...
        for (let i = 0; i < maxDays; i += 1) {
            ...
            const due = (_b = dueByDay.get(idx)) !== null && _b !== void 0 ? _b : 0;
            const reviews = (_c = counts === null || counts === void 0 ? void 0 : counts.count) !== null && _c !== void 0 ? _c : 0;
            const minutes = ((_d = counts === null || counts === void 0 ? void 0 : counts.totalMs) !== null && _d !== void 0 ? _d : 0) / 60000;
            const secondsPerCard = reviews > 0 ? ((_e = counts === null || counts === void 0 ? void 0 : counts.totalMs) !== null && _e !== void 0 ? _e : 0) / reviews / 1000 : 0;
            const passed = ((_f = ratings === null || ratings === void 0 ? void 0 : ratings.hard) !== null && _f !== void 0 ? _f : 0) + ((_g = ratings === null || ratings === void 0 ? void 0 : ratings.good) !== null && _g !== void 0 ? _g : 0) + ((_h = ratings === null || ratings === void 0 ? void 0 : ratings.easy) !== null && _h !== void 0 ? _h : 0);
            const failed = (_j = ratings === null || ratings === void 0 ? void 0 : ratings.again) !== null && _j !== void 0 ? _j : 0;
            const passedPct = reviews > 0 ? (passed / reviews) * 100 : 0;
            const failedPct = reviews > 0 ? (failed / reviews) * 100 : 0;
            if (due <= 0 && reviews <= 0 && passed <= 0 && failed <= 0)
                continue;
        ...
        const formatTimePerCard = (secondsPerCard) => {
        ...
        const closeRowsMenu = () => {
            ...
            try {
                ...
                log.swallow("remove rows menu wrapper", e);
            }
            rowsOpen = false;
        };
        ...
        const buildRowsOptions = () => {
            while (rowsMenu.firstChild)
                rowsMenu.removeChild(rowsMenu.firstChild);
        ...
        const openRowsMenu = () => {
            ...
            if (!sproutWrapper.parentElement)
            ...
            const onDocPointerDown = (ev) => {
                ...
                if (!t)
                    return;
                if (rowsWrap.contains(t) || rowsPopover.contains(t))
                    return;
                closeRowsMenu();
            };
        ...
        rowsBtn.addEventListener("pointerdown", (ev) => {
            if (ev.button !== 0)
                return;
            ev.preventDefault();
            ev.stopPropagation();
        ...
        const renderPager = () => {
            while (pagerHost.firstChild)
                pagerHost.removeChild(pagerHost.firstChild);
            ...
            pagerHost.appendChild(nav);
            const mkBtn = (label, tooltip, disabled, active, onClick) => {
            ...
            const mkEllipsisBtn = (targetPage) => {
                const b = document.createElement("button");
                b.type = "button";
            ...
            nav.appendChild(prev);
            if (start > 1) {
                nav.appendChild(mkBtn("1", tx("ui.analytics.table.pageN", "Page {page}", { page: 1 }), false, current === 1, () => {
                    currentPage = 0;
                    renderTable();
                    renderPager();
                }));
            ...
            for (let p = start; p <= end; p++) {
                nav.appendChild(mkBtn(String(p), tx("ui.analytics.table.pageN", "Page {page}", { page: p }), false, p === current, () => {
                    currentPage = p - 1;
                    renderTable();
                    renderPager();
                }));
            ...
            if (end < totalPagesLocal) {
                nav.appendChild(mkBtn(String(totalPagesLocal), tx("ui.analytics.table.pageN", "Page {page}", { page: totalPagesLocal }), false, current === totalPagesLocal, () => {
