...
function normalizeSearchValue(input) {
    return String(input || "")
        .toLowerCase()
        .replace(/\btag:\s*/g, "")
        .replace(/#/g, "")
...
function optionMatchesQuery(option, query) {
    ...
    if (!normalizedQuery)
        return false;
    const haystacks = [option.label, ...((_a = option.searchTexts) !== null && _a !== void 0 ? _a : [])]
        .map((text) => String(text || "").toLowerCase())
        .filter(Boolean);
    for (const haystack of haystacks) {
        if (haystack.includes(raw))
            return true;
        if (normalizeSearchValue(haystack).includes(normalizedQuery))
            return true;
    }
    return false;
}
...
export function buildScopeSearchPlaceholder(terms) {
...
function getPropertyDisplayKey(option) {
    if (option.propertyKey)
        return String(option.propertyKey).trim();
    const label = String(option.label || "").trim();
    const match = /^Property:\s*([^:]+)\s*:/i.exec(label);
    if (match === null || match === void 0 ? void 0 : match[1])
        return match[1].trim();
    const generic = /^([^:]+)\s*:/.exec(label);
    if (generic === null || generic === void 0 ? void 0 : generic[1])
        return generic[1].trim();
    return label || "Property";
}
function formatPropertyKeyLabel(input) {
...
function renderScopeItemIcon(target, type) {
    if (type !== "folder") {
...
export function mountSearchPopoverList(args) {
    var _a, _b;
    let filteredOptions = [];
    let activeIndex = -1;
    const typeFilterDefs = (_a = args.typeFilters) !== null && _a !== void 0 ? _a : [];
    ...
    const enabledPropertyKeys = new Set();
    ...
    const typeFilterBtn = typeFilterDefs.length
        ? searchWrap === null || searchWrap === void 0 ? void 0 : searchWrap.createEl("button", {
    ...
    const typeFilterPopover = typeFilterDefs.length
        ? searchWrap === null || searchWrap === void 0 ? void 0 : searchWrap.createDiv({ cls: "learnkit-scope-type-filter-popover learnkit-scope-type-filter-popover dropdown-menu hidden" })
        : null;
    const typeFilterList = (_b = typeFilterPopover === null || typeFilterPopover === void 0 ? void 0 : typeFilterPopover.createDiv({
    ...
    const applyTypeFilterLabel = () => {
        if (!typeFilterBtn || !typeFilterBtnLabel)
            return;
        typeFilterBtnLabel.setText("Filters");
    };
    const renderTypeFilterList = () => {
        if (!typeFilterList)
            return;
        ...
        for (const existingKey of Array.from(enabledPropertyKeys)) {
            if (!propertyGroups.has(existingKey))
                enabledPropertyKeys.delete(existingKey);
        }
        ...
        for (const entry of visibleTypeFilterDefs) {
            ...
            item.addEventListener("click", () => {
                if (enabledTypes.has(entry.type))
                    enabledTypes.delete(entry.type);
                else
        ...
        if (propertyGroupList.length) {
            ...
            for (const group of propertyGroupList) {
                ...
                item.addEventListener("mousedown", (evt) => evt.preventDefault());
                item.addEventListener("click", () => {
                    if (enabledPropertyKeys.has(group.key))
                        enabledPropertyKeys.delete(group.key);
                    else
                        enabledPropertyKeys.add(group.key);
                    renderTypeFilterList();
                    render();
                });
            }
    ...
    const applySearchIconOffset = () => {
        if (!searchWrap || !typeFilterBtn)
            return;
    ...
    const applyPlaceholder = () => {
        ...
        if (selectedPropertyKeys.length > 2)
            propertyNames.push("properties");
    ...
    const setTypeFilterOpen = (open) => {
        typeFilterOpen = open;
        typeFilterBtn === null || typeFilterBtn === void 0 ? void 0 : typeFilterBtn.setAttr("aria-expanded", open ? "true" : "false");
        if (!typeFilterPopover)
            return;
        if (open)
            typeFilterPopover.classList.remove("hidden");
        else
            typeFilterPopover.classList.add("hidden");
    };
    let outsideListenerBound = false;
    const handleOutsideTypeFilterClick = (evt) => {
        if (!typeFilterOpen)
            return;
        const target = evt.target;
        if (!target)
            return;
        if (typeFilterPopover === null || typeFilterPopover === void 0 ? void 0 : typeFilterPopover.contains(target))
            return;
        if (typeFilterBtn === null || typeFilterBtn === void 0 ? void 0 : typeFilterBtn.contains(target))
            return;
    ...
    const setTypeFilterOpenWithOutsideListener = (open) => {
