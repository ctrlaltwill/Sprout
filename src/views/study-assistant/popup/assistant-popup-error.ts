...
function humanizeCodeLabel(code: string): string {
  const raw = toCodeText(code).replace(/[.]/g, " ").trim();
  if (!raw) return "request error";
  if (/^[A-Z0-9_]+$/.test(raw)) return titleCaseWords(raw.replace(/_/g, " ").toLowerCase());
  return raw
    .replace(/[_-]+/g, " ")
    .replace(/\berror\b/gi, "")
    .replace(/\s+/g, " ")
...
function extractProviderErrorInfo(error: unknown): ProviderErrorInfo | null {
  ...
  if (!provider && !status && !detail && !code && !errorType) return null;
  return { provider, status, detail, code, errorType };
}

function summarizeProviderError(info: ProviderErrorInfo): string {
  const primaryCode = info.code || info.errorType;
  if (primaryCode) return humanizeCodeLabel(primaryCode);
...
function providerLikelyCauses(info: ProviderErrorInfo): string[] {
  ...
  const add = (value: string) => {
    if (!causes.includes(value)) causes.push(value);
  };
...
function formatCauseList(causes: string[]): string {
  if (!causes.length) return "the request could not be validated by the provider";
...
export function formatAssistantError(error: unknown, tx: Tx): string {
  ...
  if (providerInfo) {
    ...
    return tx(
      "ui.studyAssistant.error.providerHumanized",
      "Error received from AI provider ({provider}): {summary}. This might happen because {causes}.",
      { provider, summary, causes },
    );
  }
  ...
  if (httpFailure?.[1] && httpFailure?.[2]) {
    ...
    return tx(
      ...
      "Error received from AI provider ({provider}): HTTP {status}. This might happen because the API key, model, or endpoint configuration is invalid.",
      { provider, status },
    );
  }
