...
export function parseSuggestionRows(suggestion) {
    ...
    for (const row of noteRows) {
        const m = String(row !== null && row !== void 0 ? row : "").match(/^\s*([^|]+?)\s*\|\s*(.*?)\s*(?:\|\s*)?$/);
        if (!m)
            continue;
        ...
        if (suggestion.type === "basic" || suggestion.type === "reversed") {
            if ((key === "Q" || key === "RQ") && !out.question)
                out.question = value;
            if (key === "A" && !out.answer)
                out.answer = value;
        }
        else if (suggestion.type === "cloze") {
            if (key === "CQ" && !out.clozeText)
                out.clozeText = value;
        }
            ...
            if (key === "O" || key === "A") {
                ...
                if (key === "A")
                    out.correctOptionIndexes.push(out.options.length - 1);
            }
        }
...
export function normalizeOptionalGeneratorRows(suggestion, explicitRows, options) {
    const coreRows = [];
    let titleFromRows = "";
    let infoFromRows = "";
    let groupsFromRows = [];
    ...
    out.push(...coreRows);
    if (options.includeInfo && info)
        pushDelimitedField(out, "I", info);
    if (options.includeGroups && groups.length)
        pushDelimitedField(out, "G", groups.join(", "));
    return out;
}
export function buildSuggestionMarkdownLines(suggestion, options) {
    const explicitRows = Array.isArray(suggestion.noteRows)
        ? suggestion.noteRows.map((row) => String(row || "").trim()).filter(Boolean)
        : [];
    if (explicitRows.length)
        return [...normalizeOptionalGeneratorRows(suggestion, explicitRows, options), ""];
    const lines = [];
    ...
    if (suggestion.type === "basic") {
        ...
        if (ioSrc)
            pushDelimitedField(lines, "IO", ioSrc);
        ...
        const ioMaskMode = suggestion.ioMaskMode === "solo" || suggestion.ioMaskMode === "all"
            ...
            : null;
        if (ioMaskMode)
            pushDelimitedField(lines, "C", ioMaskMode);
    }
...
export function rewriteIoNoteRows(noteRows, ioSrc) {
    return noteRows.map((row) => {
        const raw = String(row !== null && row !== void 0 ? row : "");
