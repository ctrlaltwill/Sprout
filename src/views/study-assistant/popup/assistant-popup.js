 ...
 */
import { MarkdownView, Modal, Notice, Platform, Setting, TFile, setIcon } from "obsidian";
...
import { generateStudyAssistantChatReplyStreaming, generateStudyAssistantSuggestions, generateStudyAssistantSuggestionsStreaming, extractVisibleEditResponseText, parseEditProposal, classifyUserIntent, parseUserRequestOverrides, } from "../../../platform/integrations/ai/study-assistant-generator";
...
import { appendFlashcardDisclaimerIfNeeded, flashcardDisclaimerText, shouldShowGenerateSwitch, generateNonFlashcardHintText, shouldShowAskSwitch, isGenerateFlashcardRequest, extractRequestedGenerateCount, generateExcessiveCountHintText, allFlashcardsInsertedText, isTestGenerationRequest, parseTestConfigFromRequest, } from "../chat/generation-helpers";
...
import { setEditProposals, clearEditProposals, EDIT_PROPOSAL_RESOLVED_EVENT_NAME, } from "../editor/edit-decorations";
import { ASSISTANT_MODES, CHAT_LOG_SYNC_EVENT_NAME, isAssistantReviewDepth, } from "./assistant-popup-constants";
import { normalizeRemoteConversationRefs, normalizeSuggestionBatches, } from "./assistant-popup-normalizers";
...
import { buildSuggestionMarkdownLines, parseSuggestionRows, rewriteIoNoteRows, } from "./assistant-popup-suggestion-rows";
...
import { countPendingEditProposalEdits, deriveEditProposalStatusFromEdits, getEditProposalBulkActionCopy, } from "./assistant-popup-helpers";
import { bestEffortDeleteRemoteConversation, clearRemoteConversationForMode, getRemoteConversationForMode, setRemoteConversationForMode, shouldSyncDeletesToProvider, } from "./assistant-popup-remote";
import { isImageExt, isSupportedAttachmentExt, MAX_ATTACHMENTS, readVaultFileAsAttachment, readFileInputAsAttachment, SUPPORTED_FILE_ACCEPT, } from "../../../platform/integrations/ai/attachment-helpers";
...
export class SproutAssistantPopup {
    constructor(plugin) {
        ...
        this._instanceId = `assistant-popup-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
        // DOM nodes
        ...
        this._pendingTestDifficulty = "medium";
        // Streaming state
        ...
        this._activeSessionLeaf = null;
        this._leafSessions = new WeakMap();
    ...
    _loadPersistedUserResize() {
        try {
            const raw = window.localStorage.getItem(ASSISTANT_POPUP_SIZE_STORAGE_KEY);
            if (!raw)
                return;
            const parsed = JSON.parse(raw);
            const width = Number(parsed === null || parsed === void 0 ? void 0 : parsed.width);
            const height = Number(parsed === null || parsed === void 0 ? void 0 : parsed.height);
    ...
    _persistUserResize() {
        try {
            window.localStorage.setItem(ASSISTANT_POPUP_SIZE_STORAGE_KEY, JSON.stringify({
    ...
    _registerChatLogSyncListener() {
        if (this._onChatLogSynced)
            return;
        this._onChatLogSynced = (event) => {
            const custom = event;
            const detail = custom === null || custom === void 0 ? void 0 : custom.detail;
            if (!detail || detail.sourceId === this._instanceId)
                return;
            const file = this.activeFile;
            if (!file || detail.filePath !== file.path)
                return;
        ...
        window.addEventListener(CHAT_LOG_SYNC_EVENT_NAME, this._onChatLogSynced);
    }
    _unregisterChatLogSyncListener() {
        if (!this._onChatLogSynced)
            return;
        window.removeEventListener(CHAT_LOG_SYNC_EVENT_NAME, this._onChatLogSynced);
        this._onChatLogSynced = null;
    }
    _registerEditProposalResolutionListener() {
        if (this._onEditProposalResolved)
            return;
        this._onEditProposalResolved = (event) => {
            const custom = event;
            const detail = custom === null || custom === void 0 ? void 0 : custom.detail;
            if (!detail)
                return;
            if (!this._applyInlineEditProposalResolution(detail))
                return;
            this._scheduleSave();
            if (this.isOpen)
                this.render();
        };
        window.addEventListener(EDIT_PROPOSAL_RESOLVED_EVENT_NAME, this._onEditProposalResolved);
    }
    _unregisterEditProposalResolutionListener() {
        if (!this._onEditProposalResolved)
            return;
        window.removeEventListener(EDIT_PROPOSAL_RESOLVED_EVENT_NAME, this._onEditProposalResolved);
        this._onEditProposalResolved = null;
    }
    _applyInlineEditProposalResolution(detail) {
        for (const message of this.chatMessages) {
            const editProposal = message.editProposal;
            if (!editProposal || editProposal.status === "expired")
                continue;
    ...
    _emitChatLogSynced(filePath) {
        window.dispatchEvent(new CustomEvent(CHAT_LOG_SYNC_EVENT_NAME, {
            detail: {
    ...
    importChatData(_data) { }
    /** @deprecated Chat data is now persisted per-note in the chats/ folder. */
    exportChatData() { return {}; }
    // ---------------------------------------------------------------------------
    //  Per-note chat file persistence
    // ---------------------------------------------------------------------------
    _getChatsFolderPath() {
        var _a, _b, _c;
        const configDir = (_b = (_a = this.plugin.app) === null || _a === void 0 ? void 0 : _a.vault) === null || _b === void 0 ? void 0 : _b.configDir;
        const pluginId = (_c = this.plugin.manifest) === null || _c === void 0 ? void 0 : _c.id;
        if (!configDir || !pluginId)
            return null;
        return joinPath(configDir, "plugins", pluginId, "chats");
    }
    _getChatFilePath(file) {
    ...
    async _loadChatForFile(file, options) {
        var _a, _b, _c, _d, _e;
        if (!((_c = (_b = (_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.studyAssistant) === null || _b === void 0 ? void 0 : _b.privacy) === null || _c === void 0 ? void 0 : _c.saveChatHistory))
            return;
        const adapter = (_e = (_d = this.plugin.app) === null || _d === void 0 ? void 0 : _d.vault) === null || _e === void 0 ? void 0 : _e.adapter;
        const chatPath = this._getChatFilePath(file);
        if (!adapter || !chatPath)
            return;
        const preserveDrafts = !!(options === null || options === void 0 ? void 0 : options.preserveDrafts);
        ...
        try {
            if (await adapter.exists(chatPath)) {
                ...
                const normalizedBatches = normalizeSuggestionBatches(data.generateSuggestionBatches, this.chatMessages, this.generateMessages);
    ...
    async _saveChatForFile(file, hasData, data) {
        var _a, _b, _c, _d;
        ...
        if (!adapter || !chatPath || !chatsFolder)
            return;
        if (!hasData) {
            ...
            try {
                ...
                log.swallow("remove empty chat file", e);
            }
            return;
        }
        try {
            if (!(await adapter.exists(chatsFolder))) {
                await ((_d = (_c = adapter).mkdir) === null || _d === void 0 ? void 0 : _d.call(_c, chatsFolder));
            }
     ...
     */
    async _appendResponseToPersistedChat(file, field, ...newMessages) {
        ...
        const chatPath = this._getChatFilePath(file);
        const chatsFolder = this._getChatsFolderPath();
        if (!adapter || !chatPath || !chatsFolder || !newMessages.length)
            return;
        try {
            let data = {};
            if (await adapter.exists(chatPath)) {
            ...
            data[field] = arr;
            if (!(await adapter.exists(chatsFolder))) {
                await ((_g = (_f = adapter).mkdir) === null || _g === void 0 ? void 0 : _g.call(_f, chatsFolder));
            }
            await adapter.write(chatPath, JSON.stringify(data, null, 2));
            this._emitChatLogSynced(file.path);
        }
    ...
    async _appendGeneratedResponseToPersistedChat(file, field, message, batch) {
        var _a, _b, _c, _d, _e, _f, _g;
        if (!((_c = (_b = (_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.studyAssistant) === null || _b === void 0 ? void 0 : _b.privacy) === null || _c === void 0 ? void 0 : _c.saveChatHistory))
            return;
        const adapter = (_e = (_d = this.plugin.app) === null || _d === void 0 ? void 0 : _d.vault) === null || _e === void 0 ? void 0 : _e.adapter;
        ...
        try {
            ...
            const messages = Array.isArray(data[field]) ? data[field] : [];
            const assistantMessageIndex = messages.length;
            messages.push(message);
            data[field] = messages;
            if (batch === null || batch === void 0 ? void 0 : batch.suggestions.length) {
    ...
    _currentModeLabel() {
        if (this.mode === "assistant")
            return this._tx("ui.studyAssistant.mode.assistant", "Ask");
        if (this.mode === "review")
            return this._tx("ui.studyAssistant.mode.review", "Review");
        return this._tx("ui.studyAssistant.mode.generator", "Generate");
    }
    async _confirmDeleteAllConversations() {
        const message = this._tx("ui.studyAssistant.chat.confirmClearAll", "Clear all Companion chats? This permanently deletes all saved chat logs and resets current AI context.");
    ...
    async _resetCurrentModeConversation() {
        var _a;
        for (const mode of ASSISTANT_MODES) {
            const remoteRef = getRemoteConversationForMode(this.remoteConversationsByMode, mode);
            if (shouldSyncDeletesToProvider((_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.studyAssistant) && remoteRef) {
        ...
        this._captureCurrentLeafSession();
        this.render();
        new Notice(this._tx("ui.studyAssistant.chat.resetCurrentMode", "LearnKit – reset {mode} conversation", { mode: this._currentModeLabel() }));
    }
    async _deleteAllConversations() {
        var _a, _b, _c, _d, _e, _f, _g;
        ...
        if (shouldSyncDeletesToProvider((_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.studyAssistant)) {
        ...
        let deletedCount = 0;
        try {
            const adapter = (_c = (_b = this.plugin.app) === null || _b === void 0 ? void 0 : _b.vault) === null || _c === void 0 ? void 0 : _c.adapter;
            const chatsFolder = this._getChatsFolderPath();
            if (adapter && chatsFolder && await adapter.exists(chatsFolder)) {
                const listResult = await ((_e = (_d = adapter).list) === null || _e === void 0 ? void 0 : _e.call(_d, chatsFolder));
                const files = (_f = listResult === null || listResult === void 0 ? void 0 : listResult.files) !== null && _f !== void 0 ? _f : [];
                for (const filePath of files) {
                    await adapter.remove(filePath);
                    deletedCount++;
                }
                const folders = ((_g = listResult === null || listResult === void 0 ? void 0 : listResult.folders) !== null && _g !== void 0 ? _g : []).sort((a, b) => b.length - a.length);
                for (const folderPath of folders) {
                    try {
                        await adapter.remove(folderPath);
                    }
                    catch (e) {
                        log.swallow("remove nested chats folder", e);
                    }
                }
                try {
                    await adapter.remove(chatsFolder);
                }
                catch (e) {
                    log.swallow("remove chats folder", e);
                }
            }
            this._captureCurrentLeafSession();
            new Notice(this._tx("ui.studyAssistant.chat.clearedAll", "LearnKit – all chats have been cleared"));
            if (remoteAttemptCount > 0) {
                new Notice(this._tx("ui.studyAssistant.chat.remoteDeleteSummary", "LearnKit – remote delete requested for {attempted} conversation(s); deleted {deleted}", { attempted: remoteAttemptCount, deleted: remoteDeletedCount }));
            }
        }
        catch (e) {
    ...
    async onFileRename(oldPath, newFile) {
        var _a, _b, _c, _d;
        const adapter = (_b = (_a = this.plugin.app) === null || _a === void 0 ? void 0 : _a.vault) === null || _b === void 0 ? void 0 : _b.adapter;
        const chatsFolder = this._getChatsFolderPath();
        if (!adapter || !chatsFolder)
            return;
        const oldName = (_d = (_c = oldPath.split("/").pop()) === null || _c === void 0 ? void 0 : _c.replace(/\.md$/i, "")) !== null && _d !== void 0 ? _d : "";
        if (!oldName)
            return;
        const oldChatPath = joinPath(chatsFolder, `${oldName}.json`);
        const newChatPath = this._getChatFilePath(newFile);
        if (!newChatPath || oldChatPath === newChatPath)
            return;
        try {
    ...
    mount() {
        ...
        this._onClickOutside = (e) => {
            ...
            if (this._getAssistantLocation() !== "modal")
                return;
            if (this._suspendPopupAutoClose)
                return;
            const target = e.target;
            if ((_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.contains(target))
                return;
            if ((_b = this._headerMenuPopoverEl) === null || _b === void 0 ? void 0 : _b.contains(target))
                return;
            if ((_c = this.triggerBtn) === null || _c === void 0 ? void 0 : _c.contains(target))
                return;
    ...
    mountEmbedded(host) {
        ...
        this._registerChatLogSyncListener();
        this._registerEditProposalResolutionListener();
        (_a = this.triggerBtn) === null || _a === void 0 ? void 0 : _a.remove();
        this.triggerBtn = null;
        (_b = this.triggerHotzone) === null || _b === void 0 ? void 0 : _b.remove();
        this.triggerHotzone = null;
        (_c = this._hotzoneProximityCleanup) === null || _c === void 0 ? void 0 : _c.call(this);
        this._hotzoneProximityCleanup = null;
        this._isTriggerButtonHovered = false;
        if (!this.popupEl) {
        ...
        (_d = this.popupEl) === null || _d === void 0 ? void 0 : _d.removeClass("is-hidden");
        if (this.activeFile) {
    ...
    unmountEmbedded() {
        ...
        (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.remove();
        this.popupEl = null;
        this.isEmbeddedMode = false;
        this.embeddedHost = null;
        this.isOpen = false;
    ...
    destroy() {
        ...
        (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.remove();
        this.popupEl = null;
        (_b = this.triggerBtn) === null || _b === void 0 ? void 0 : _b.remove();
        this.triggerBtn = null;
        (_c = this.triggerHotzone) === null || _c === void 0 ? void 0 : _c.remove();
        this.triggerHotzone = null;
        (_d = this._hotzoneProximityCleanup) === null || _d === void 0 ? void 0 : _d.call(this);
        this._hotzoneProximityCleanup = null;
        this._isTriggerButtonHovered = false;
        this._isTriggerReplyNotificationActive = false;
        this.isOpen = false;
    ...
    onFileOpen(file) {
        ...
        if (!this._isActiveMarkdownNoteContext()) {
            (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.addClass("is-hidden");
            (_b = this.triggerBtn) === null || _b === void 0 ? void 0 : _b.removeClass("is-open");
            return;
        }
        const previousPath = ((_c = this.activeFile) === null || _c === void 0 ? void 0 : _c.path) || "";
        const nextPath = (file === null || file === void 0 ? void 0 : file.path) || "";
        ...
        if (this.isOpen) {
            this.ensurePopup();
            (_d = this.popupEl) === null || _d === void 0 ? void 0 : _d.removeClass("is-hidden");
            (_e = this.triggerBtn) === null || _e === void 0 ? void 0 : _e.addClass("is-open");
            this.render();
        }
        else {
            (_f = this.popupEl) === null || _f === void 0 ? void 0 : _f.addClass("is-hidden");
            (_g = this.triggerBtn) === null || _g === void 0 ? void 0 : _g.removeClass("is-open");
        }
        this._applyPresentationState();
    }
    _shutdownTransientUiActivity() {
        var _a, _b, _c;
        this._headerMenuOpen = false;
        (_a = this._headerMenuAbort) === null || _a === void 0 ? void 0 : _a.abort();
        this._headerMenuAbort = null;
        this._teardownHeaderMenuPortal();
        this.reviewDepthMenuOpen = false;
        (_b = this._reviewDepthMenuAbort) === null || _b === void 0 ? void 0 : _b.abort();
        this._reviewDepthMenuAbort = null;
        ...
        (_c = this._resizeCleanup) === null || _c === void 0 ? void 0 : _c.call(this);
        this._resizeCleanup = null;
    }
    onActiveLeafChange() {
        ...
        if (this.isOpen) {
            this.ensurePopup();
            (_c = this.popupEl) === null || _c === void 0 ? void 0 : _c.removeClass("is-hidden");
            (_d = this.triggerBtn) === null || _d === void 0 ? void 0 : _d.addClass("is-open");
            this.render();
        }
        else {
            (_e = this.popupEl) === null || _e === void 0 ? void 0 : _e.addClass("is-hidden");
            (_f = this.triggerBtn) === null || _f === void 0 ? void 0 : _f.removeClass("is-open");
        }
        this._applyPresentationState();
    }
    // ---------------------------------------------------------------------------
    ...
    open() {
        ...
        if (this.isEmbeddedMode) {
            this.isOpen = true;
            (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.removeClass("is-hidden");
            this._applyPresentationState();
            this.render();
            return;
        }
        ...
        if (this._getAssistantLocation() !== "modal") {
            this.isOpen = false;
            (_b = this.popupEl) === null || _b === void 0 ? void 0 : _b.addClass("is-hidden");
            this._applyPresentationState();
            return;
        }
        if (this.isOpen)
        ...
        this._syncHosts();
        if (!this._isActiveMarkdownNoteContext()) {
            (_c = this.popupEl) === null || _c === void 0 ? void 0 : _c.addClass("is-hidden");
            (_d = this.triggerBtn) === null || _d === void 0 ? void 0 : _d.removeClass("is-open");
            this.isOpen = false;
            return;
        }
        this.activeFile = this.plugin.app.workspace.getActiveFile();
        ...
        (_e = this.popupEl) === null || _e === void 0 ? void 0 : _e.removeClass("is-closing");
        (_f = this.popupEl) === null || _f === void 0 ? void 0 : _f.removeClass("is-hidden");
        this._clearPendingReplyForMode(this.mode);
        (_g = this.triggerBtn) === null || _g === void 0 ? void 0 : _g.addClass("is-open");
        this.ensurePopup();
        this._syncHosts();
        if (this.activeFile) {
            void this._loadChatForFile(this.activeFile).then(() => this.render());
    ...
    close(force = false) {
        ...
        if (this.popupEl) {
            ...
            this._popupCloseTimer = setTimeout(() => {
                var _a, _b;
                this._popupCloseTimer = null;
                (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.removeClass("is-closing");
                (_b = this.popupEl) === null || _b === void 0 ? void 0 : _b.addClass("is-hidden");
                this._applyPresentationState();
            }, 180);
        }
    ...
    _toggleNativeLeftSidebar() {
        var _a, _b;
        const commandApi = this.plugin.app.commands;
        if (commandApi === null || commandApi === void 0 ? void 0 : commandApi.executeCommandById("app:toggle-left-sidebar"))
            return;
        if (commandApi === null || commandApi === void 0 ? void 0 : commandApi.executeCommandById("app:toggle-sidebar"))
            return;
        const workspaceLike = this.plugin.app.workspace;
        (_b = (_a = workspaceLike.leftSplit) === null || _a === void 0 ? void 0 : _a.toggle) === null || _b === void 0 ? void 0 : _b.call(_a);
    }
    _navigateFromHeaderMenu(target) {
        this._headerMenuOpen = false;
        this.close(true);
        this.render();
        if (target === "home") {
    ...
    _isAssistantEnabled() {
        var _a, _b;
        return !!((_b = (_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.studyAssistant) === null || _b === void 0 ? void 0 : _b.enabled);
    }
    _getAssistantLocation() {
        return "modal";
    }
    _getModalButtonVisibility() {
        var _a, _b;
        const visibility = (_b = (_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.studyAssistant) === null || _b === void 0 ? void 0 : _b.modalButtonVisibility;
        if (visibility === "hidden" || visibility === "hover")
            return visibility;
        return "always";
    }
    _refreshTriggerIcon() {
    ...
    _clearPendingReplyForMode(mode) {
        if (!this._hasPendingReplyForMode(mode))
            return;
    ...
    _applyPresentationState() {
        ...
        if (this.isEmbeddedMode) {
            (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.addClass("learnkit-assistant-popup-embedded");
            (_b = this.popupEl) === null || _b === void 0 ? void 0 : _b.removeClass("is-hidden");
            this.isOpen = true;
            this._refreshTriggerIcon();
            return;
        }
        ...
        const shouldShowHoverTrigger = this._isTriggerHotzoneActive
            ...
            || this.isOpen
            || ((_c = this.popupEl) === null || _c === void 0 ? void 0 : _c.hasClass("is-closing")) === true
            || this._getPendingReplyTabCount() > 0
            || this._isTriggerReplyNotificationActive;
        (_d = this.triggerBtn) === null || _d === void 0 ? void 0 : _d.toggleClass("is-hidden", !enabled || isWidgetLocation || shouldHideTrigger);
        (_e = this.triggerBtn) === null || _e === void 0 ? void 0 : _e.toggleClass("is-hover-only", enabled && !isWidgetLocation && shouldUseHover);
        (_f = this.triggerBtn) === null || _f === void 0 ? void 0 : _f.toggleClass("is-hover-active", enabled && !isWidgetLocation && shouldUseHover && shouldShowHoverTrigger);
        (_g = this.triggerBtn) === null || _g === void 0 ? void 0 : _g.toggleClass("is-open", enabled && !isWidgetLocation && this.isOpen);
        this._refreshTriggerIcon();
        (_h = this.triggerHotzone) === null || _h === void 0 ? void 0 : _h.toggleClass("is-hidden", !enabled || isWidgetLocation || shouldHideTrigger || !shouldUseHover || this.isOpen);
        if (this.popupEl) {
        ...
        if (!enabled) {
            ...
            this.isOpen = false;
            (_j = this.popupEl) === null || _j === void 0 ? void 0 : _j.addClass("is-hidden");
            return;
        }
        if (isWidgetLocation) {
            if (this.isOpen) {
            ...
            (_k = this.popupEl) === null || _k === void 0 ? void 0 : _k.addClass("is-hidden");
            return;
        }
        if (!this.isOpen && !((_l = this.popupEl) === null || _l === void 0 ? void 0 : _l.hasClass("is-closing")))
            (_m = this.popupEl) === null || _m === void 0 ? void 0 : _m.addClass("is-hidden");
    }
    _notifyIncomingAssistantReply(mode) {
     ...
     * in the thread.
     */
    _isReviewReply(msgIndex) {
        const msgs = this.chatMessages;
        if (msgIndex < 1 || msgIndex >= msgs.length)
            return false;
        // Only show on the last assistant message
        if (msgs.slice(msgIndex + 1).some(m => m.role === "assistant"))
            return false;
    ...
    _extractGeneratedCodeFromReply(reply) {
    ...
    _resolveRequestedGenerateCount(userMessage, fallbackCount) {
    ...
    _formatGeneratingSuggestionsMessage(count) {
        return this._tx("ui.studyAssistant.generator.creatingSummary", "Creating {count} flashcard{suffix}. Use + to insert.", {
            count,
            suffix: count === 1 ? "" : "s",
        });
    }
    _suggestionStreamKey(suggestion) {
    ...
    _formatGeneratedSuggestionsAsJsonMessage(suggestions) {
        const count = Array.isArray(suggestions) ? suggestions.length : 0;
        if (count <= 0) {
            return this._tx("ui.studyAssistant.generator.empty", "No valid suggestions were returned. Try adjusting your model, prompt, or enabled card types.");
        }
        return this._tx("ui.studyAssistant.generator.createdSummary", "Created {count} flashcard{suffix}. Use + to insert.", {
            count,
            suffix: count === 1 ? "" : "s",
        });
    }
    _shouldShowGenerateMoreButton(text, assistantMessageIndex, source) {
        if (String(text || "").trim() !== this._allFlashcardsInsertedText())
            return false;
        const batch = this._getSuggestionBatchForAssistantIndex(assistantMessageIndex, source);
        return !(batch === null || batch === void 0 ? void 0 : batch.suggestions.length);
    }
    _renderGenerateMoreButton(parent, ariaLabel) {
    ...
    _openAttachmentPicker() {
        ...
        const modal = new AttachmentPickerModal(this.plugin.app, candidates, (file) => {
            ...
            this._attachedFiles.push(attached);
            this.render();
        }, (_b = (_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.general) === null || _b === void 0 ? void 0 : _b.interfaceLanguage);
        modal.open();
    }
    _renderAttachmentChips(parent) {
        if (!this._attachedFiles.length)
            return;
    ...
    _isChatWrapNearBottom(chatWrap) {
    ...
    _captureVisibleChatScrollTop(mode) {
        var _a;
        const chatWrap = (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.querySelector(".learnkit-assistant-popup-chat-wrap");
        if (!(chatWrap instanceof HTMLElement))
            return null;
        this._autoFollowChatByMode[mode] = this._isChatWrapNearBottom(chatWrap);
        return chatWrap.scrollTop;
    }
    _bindChatAutoFollowTracking(chatWrap, mode) {
        chatWrap.addEventListener("scroll", () => {
            this._autoFollowChatByMode[mode] = this._isChatWrapNearBottom(chatWrap);
        }, { passive: true });
    }
    _beginStreaming(mode) {
        this._streamAbortRequestedByUser = false;
        this._activeStreamingMode = mode;
        this._captureVisibleChatScrollTop(mode);
    }
    _isStopActionAvailable(mode) {
        return this._activeStreamingMode === mode && !!this._streamAbort;
    }
    _requestStopActiveStream(mode) {
        var _a;
        if (!this._isStopActionAvailable(mode))
            return;
        this._streamAbortRequestedByUser = true;
        (_a = this._streamAbort) === null || _a === void 0 ? void 0 : _a.abort();
    }
    _consumeUserStopRequest() {
        const requested = this._streamAbortRequestedByUser;
        this._streamAbortRequestedByUser = false;
        return requested;
    }
    _stoppedReplyStatusText() {
        return this._tx("ui.studyAssistant.chat.stoppedReply", "Response stopped.");
    }
    _stoppedPartialReplyStatusText() {
        return this._tx("ui.studyAssistant.chat.stoppedReplyPartial", "Response stopped early.");
    }
    _stoppedGenerationStatusText(generatedCount = 0) {
        if (generatedCount > 0) {
            return this._tx("ui.studyAssistant.generator.stoppedAfterCount", "Stopped after generating {count} flashcard{suffix}. Use + to insert.", {
    ...
    _finalizeStoppedAssistantMessage(messages, streamingMsg, placeholderPushed) {
    ...
    _configureComposerSendButton(button, options) {
        const applyState = () => {
    ...
    _shouldAutoFollowChat(mode) {
        return this._autoFollowChatByMode[mode];
    }
    _restoreChatScrollAfterRender(chatWrap, mode, preservedScrollTop, options = {}) {
    ...
    _updateStreamingBubble() {
        var _a, _b;
        const rows = (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.querySelectorAll(".learnkit-assistant-popup-message-row.is-assistant");
        const lastRow = rows === null || rows === void 0 ? void 0 : rows[rows.length - 1];
        const bubbleEl = lastRow === null || lastRow === void 0 ? void 0 : lastRow.querySelector(".learnkit-assistant-popup-message-content");
        if (bubbleEl) {
            ...
            const streamingMode = (_b = this._activeStreamingMode) !== null && _b !== void 0 ? _b : this.mode;
            if (chatWrap instanceof HTMLElement && this._shouldAutoFollowChat(streamingMode)) {
                chatWrap.scrollTop = chatWrap.scrollHeight;
            }
        }
    }
    _isStreamingAssistantMessage(messageIndex, messages) {
        var _a;
        return this._isStreaming
            && messageIndex === messages.length - 1
            && ((_a = messages[messageIndex]) === null || _a === void 0 ? void 0 : _a.role) === "assistant";
    }
    _renderAssistantWelcomeActions(parent) {
    ...
    _renderSwitchToGenerateButton(parent) {
    ...
    _renderSwitchToAskButton(parent) {
        const actions = parent.createDiv({ cls: "learnkit-assistant-popup-review-starters learnkit-assistant-popup-review-starters learnkit-assistant-popup-message-actions learnkit-assistant-popup-message-actions" });
        const btn = actions.createEl("button", {
    ...
    async _collectRemoteConversationsFromSavedChats() {
        var _a, _b, _c, _d, _e;
        const adapter = (_b = (_a = this.plugin.app) === null || _a === void 0 ? void 0 : _a.vault) === null || _b === void 0 ? void 0 : _b.adapter;
        const chatsFolder = this._getChatsFolderPath();
        if (!adapter || !chatsFolder)
        ...
        if (!(await adapter.exists(chatsFolder)))
            return [];
        const listResult = await ((_d = (_c = adapter).list) === null || _d === void 0 ? void 0 : _d.call(_c, chatsFolder));
        const files = (_e = listResult === null || listResult === void 0 ? void 0 : listResult.files) !== null && _e !== void 0 ? _e : [];
        const refs = [];
        for (const filePath of files) {
            try {
    ...
    _legacyBatchesFromSuggestions(messages, legacySuggestions) {
        var _a;
        if (!legacySuggestions.length)
            return [];
    ...
    _getSuggestionBatchForAssistantIndex(assistantMessageIndex, source) {
        var _a;
        return (_a = this.generateSuggestionBatches.find((batch) => batch.assistantMessageIndex === assistantMessageIndex
            && (batch.source ? batch.source === source : true))) !== null && _a !== void 0 ? _a : null;
    }
    _getActiveMarkdownLeaf() {
    ...
    _syncSessionForActiveLeaf() {
        ...
        this._restoreSession(cached !== null && cached !== void 0 ? cached : this._newLeafSession());
    }
    _isActiveMarkdownNoteContext() {
        var _a;
        const activeFile = this.plugin.app.workspace.getActiveFile();
        if (activeFile && activeFile.path.toLowerCase().endsWith(".md"))
            return true;
        if (this.activeFile && this.activeFile.path.toLowerCase().endsWith(".md"))
            return true;
        const leafFile = (((_a = this._activeSessionLeaf) === null || _a === void 0 ? void 0 : _a.view) instanceof MarkdownView)
    ...
    _getHostLeaf() {
        ...
        if (((_a = this._activeSessionLeaf) === null || _a === void 0 ? void 0 : _a.view) instanceof MarkdownView)
            return this._activeSessionLeaf;
        return null;
    }
    _getHostElement() {
        var _a;
        const hostLeaf = this._getHostLeaf();
        if (!((hostLeaf === null || hostLeaf === void 0 ? void 0 : hostLeaf.view) instanceof MarkdownView))
            return null;
        const leafContainer = hostLeaf.view.containerEl;
        if (!leafContainer || !leafContainer.isConnected)
            return null;
        return (_a = leafContainer.querySelector(":scope > .view-content")) !== null && _a !== void 0 ? _a : leafContainer.querySelector(".view-content");
    }
    _attachToBestHost(el) {
        if (this.isEmbeddedMode) {
    ...
    _attachHotzoneToBestHost() {
        if (!this.triggerHotzone)
            return false;
        return this._attachToBestHost(this.triggerHotzone);
    }
    _buildTriggerHotzone() {
    ...
    _syncHosts() {
        ...
        if (this.popupEl)
            this._attachToBestHost(this.popupEl);
     ...
     */
    _attachHostProximityHover() {
        var _a;
        (_a = this._hotzoneProximityCleanup) === null || _a === void 0 ? void 0 : _a.call(this);
        this._hotzoneProximityCleanup = null;
        const host = this._getHostElement();
        if (!host || !this.triggerBtn)
            return;
    ...
    getActiveNoteDisplayName() {
        if (!this.activeFile)
            return null;
        const basename = this.activeFile.basename;
    ...
    async readActiveMarkdown(file) {
        var _a;
        const view = this.plugin.app.workspace.getActiveViewOfType(MarkdownView);
        if (((_a = view === null || view === void 0 ? void 0 : view.file) === null || _a === void 0 ? void 0 : _a.path) === file.path && view.editor) {
    ...
    async buildVisionImageDataUrls(file, imageRefs) {
    ...
    async readVisionClipboardImage(file, ref, maxBytes = 5 * 1024 * 1024) {
    ...
    async detectOcrTextRegionsForImage(file, ref) {
    ...
    async buildVisionImageDescriptors(file, noteContent, imageRefs, includeOcrTargets) {
        ...
        if (!includeOcrTargets || !descriptors.length)
            return descriptors;
        const ocrEntries = await Promise.all(imageRefs.map(async (ref) => [ref, await this.detectOcrTextRegionsForImage(file, ref)]));
        const ocrByRef = new Map(ocrEntries.filter((entry) => entry[1].length > 0));
        return descriptors.map((descriptor) => {
            const ocrTextRegions = ocrByRef.get(descriptor.ref);
            return (ocrTextRegions === null || ocrTextRegions === void 0 ? void 0 : ocrTextRegions.length)
                ? { ...descriptor, ocrTextRegions }
                : descriptor;
        });
    }
    async resolveVisionImageRefs(file, refs) {
     ...
     * wikilink expansion).
     */
    async buildNoteEmbedNonImageAttachmentUrls(file, embedRefs) {
    ...
    async buildNoteLinkedAttachmentUrls(file, markdown) {
    ...
    async buildLinkedNotesTextContext(file, markdown) {
        this._lastLinkedContextStats = { included: 0, skipped: 0, truncatedNotes: 0 };
        const linkedRefs = this.extractLinkedRefs(markdown);
        if (!linkedRefs.length)
            return "";
        ...
        for (const ref of linkedRefs) {
            ...
            if (clipped.length < content.length)
                this._lastLinkedContextStats.truncatedNotes += 1;
    ...
    _emitLinkedContextTruncationNotice() {
        const { skipped, truncatedNotes } = this._lastLinkedContextStats;
        if (!skipped && !truncatedNotes)
            return;
        const parts = [];
        if (truncatedNotes > 0)
            parts.push(this._tx("ui.assistant.context.notesTruncated", "{count} linked note{suffix} truncated", { count: truncatedNotes, suffix: truncatedNotes > 1 ? "s" : "" }));
        if (skipped > 0)
            parts.push(this._tx("ui.assistant.context.notesSkipped", "{count} linked note{suffix} skipped", { count: skipped, suffix: skipped > 1 ? "s" : "" }));
    ...
    _appendPlainTextCustomInstructions(noteContent, customInstructions, label) {
    ...
    renderMarkdownMessage(parent, text) {
    ...
    get _speechRecognitionSupported() {
    ...
    async _startVoiceInput() {
        var _a;
        if (this._isListening || !this._speechRecognitionSupported)
            return;
        const recognitionCtorSource = (_a = window.SpeechRecognition) !== null && _a !== void 0 ? _a : window.webkitSpeechRecognition;
        if (typeof recognitionCtorSource !== "function")
            return;
        const speechRecognitionCtor = recognitionCtorSource;
        ...
        if (!localReady) {
            this.chatError = this._tx("ui.studyAssistant.chat.voiceLocalUnavailable", "On-device dictation is not available in this runtime. Falling back to browser speech service.");
        }
        this._voiceStopRequested = false;
        this._clearVoiceAutoStopTimer();
        ...
        recognition.onerror = (event) => {
            const code = String((event === null || event === void 0 ? void 0 : event.error) || "unknown");
            if (code === "network" && networkRetryCount < 1) {
                shouldRetryNetwork = true;
                this.chatError = this._tx("ui.studyAssistant.chat.voiceRetrying", "Voice service disconnected. Retrying once...");
                this.render();
                return;
            }
            this._isListening = false;
    ...
    async _startVoiceMeter() {
        var _a;
        if (this._voiceMeterAnalyser || !((_a = navigator.mediaDevices) === null || _a === void 0 ? void 0 : _a.getUserMedia))
            return;
        try {
    ...
    _tickVoiceMeter() {
        if (!this._voiceMeterAnalyser || !this._isListening)
            return;
    ...
    _applyVoiceMeterLevel(level) {
        var _a;
        const bars = (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.querySelectorAll(".learnkit-assistant-popup-voice-meter-bar");
        if (!(bars === null || bars === void 0 ? void 0 : bars.length))
            return;
        ...
        bars.forEach((bar, idx) => {
            ...
            setCssProps(bar, "height", `${Math.max(4, Math.min(20, height))}px`);
        });
    }
    _stopVoiceMeter() {
    ...
    _setLiveAssistantDraft(value) {
        var _a;
        this.chatDraft = value;
        const input = (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.querySelector(".learnkit-assistant-popup-input");
        if (!input)
            return;
        input.value = value;
     ...
     */
    _bindComposerFocusOnExplicitClick(composer, shell, input) {
        composer.addEventListener("mousedown", (event) => {
            ...
            if (!target)
                return;
            if (target.closest(".learnkit-assistant-popup-send"))
                return;
            ...
            if (!isInputTarget && !isComposerTarget)
                return;
            requestAnimationFrame(() => input.focus());
        });
    }
    async _configureLocalDictation(speechCtor, recognition, lang) {
        if (!("processLocally" in recognition)) {
            return false;
        }
        recognition.processLocally = true;
        if (!speechCtor.available)
            return true;
        try {
    ...
    _toggleReplyAudio(text, messageIndex) {
        if (!this.plugin.settings.studyAssistant.voiceChat)
            return;
    ...
    _syncActiveReplyAudioControlUI() {
        var _a;
        if (this._activeTtsMessageIndex == null)
            return;
        const row = (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.querySelector(`.learnkit-assistant-popup-message-row[data-msg-idx="${this._activeTtsMessageIndex}"]`);
        if (!row)
            return;
        ...
        const btn = audioBar.querySelector(".learnkit-assistant-reply-audio-btn");
        if (!btn)
            return;
    ...
    _resetTtsPlaybackState(preserveFinishTransition = false) {
        this._isTtsSpeaking = false;
        this._ttsPaused = false;
        this._activeTtsMessageIndex = null;
        this._pendingTtsMessageIndex = null;
    ...
    _collapseActiveReplyAudioControl() {
        ...
        const btn = audioBar.querySelector(".learnkit-assistant-reply-audio-btn");
        if (btn)
            this._setReplyAudioButtonIcon(btn, false);
    }
    /** Force close animation at playback end, even while row is hovered. */
    _startReplyAudioFinishTransition() {
        var _a;
        if (this._activeTtsMessageIndex == null)
            return;
        const row = (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.querySelector(`.learnkit-assistant-popup-message-row[data-msg-idx="${this._activeTtsMessageIndex}"]`);
    ...
    _animateTtsWaveform() {
        ...
        if (this._activeTtsMessageIndex == null)
            return;
        const bars = (_a = this.popupEl) === null || _a === void 0 ? void 0 : _a.querySelectorAll(`.learnkit-assistant-popup-message-row[data-msg-idx="${this._activeTtsMessageIndex}"] .learnkit-assistant-reply-audio-wave-bar`);
        if (!(bars === null || bars === void 0 ? void 0 : bars.length))
            return;
        const tick = () => {
            if (!this._isTtsSpeaking)
    ...
    _setReplyAudioButtonIcon(btn, isPlaying) {
    ...
    async sendChatMessage() {
        ...
        const originPath = (_a = originFile === null || originFile === void 0 ? void 0 : originFile.path) !== null && _a !== void 0 ? _a : "";
        this._expireStaleEditProposals();
        // --- Intercept: if there is a pending inline test, grade instead of classifying intent ---
        if (draft && ((_b = this._pendingTestQuestions) === null || _b === void 0 ? void 0 : _b.length)) {
            const chatMsg = { role: "user", text: draft };
        ...
        if (draft) {
            ...
            if (pendingAttachmentNames.length)
                chatMsg.attachmentNames = pendingAttachmentNames;
            this.chatMessages.push(chatMsg);
            ...
            if (routing.requiresClassifierFallback) {
                ...
                if (((_c = this.activeFile) === null || _c === void 0 ? void 0 : _c.path) !== originPath)
                    return;
            }
            switch (intent) {
                case "generate": {
        ...
        const askLatency = {
            ...
            route: intent,
            notePath: (file === null || file === void 0 ? void 0 : file.path) || "",
            attachmentCount: attachedFileDataUrls.length,
        };
        this._clearAttachments();
        const chatMsg = { role: "user", text: displayText };
        if (attachmentNames.length)
            chatMsg.attachmentNames = attachmentNames;
        if (!draft) {
            this.chatMessages.push(chatMsg);
        }
        else {
            const lastMessage = this.chatMessages[this.chatMessages.length - 1];
            if ((lastMessage === null || lastMessage === void 0 ? void 0 : lastMessage.role) === "user" && lastMessage.text === draft) {
                if (attachmentNames.length)
                    lastMessage.attachmentNames = attachmentNames;
            }
            else {
                this.chatMessages.push(chatMsg);
            }
        ...
        const streamingMsg = { role: "assistant", text: "" };
        try {
            const settings = this.plugin.settings.studyAssistant;
            ...
            const context = file
                ...
                : {
                    noteContent: "",
                    noteContentForAi: this._appendPlainTextCustomInstructions("", settings.prompts.assistant, "Companion"),
                    imageRefs: [],
                    imageDataUrls: [],
                    noteEmbedUrls: [],
            ...
            const conversationId = (_d = getRemoteConversationForMode(this.remoteConversationsByMode, "assistant")) === null || _d === void 0 ? void 0 : _d.conversationId;
            ...
            const result = await generateStudyAssistantChatReplyStreaming({
                ...
                onChunk: (token) => {
                    if (askLatency.firstTokenAt == null)
                        askLatency.firstTokenAt = performance.now();
                    ...
                    if (!placeholderPushed) {
                        ...
                        this.render();
                        if (askLatency.firstRenderAt == null)
                            askLatency.firstRenderAt = performance.now();
                    }
                    else {
            ...
            this._isStreaming = false;
            this._streamAbort = null;
            const reply = this._appendFlashcardDisclaimerIfNeeded(String(result.reply || "").trim(), draft) || this._tx("ui.studyAssistant.chat.emptyReply", "No response returned.");
            if (!placeholderPushed)
                this.chatMessages.push(streamingMsg);
            ...
            if (((_e = this.activeFile) === null || _e === void 0 ? void 0 : _e.path) !== originPath && originFile) {
            ...
            if (!placeholderPushed) {
                ...
                if ((lastMsg === null || lastMsg === void 0 ? void 0 : lastMsg.role) === "assistant" && !lastMsg.text.trim()) {
            ...
            if ((e === null || e === void 0 ? void 0 : e.name) === "AbortError") {
                ...
                if (wasStoppedByUser)
                    this._finalizeStoppedAssistantMessage(this.chatMessages, streamingMsg, placeholderPushed);
                askLatency.completedAt = performance.now();
            ...
            this._streamAbortRequestedByUser = false;
            this.isSendingChat = false;
            if (((_f = this.activeFile) === null || _f === void 0 ? void 0 : _f.path) === originPath) {
                this._scheduleSave();
                this.render();
            }
            this._activeStreamingMode = null;
    ...
    async _sendEditRequest(userMessage, options = {}) {
        var _a, _b, _c;
        if (this._isAssistantBusy())
            return;
        ...
        try {
            ...
            const settings = this.plugin.settings.studyAssistant;
            const conversationId = (_a = getRemoteConversationForMode(this.remoteConversationsByMode, "assistant")) === null || _a === void 0 ? void 0 : _a.conversationId;
            ...
            const fallbackText = (proposal === null || proposal === void 0 ? void 0 : proposal.summary) || streamedSummary || this._tx("ui.studyAssistant.edit.noChanges", "No changes could be made to the note.");
            const invalidProposalText = this._tx("ui.studyAssistant.edit.invalidProposal", "I couldn't prepare an editable note update this time, so nothing was written to the note. Please try again with a direct instruction like 'rewrite this section and add it to my note'.");
            // File-switch guard: if user moved to another note, persist reply to originating file
            if (((_b = this.activeFile) === null || _b === void 0 ? void 0 : _b.path) !== originPath) {
            ...
            if (!proposal) {
                ...
                if (!validation.validEdits.length) {
                    ...
                    const editProposal = {
                        ...
                        edits: validation.validEdits.map(e => ({ ...e, status: "pending" })),
                        status: "pending",
                    };
                    if (!placeholderPushed) {
    ...
    _dispatchEditDecorationsToEditor(edits, noteContent) {
        const view = this.plugin.app.workspace.getActiveViewOfType(MarkdownView);
        if (!view) {
        ...
        if (view.getMode() === "preview") {
            ...
            if (leaf) {
                const file = this.getActiveMarkdownFile();
                void leaf.setViewState({ type: "markdown", state: { file: file === null || file === void 0 ? void 0 : file.path, mode: "source" }, active: true }, { focus: false });
            }
    ...
    _dispatchEditDecorationsToEditorInner(edits) {
        var _a, _b, _c;
        const view = this.plugin.app.workspace.getActiveViewOfType(MarkdownView);
        if (!(view === null || view === void 0 ? void 0 : view.editor))
            return;
        const cmEditor = view.editor.cm;
        if (!(cmEditor === null || cmEditor === void 0 ? void 0 : cmEditor.dispatch) || !cmEditor.state)
            return;
        const editorContent = (_c = (_b = (_a = cmEditor.state.doc) === null || _a === void 0 ? void 0 : _a.toString) === null || _b === void 0 ? void 0 : _b.call(_a)) !== null && _c !== void 0 ? _c : "";
        const pendingEdits = [];
    ...
    async _applyAllPendingEdits(editProposal) {
        const file = this.getActiveMarkdownFile();
        if (!file)
            return;
    ...
    _rejectAllPendingEdits(editProposal) {
    ...
    _clearEditDecorations() {
        ...
        if (!(view === null || view === void 0 ? void 0 : view.editor))
            return;
        const cmEditor = view.editor.cm;
        if (!(cmEditor === null || cmEditor === void 0 ? void 0 : cmEditor.dispatch))
            return;
        cmEditor.dispatch({ effects: clearEditProposals.of(null) });
    }
    async _sendReviewMessageInAssistantThread(userMessage, depthOverride, options = {}) {
        var _a, _b, _c;
        if (this._isAssistantBusy())
            return;
        const draft = String(userMessage || "").trim();
        ...
        const originPath = file.path;
        if (depthOverride)
            this.reviewDepth = depthOverride;
        if (!options.skipUserMessageAppend) {
            this.chatMessages.push({ role: "user", text: draft });
        }
        ...
        try {
            ...
            const noteContentWithLinked = linkedNotesContext
                ? `${noteContent}\n\n${linkedNotesContext}`
                : noteContent;
            const noteContentForAi = this._appendPlainTextCustomInstructions(noteContentWithLinked, settings.prompts.assistant, "Companion");
            const allAttachmentUrls = this._dedupeDataUrls([...threadReviewAttachedUrls, ...noteEmbedUrls, ...linkedAttachmentUrls]);
            const conversationId = (_a = getRemoteConversationForMode(this.remoteConversationsByMode, "review")) === null || _a === void 0 ? void 0 : _a.conversationId;
            ...
            const result = await generateStudyAssistantChatReplyStreaming({
                ...
                input: {
                    ...
                    userMessage: draft,
                    customInstructions: settings.prompts.assistant,
                    reviewDepth: depthOverride !== null && depthOverride !== void 0 ? depthOverride : this.reviewDepth,
                    conversationId,
                    conversationHistory: this.chatMessages,
                },
                onChunk: (token) => {
            ...
            this._streamAbort = null;
            this._streamAbortRequestedByUser = false;
            const reply = String(result.reply || "").trim() || this._tx("ui.studyAssistant.chat.emptyReply", "No response returned.");
            if (!placeholderPushed)
                this.chatMessages.push(streamingMsg);
            streamingMsg.text = reply;
    ...
    //  Test generation
    // ---------------------------------------------------------------------------
    async _generateTestForAssistantThread(userMessage) {
        var _a, _b, _c, _d, _e, _f;
        if (this._isAssistantBusy())
            return;
        const file = this.getActiveMarkdownFile();
        ...
        try {
            ...
            const noteContentWithLinked = linkedNotesContext
                ? `${noteContent}\n\n${linkedNotesContext}`
                : noteContent;
            const noteContentForAi = this._appendPlainTextCustomInstructions(noteContentWithLinked, settings.prompts.tests, "Tests");
            ...
            const config = {
                difficulty: (_a = parsed.difficulty) !== null && _a !== void 0 ? _a : "medium",
                questionMode: (_b = parsed.questionMode) !== null && _b !== void 0 ? _b : "mixed",
                questionCount: (_c = parsed.questionCount) !== null && _c !== void 0 ? _c : 5,
                testName: "",
                appliedScenarios: (_d = parsed.appliedScenarios) !== null && _d !== void 0 ? _d : false,
                timed: false,
                durationMinutes: 15,
            ...
            // File-switch guard: if user moved to another note, persist reply to originating file
            const fileChanged = ((_e = this.activeFile) === null || _e === void 0 ? void 0 : _e.path) !== originPath;
            if (!questions.length) {
                const noQuestionsMsg = {
                    role: "assistant",
                    text: this._tx("ui.studyAssistant.test.noQuestions", "I couldn't generate questions from this note. Try a note with more educational content."),
                };
                if (fileChanged) {
    ...
    _formatTestQuestionsAsMarkdown(questions, config) {
    ...
    _formatTestResultsAsMarkdown(gradeResult, questions) {
        const OPTION_LABELS = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        ...
        const items = gradeResult.results.map((r) => {
            ...
            const lines = [`**Q${r.questionNumber}.** ${icon} (${r.scorePercent}%) — ${r.feedback}`];
            if (q) {
                if (q.type === "mcq" && q.options) {
                    const indices = (_a = q.correctIndices) !== null && _a !== void 0 ? _a : (q.correctIndex != null ? [q.correctIndex] : []);
                    const labels = indices.map((idx) => { var _a; return (_a = OPTION_LABELS[idx]) !== null && _a !== void 0 ? _a : String(idx + 1); });
    ...
    async _gradeTestFromChat(userAnswerText) {
        var _a, _b, _c, _d;
        ...
        this._pendingTestQuestions = null;
        const originPath = (_b = (_a = this.activeFile) === null || _a === void 0 ? void 0 : _a.path) !== null && _b !== void 0 ? _b : "";
        this.isSendingChat = true;
        this.chatError = "";
        this.render();
        try {
            ...
            const fileChanged = ((_c = this.activeFile) === null || _c === void 0 ? void 0 : _c.path) !== originPath;
            const resultsMsg = { role: "assistant", text: resultsMarkdown };
            if (fileChanged) {
                const file = this.getActiveMarkdownFile();
                if (file)
                    await this._appendResponseToPersistedChat(file, "messages", resultsMsg);
            }
            else {
            ...
            this.isSendingChat = false;
            if (((_d = this.activeFile) === null || _d === void 0 ? void 0 : _d.path) === originPath) {
                this._scheduleSave();
                this.render();
            }
        }
    ...
    //  Review mode
    // ---------------------------------------------------------------------------
    async sendReviewMessage(userMessage, depthOverride) {
        var _a, _b, _c;
        ...
        try {
            ...
            if ((e === null || e === void 0 ? void 0 : e.name) === "AbortError") {
                const wasStoppedByUser = this._consumeUserStopRequest();
                if (wasStoppedByUser)
                    this._finalizeStoppedAssistantMessage(this.reviewMessages, streamingMsg, placeholderPushed);
                return;
            }
            this._streamAbortRequestedByUser = false;
            const userMessage = formatAssistantError(e, (token, fallback, vars) => this._tx(token, fallback, vars));
    ...
    renderIoSuggestionPreview(parent, ioSrc, ioRectsRaw) {
    ...
    resolveAvailableVaultPath(preferredPath, sourcePath) {
        const vault = this.plugin.app.vault;
        const normalizedSource = normaliseVaultPath(sourcePath);
        const normalizedPreferred = normaliseVaultPath(preferredPath);
        if (!normalizedPreferred || normalizedPreferred === normalizedSource)
            return normalizedPreferred;
        if (!vault.getAbstractFileByPath(normalizedPreferred))
            return normalizedPreferred;
    ...
    async prepareSuggestionForInsert(file, suggestion) {
        ...
        if (!(sourceImage instanceof TFile))
            return suggestion;
    ...
    validateSuggestionsForDisplay(file, suggestions) {
        const validSuggestions = [];
        const rejectedReasons = [];
        for (const suggestion of suggestions) {
            const text = formatInsertBlock(buildSuggestionMarkdownLines(suggestion, this._generatorOutputOptions()).join("\n"));
            const validationError = validateGeneratedCardBlock(file.path, suggestion, text, (token, fallback, vars) => this._tx(token, fallback, vars));
            if (!validationError) {
            ...
            if (hasExplicitRows) {
                ...
                const normalizedError = validateGeneratedCardBlock(file.path, normalizedSuggestion, normalizedText, (token, fallback, vars) => this._tx(token, fallback, vars));
                if (!normalizedError) {
    ...
    async insertSuggestion(suggestion, assistantMessageIndex, suggestionIndex, source) {
        ...
        try {
            ...
            const validationError = validateGeneratedCardBlock(file.path, preparedSuggestion, text, (token, fallback, vars) => this._tx(token, fallback, vars));
            if (validationError)
            ...
            if (batch) {
                ...
                if (!batch.suggestions.length) {
                    ...
                    if ((assistantSummaryInChat === null || assistantSummaryInChat === void 0 ? void 0 : assistantSummaryInChat.role) === "assistant") {
                        ...
                        if ((summaryMessage === null || summaryMessage === void 0 ? void 0 : summaryMessage.role) === "assistant") {
    ...
    async generateSuggestions(userMessage) {
        ...
        try {
            ...
            const conversationId = (_a = getRemoteConversationForMode(this.remoteConversationsByMode, "generate")) === null || _a === void 0 ? void 0 : _a.conversationId;
            const customInstructions = [
                String(settings.prompts.assistant || "").trim(),
                threadContext,
                extraRequest
                    ? this._tx("ui.studyAssistant.generator.requestPrefix", "Additional user request for this generation: {request}", { request: extraRequest })
                    : "",
            ].filter(Boolean).join("\n\n");
            ...
            if (shouldRetryForFormat) {
                const retryStatus = this._tx("ui.studyAssistant.generator.retryingFormat", "AI returned too many formatting errors, retrying with stricter card formatting. This may take a little longer.");
            ...
            if (!generatedSuggestions.length) {
                ...
                this.generatorError = rejectedCount > 0
                    ? this._tx("ui.studyAssistant.generator.emptyAfterValidation", "No parser-valid suggestions were returned. {count} suggestion(s) were filtered before display. Try adjusting your model, prompt, or enabled card types.", { count: rejectedCount }) + firstRejectedReason
            ...
            if (shouldRetryForFormat) {
                log.warn(`[study-assistant] Triggered format retry after first-pass invalid ratio ${(firstAttemptInvalidRatio * 100).toFixed(0)}% (${firstAttemptRejected}/${firstAttemptTotal}).`);
            }
            this._scheduleSave();
        }
        catch (e) {
    ...
    _buildGenerateThreadContext() {
        ...
        const priorGeneratedTopics = this.generateSuggestionBatches
            ...
            .slice(-20);
        if (!recentMessages.length && !priorGeneratedTopics.length)
            return "";
        const blocks = [
    ...
    _buildAssistantThreadGenerationContext() {
    ...
    async _generateSuggestionsForAssistantThread(userMessage) {
        var _a, _b, _c, _d, _e;
        if (this._isAssistantBusy())
            return;
        const file = this.getActiveMarkdownFile();
        ...
        try {
            ...
            const streamedSuggestionKeys = new Set();
            ...
            this.render();
            const syncLiveSuggestionState = () => {
                var _a;
                if (!liveSuggestionBatch)
                    return;
                if (((_a = this.activeFile) === null || _a === void 0 ? void 0 : _a.path) !== originPath) {
            ...
            const collectValidSuggestions = (candidates) => {
                if (!Array.isArray(candidates) || !candidates.length)
                    return;
                if (streamedSuggestions.length >= requestedSuggestionCount)
                    return;
                ...
                for (const suggestion of validSuggestions) {
                    ...
                    if (streamedSuggestionKeys.has(key))
                        continue;
            ...
            const liveMessagePresent = !!streamingSummaryMessage
                && ((_b = this.activeFile) === null || _b === void 0 ? void 0 : _b.path) === originPath
            ...
            if (liveMessagePresent && streamingSummaryMessage && liveSuggestionBatch) {
            ...
            else if (((_c = this.activeFile) === null || _c === void 0 ? void 0 : _c.path) === originPath) {
                if (liveSuggestionBatch) {
                    this.generateSuggestionBatches = this.generateSuggestionBatches.filter((batch) => batch !== liveSuggestionBatch);
                }
                ...
                await this._appendGeneratedResponseToPersistedChat(file, "messages", { role: "assistant", text: assistantJson }, finalSuggestions.length
            ...
            this._streamAbort = null;
            const wasStoppedByUser = (e === null || e === void 0 ? void 0 : e.name) === "AbortError" && this._consumeUserStopRequest();
            if (wasStoppedByUser) {
                const generatedCount = (_d = liveSuggestionBatch === null || liveSuggestionBatch === void 0 ? void 0 : liveSuggestionBatch.suggestions.length) !== null && _d !== void 0 ? _d : 0;
                if (streamingSummaryMessage)
                    streamingSummaryMessage.text = this._stoppedGenerationStatusText(generatedCount);
            ...
            this.isGenerating = false;
            this._activeStreamingMode = null;
            if (((_e = this.activeFile) === null || _e === void 0 ? void 0 : _e.path) === originPath) {
                this._scheduleSave();
                this.render();
            }
        }
    ...
    tokenizeSourceCandidates(candidates) {
        return Array.from(new Set(candidates.join(" ").toLowerCase().split(/[^a-z0-9]+/g)
            .filter((token) => token.length >= 4 && !SOURCE_TOKEN_STOP_WORDS.has(token))));
    }
    scoreLineForTokens(line, tokens) {
    ...
    rangeFromLineIndexes(haystack, lines, lineStarts, startLine, endLine) {
        var _a, _b;
        if (!lines.length)
            return null;
        ...
        const start = (_a = lineStarts[safeStartLine]) !== null && _a !== void 0 ? _a : 0;
        const endBase = (_b = lineStarts[safeEndLine]) !== null && _b !== void 0 ? _b : 0;
        const end = Math.min(haystack.length, endBase + Math.max(1, (lines[safeEndLine] || "").length));
        if (end <= start)
            return null;
        return { start, end };
    }
    findHeadingSectionRange(haystack, lines, lineStarts, bestLine, lineScores, tokens) {
        const minLine = Math.max(0, bestLine - 20);
        for (let i = bestLine; i >= minLine; i--) {
            if (!this.isMarkdownHeading(lines[i] || ""))
                continue;
            let nextHeading = lines.length;
            for (let j = i + 1; j < lines.length; j++) {
                if (this.isMarkdownHeading(lines[j] || "")) {
                    nextHeading = j;
                    break;
                }
            }
            ...
            let sectionScore = 0;
            for (let j = i + 1; j < nextHeading; j++)
                sectionScore += lineScores[j] || 0;
            if (headingScore <= 0 && sectionScore <= 0)
                continue;
            let endLine = Math.max(i, nextHeading - 1);
            while (endLine > i && !String(lines[endLine] || "").trim())
                endLine -= 1;
    ...
    findBestSuggestionRange(noteContent, suggestion) {
        var _a, _b;
        ...
        for (const candidate of candidates) {
            ...
            if (idx >= 0) {
                ...
                while (startLine > 0) {
                    ...
                    if (!prev.trim())
                        break;
                    if ((lineScores[startLine - 1] || 0) <= 0 && !this.isListLikeLine(prev) && !this.isMarkdownHeading(prev))
                        break;
                    startLine -= 1;
                }
                while (endLine < lines.length - 1) {
                    ...
                    if (!next.trim())
                        break;
                    if ((lineScores[endLine + 1] || 0) <= 0 && !this.isListLikeLine(next))
                        break;
                    endLine += 1;
                }
                return (_a = this.rangeFromLineIndexes(haystack, lines, lineStarts, startLine, endLine)) !== null && _a !== void 0 ? _a : { start: idx, end: Math.min(haystack.length, idx + Math.max(1, candidate.length)) };
            }
        }
        if (!tokens.length)
            return null;
        ...
        return (_b = this.rangeFromLineIndexes(haystack, lines, lineStarts, startLine, endLine)) !== null && _b !== void 0 ? _b : this.rangeFromLineIndexes(haystack, lines, lineStarts, bestLine, bestLine);
    }
    async focusSuggestionSource(suggestion) {
        const file = this.getActiveMarkdownFile();
        if (!file) {
        ...
        const leaf = preferredLeaf !== null && preferredLeaf !== void 0 ? preferredLeaf : this.plugin.app.workspace.getLeaf(false);
        const preferredMode = (preferredLeaf === null || preferredLeaf === void 0 ? void 0 : preferredLeaf.view) instanceof MarkdownView ? preferredLeaf.view.getMode() : "preview";
        await leaf.setViewState({ type: "markdown", state: { file: file.path, mode: preferredMode }, active: true }, { focus: true });
    ...
    highlightPreviewSuggestionContext(view, rawSnippet) {
        var _a;
        const host = view.containerEl;
        if (!host)
            return false;
        const root = (_a = host.querySelector(".markdown-reading-view, .markdown-preview-view, .markdown-rendered, .markdown-preview-sizer, .markdown-preview-section")) !== null && _a !== void 0 ? _a : host;
        const snippet = String(rawSnippet || "").replace(/\s+/g, " ").trim().toLowerCase();
        const tokens = Array.from(new Set(snippet
            .split(/[^a-z0-9]+/g)
            .filter((token) => token.length >= 4 && !SOURCE_TOKEN_STOP_WORDS.has(token))));
        if (!tokens.length)
            return false;
        const candidates = Array.from(root.querySelectorAll("p, li, blockquote, h1, h2, h3, h4, h5, h6, td, th, pre, code"));
    ...
    // ---------------------------------------------------------------------------
    appendSuggestionText(parent, value, cls = "sprout-assistant-popup-suggestion-line") {
        if (!value)
            return;
        parent.createEl("p", { cls, text: value });
    }
    renderDifficultyStars(parent, difficulty) {
    ...
    renderSuggestionSummary(parent, suggestion) {
        const data = parseSuggestionRows(suggestion);
        ...
        if (suggestion.type === "basic" || suggestion.type === "reversed") {
            ...
            if (data.ioSrc)
                this.renderIoSuggestionPreview(summary, data.ioSrc, suggestion.ioOcclusions);
        }
        else {
    ...
    _teardownHeaderMenuPortal() {
        var _a, _b;
        (_a = this._headerMenuPopoverEl) === null || _a === void 0 ? void 0 : _a.remove();
        this._headerMenuPopoverEl = null;
        (_b = this._headerMenuPortalRoot) === null || _b === void 0 ? void 0 : _b.remove();
        this._headerMenuPortalRoot = null;
    }
    _schedulePopupHeightSync() {
        if (!this.popupEl || this.popupEl.hasClass("is-hidden"))
            return;
        ...
        this._popupHeightFrame = requestAnimationFrame(() => {
            ...
            if (naturalHeight > this._maxObservedPopupHeight)
                this._maxObservedPopupHeight = naturalHeight;
            const targetHeight = Math.min(Math.max(this._maxObservedPopupHeight, minInitialAssistantHeight), viewportCap);
            if (targetHeight > 0)
                setCssProps(popup, "height", `${targetHeight}px`);
        });
    }
    _clearUserResize() {
        var _a;
        this._userResizedWidth = null;
        this._userResizedHeight = null;
        (_a = this._resizeCleanup) === null || _a === void 0 ? void 0 : _a.call(this);
        this._resizeCleanup = null;
        if (this.popupEl) {
    ...
    _initResizeHandles(root) {
        ...
        const getHostRect = () => {
            var _a;
            const host = this._getHostElement();
            return (_a = host === null || host === void 0 ? void 0 : host.getBoundingClientRect()) !== null && _a !== void 0 ? _a : null;
        };
        ...
        const onTopDown = (e) => startDrag(e, "height");
        const onLeftDown = (e) => startDrag(e, "width");
        const onCornerDown = (e) => startDrag(e, "both");
    ...
    //  Mode button
    // ---------------------------------------------------------------------------
    _buildModeButton(parent, mode, label, icon) {
    ...
    render() {
        ...
        if (!this.popupEl)
            return;
        (_a = this._reviewDepthMenuAbort) === null || _a === void 0 ? void 0 : _a.abort();
        this._reviewDepthMenuAbort = null;
        (_b = this._headerMenuAbort) === null || _b === void 0 ? void 0 : _b.abort();
        this._headerMenuAbort = null;
        this._teardownHeaderMenuPortal();
    ...
    //  Ask mode
    // ---------------------------------------------------------------------------
    renderAssistantMode(parent, preservedScrollTop = null) {
        var _a;
        ...
        if (!messages.length) {
            ...
            this.renderMarkdownMessage(welcomeBubble, [
                this._tx("ui.studyAssistant.chat.welcome", "Hi, I'm Companion. You can think of me as your AI learning assistant. I can answer questions about your Obsidian notes, provide feedback, and generate LearnKit flashcards."),
                "",
                this._tx("ui.studyAssistant.chat.welcome.attachments", "You can also attach files like PDFs, images, PowerPoints or Word documents to give me more context for your requests."),
                "",
                this._tx("ui.studyAssistant.chat.welcome.currentNote", "It looks like you're viewing Home. Ask me anything, or choose an action below to get started."),
            ].join("\n"));
        ...
        for (let i = 0; i < messages.length; i += 1) {
            ...
            if (msg.role === "user" && ((_a = msg.attachmentNames) === null || _a === void 0 ? void 0 : _a.length)) {
            ...
            if (suggestionBatch === null || suggestionBatch === void 0 ? void 0 : suggestionBatch.suggestions.length) {
                ...
                suggestionBatch.suggestions.forEach((suggestion, idx) => {
                    ...
                    const typeLabel = (_a = typeLabelMap[suggestion.type]) !== null && _a !== void 0 ? _a : String(suggestion.type || "");
                    ...
                    if (isNoteBased) {
                        ...
                        item.addEventListener("click", (evt) => {
                            ...
                            if (target === null || target === void 0 ? void 0 : target.closest(".learnkit-assistant-popup-insert-btn"))
                                return;
            ...
            if (msg.role === "assistant" && !isStreamingAssistantMessage && !msg.editProposal && this._isReviewReply(i)) {
                ...
                implBtn.addEventListener("click", () => {
                    void this._sendEditRequest(this._tx("ui.studyAssistant.edit.implementPrompt", "Implement the changes you suggested in your review."));
                });
            }
        ...
        if (surfacedError)
            this.renderAssistantErrorBubble(chatWrap, surfacedError);
    ...
    //  Review mode
    // ---------------------------------------------------------------------------
    renderReviewMode(parent, preservedScrollTop = null) {
        const chatWrap = parent.createDiv({ cls: "learnkit-assistant-popup-chat-wrap learnkit-assistant-popup-chat-wrap" });
        ...
        if (!this.reviewMessages.length) {
            ...
            this.renderMarkdownMessage(welcomeBubble, this._tx("ui.studyAssistant.review.welcome", "Would you like a quick review or a comprehensive review of **{name}**?", { name: reviewNoteName }));
        ...
        if (this.reviewError)
            this.renderAssistantErrorBubble(chatWrap, this.reviewError);
    ...
    renderAssistantErrorBubble(parent, text) {
    ...
    createUserAvatar(parent, initial) {
    ...
    getUserAvatarInitial() {
        var _a, _b, _c;
        const name = String((_c = (_b = (_a = this.plugin.settings) === null || _a === void 0 ? void 0 : _a.general) === null || _b === void 0 ? void 0 : _b.userName) !== null && _c !== void 0 ? _c : "").trim();
        if (!name)
            return "";
    ...
    //  Generate mode
    // ---------------------------------------------------------------------------
    renderGenerateMode(parent, preservedScrollTop = null) {
        const chatWrap = parent.createDiv({ cls: "learnkit-assistant-popup-chat-wrap learnkit-assistant-popup-chat-wrap" });
        ...
        if (!hasGenerationActivity) {
            ...
            this.createAssistantAvatar(welcomeRow);
            const welcomeBubble = welcomeRow.createDiv({ cls: "learnkit-assistant-popup-message-bubble learnkit-assistant-popup-message-bubble is-assistant" });
            this.renderMarkdownMessage(welcomeBubble, this._tx("ui.studyAssistant.generator.welcome", "Ready to generate flashcards on **{name}**?", { name: activeNoteName || this._tx("ui.studyAssistant.chat.currentNoteFallback", "this note") }));
        ...
        if (this.generatorError)
            this.renderAssistantErrorBubble(chatWrap, this.generatorError);
...
class AttachmentPickerModal extends Modal {
    constructor(app, files, onPick, onPickExternal, locale) {
        super(app);
        this._filteredFiles = [];
        this._listEl = null;
