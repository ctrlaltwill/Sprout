...
import { classifyEditProposalRender, } from "./edit-diff-helpers";
// ── State effects ──
export const setEditProposals = StateEffect.define();
export const clearEditProposals = StateEffect.define();
...
function emitEditProposalResolved(detail) {
    if (typeof window === "undefined")
        return;
    window.dispatchEvent(new CustomEvent(EDIT_PROPOSAL_RESOLVED_EVENT_NAME, { detail }));
}
const proposalModeObserverByRoot = new WeakMap();
function disconnectProposalModeObserver(root) {
...
function getProposalRenderMode(sourceViewEl) {
    return (sourceViewEl === null || sourceViewEl === void 0 ? void 0 : sourceViewEl.classList.contains("is-live-preview")) ? "live-preview" : "source";
}
function bindProposalContentToEditorMode(root, contentEl, view, text) {
...
function renderProposalContent(root, contentEl, text, mode) {
...
function renderSourceProposalFragment(contentEl, text) {
...
function createSourceProposalLine(line) {
    if (!line.length) {
    ...
    if (/^-{3,}$/.test(trimmed)) {
        const hrLine = createSourceProposalLineEl("HyperMD-hr HyperMD-hr-bg cm-line learnkit-edit-proposal-source-line learnkit-edit-proposal-source-hr");
    ...
    if (headingMatch) {
        ...
        const headerLine = createSourceProposalLineEl(`HyperMD-header HyperMD-header-${level} cm-line learnkit-edit-proposal-source-line`);
        headerLine.appendChild(createSpan(`cm-formatting cm-formatting-header cm-formatting-header-${level} cm-header cm-header-${level}`, `${hashes}${spacing}`));
    ...
    if (listMatch) {
        ...
        const listLine = createSourceProposalLineEl(`HyperMD-list-line HyperMD-list-line-${level} cm-line learnkit-edit-proposal-source-line learnkit-edit-proposal-source-list-line`);
        if (depth > 0) {
        ...
        listLine.appendChild(createSpan(`learnkit-edit-proposal-source-marker cm-formatting cm-formatting-list ${isOrdered ? "cm-formatting-list-ol" : "cm-formatting-list-ul"} cm-list-${level}`, `${marker}${spacing}`));
...
function appendSourceInlineContent(parent, text, context = {}) {
    ...
    while ((match = tokenRe.exec(text)) !== null) {
        ...
        if (match[1]) {
            ...
            appendWrappedSourceToken(parent, "**", (_b = (_a = match[3]) !== null && _a !== void 0 ? _a : match[4]) !== null && _b !== void 0 ? _b : "", context, "cm-strong");
        }
        else if (match[5] || match[6]) {
            appendWrappedSourceToken(parent, match[5] ? "*" : "_", (_d = (_c = match[5]) !== null && _c !== void 0 ? _c : match[6]) !== null && _d !== void 0 ? _d : "", context, "cm-em");
        }
        else if (match[7]) {
...
function appendWrappedSourceToken(parent, token, content, context, emphasisClass) {
    var _a, _b;
    const formattingClasses = joinClasses(["cm-formatting", ...(_a = context.textClasses) !== null && _a !== void 0 ? _a : [], emphasisClass]);
    const contentClasses = joinClasses([...((_b = context.textClasses) !== null && _b !== void 0 ? _b : []), emphasisClass]);
    parent.appendChild(createSpan(formattingClasses, token));
    parent.appendChild(createSpan(contentClasses, content));
    parent.appendChild(createSpan(formattingClasses, token));
}
function appendSourceTextSegment(parent, text, classes) {
    if (!text)
        return;
...
function createSourceInternalLink(target, display) {
    const wrap = document.createElement("span");
...
class EditInlineDiffWidget extends WidgetType {
    constructor(segments, from, to, original, replacement) {
        super();
...
class EditReplacementWidget extends WidgetType {
    constructor(replacement, from, to, original) {
        super();
        this.replacement = replacement;
        this.from = from;
        this.to = to;
...
const editProposalField = StateField.define({
    create() {
    ...
    update({ entries }, tr) {
        ...
        if (tr.docChanged && updated.length) {
            updated = updated
                ...
                if (newFrom >= newTo)
                    return null; // Range collapsed, discard
...
// ── Decoration builder ──
function buildDecorations(entries, doc) {
    var _a;
    if (!entries.length)
    ...
    for (const entry of sorted) {
        ...
        if (renderPlan.mode === "block-compare") {
            ...
            for (let ln = startLine; ln <= endLine; ln++) {
                ...
                decorations.push(Decoration.line({ class: "learnkit-edit-proposal-original-line" }).range(lineStart));
            }
        ...
        if (renderPlan.mode === "inline-diff" && ((_a = renderPlan.segments) === null || _a === void 0 ? void 0 : _a.length)) {
...
// ── Public extension ──
export const editDecorationExtension = [
    editProposalField,
];
