...
function looksLikeReviewRequest(value) {
    if (!value)
        return false;
    if (/^(quick|standard|comprehensive)\s+review\b/.test(value))
        return true;
...
function looksLikeEditRequest(value) {
    if (!value)
        return false;
...
function assistantRecentlyOfferedNoteEdits(recentMessages) {
...
function looksLikeContextualEditFollowUp(value, recentMessages) {
    if (!value || !assistantRecentlyOfferedNoteEdits(recentMessages))
        return false;
...
function needsIntentClassifierFallback(value, recentMessages) {
    if (!value || !recentMessages.length)
        return false;
...
export function inferStudyAssistantIntentHeuristically(params) {
    ...
    if (!value)
        return { intent: "ask", requiresClassifierFallback: false };
    if (looksLikeContextualEditFollowUp(value, recentMessages))
        return { intent: "edit", requiresClassifierFallback: false };
    if (looksLikeEditRequest(value))
        return { intent: "edit", requiresClassifierFallback: false };
    ...
    if (looksLikeReviewRequest(value))
        return { intent: "review", requiresClassifierFallback: false };
    return {
        intent: "ask",
