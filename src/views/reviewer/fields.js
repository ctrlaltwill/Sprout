 ...
 */
export function splitUnescapedPipes(s) {
    return splitUnescapedDelimiters(s);
}
export function parseMcqOptionsFromCell(raw) {
    ...
    const parts = cleaned.includes(getDelimiter())
        ...
        : cleaned
            ...
            .filter(Boolean);
    if (parts.length < 2)
        throw new Error(`MCQ requires at least 2 options (separate with ${getDelimiter()}).`);
...
export function buildAnswerOrOptionsFor(card) {
    if (card.type === "basic")
        return card.a || "";
    if (card.type === "reversed")
        return card.q || "";
