...
export function isSkipEnabled(plugin) {
    var _a;
    return !!((_a = plugin.settings.study) === null || _a === void 0 ? void 0 : _a.enableSkipButton);
}
export function initSkipState(session) {
    if (!session.skipCounts || typeof session.skipCounts !== "object")
        session.skipCounts = {};
}
function computeDefaultSkipDelay(remainingAfterThisCard) {
...
class ConfirmBuryForTodayModal extends Modal {
    constructor(app, onBury, onIgnore, interfaceLanguage) {
        super(app);
        this._onBury = onBury;
    ...
    onOpen() {
        const tx = (token, fallback, vars) => t(this._interfaceLanguage, token, fallback, vars);
        ...
        p.setText(tx("ui.reviewer.skip.modal.body", "You have skipped this card multiple times in this session. Burying will postpone it until tomorrow. Skipping does not change scheduling unless you confirm bury."));
...
export function skipCurrentCard(view) {
    var _a, _b;
    ...
    if (type !== "basic" && type !== "reversed" && type !== "reversed-child" && type !== "cloze" && type !== "cloze-child")
        return;
    ...
    if (!view.session.skipCounts || typeof view.session.skipCounts !== "object")
        view.session.skipCounts = {};
    const skipCounts = view.session.skipCounts;
    const nextCount = (Number(skipCounts[id]) || 0) + 1;
    skipCounts[id] = nextCount;
    if (nextCount >= 3) {
        new ConfirmBuryForTodayModal(view.app, () => {
            void view.buryCurrentCard();
        }, () => { }, (_b = (_a = view.plugin.settings) === null || _a === void 0 ? void 0 : _a.general) === null || _b === void 0 ? void 0 : _b.interfaceLanguage).open();
        return;
    }
    ...
    const insertAt = remainingAfterRemoval < delay ? q.length : Math.min(q.length, idx + delay);
