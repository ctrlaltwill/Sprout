...
export function keySymbol(label) {
    if (label.toLowerCase() === "enter")
        return "⏎";
    return label;
}
export function keybox(label, extraClass = "") {
...
export function appendKeyboxRight(btn, label) {
    const wrap = el("span", "sprout-btn-right");
    wrap.appendChild(keybox(label));
    btn.appendChild(wrap);
}
export function makePlainButton(label, onClick, className = "learnkit-btn") {
