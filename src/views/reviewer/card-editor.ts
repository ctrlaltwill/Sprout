...
const tx = (token: string, fallback: string, vars?: Record<string, string | number>) =>
  t(undefined, token, fallback, vars);
...
export class CardEditModal extends Modal {
  ...
  onOpen() {
    ...
    const addMcqOptionRow = (initialValue = "", makeCorrect = false) => {
      ...
      delBtn.addEventListener("click", () => {
        ...
        optionRows.forEach((r, i) => {
          ...
          if (labelEl) labelEl.textContent = tx("ui.reviewer.cardEditor.field.optionN", "Option {index}", { index: i + 1 });
        });
      });
    ...
    if (type === "oq") {
      ...
      oqPlusBtn.onclick = () => {
        if (oqStepRows.length >= 20) { new Notice(tx("ui.common.maxSteps20", "Maximum 20 steps.")); return; }
        addOqStepRow("");
        oqRenumber();
      };
