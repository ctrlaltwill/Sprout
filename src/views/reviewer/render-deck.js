...
import { clearNode, titleCaseGroupPath, expandGroupAncestors, } from "../../platform/core/shared-utils";
/** Format a group path for display (alias for formatGroupDisplay without title-casing). */
function prettifyGroupLabel(groupPath) {
...
function makeIconButton(opts) {
    ...
    (_a = queryFirst(iconWrap, "svg")) === null || _a === void 0 ? void 0 : _a.classList.add("shrink-0");
    const text = document.createElement("span");
    text.textContent = opts.label;
    text.className = (_b = opts.labelClassName) !== null && _b !== void 0 ? _b : "ml-2";
...
function makeDisclosureChevron(isOpen, onToggle, collapseLabel, expandLabel) {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className =
...
export function renderDeckMode(args) {
    var _a, _b, _c, _d, _e;
    const { app, plugin, container } = args;
    const tx = (token, fallback, vars) => { var _a, _b; return t((_b = (_a = plugin.settings) === null || _a === void 0 ? void 0 : _a.general) === null || _b === void 0 ? void 0 : _b.interfaceLanguage, token, fallback, vars); };
    ensureBasecoatStartedOnce();
    const animationsEnabled = (_c = (_b = (_a = plugin.settings) === null || _a === void 0 ? void 0 : _a.general) === null || _b === void 0 ? void 0 : _b.enableAnimations) !== null && _c !== void 0 ? _c : true;
    const shouldAnimateOnce = animationsEnabled && !!args.applyAOS;
    const applyAos = (el, delay, animation = "fade-up") => {
    ...
    const vaultName = ((_e = (_d = app === null || app === void 0 ? void 0 : app.vault) === null || _d === void 0 ? void 0 : _d.getName) === null || _e === void 0 ? void 0 : _e.call(_d)) || "Vault";
    ...
    const titleActionsHost = titleRoot === null || titleRoot === void 0 ? void 0 : titleRoot.querySelector(".lk-review-title-timer-host, .learnkit-review-title-timer-host");
    // Tree early (needed for badges + expand/collapse all)
    const tree = buildDeckTree(cards, states, now, vaultName);
    let expandedState = new Set(args.expanded);
    ...
    const allOptions = Array.from(new Set(getAllGroups()
    ...
    const groupPopover = createBodyPortalPopover({
        ...
        buildContent(panel, close) {
            ...
            searchInput.addEventListener("input", () => {
                ...
                window.requestAnimationFrame(() => {
                    ...
                    try {
                        ...
                        log.swallow("render-deck searchInput setSelectionRange", e);
                    }
                });
            });
            const stopKey = (ev) => {
            ...
            searchInput.addEventListener("keypress", stopKey);
            // Focus search on open
            window.requestAnimationFrame(() => searchInput.focus());
        },
        onOpened(panel) {
            const input = panel.querySelector(".learnkit-ss-search-input");
            if (input)
                input.focus();
        },
    });
            ...
            for (const child of children) {
                ...
                if (child.type === "folder") {
                    ...
                    const toggle = makeDisclosureChevron(isOpen, () => {
                        ...
                        if (next.has(child.key))
                            next.delete(child.key);
                        else
                            next.add(child.key);
                        setExpanded(next);
                        renderTable();
                    }, tx("ui.reviewer.deck.folder.collapse", "Collapse folder"), tx("ui.reviewer.deck.folder.expand", "Expand folder"));
