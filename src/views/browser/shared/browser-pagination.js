 ...
 */
export function renderPagination(host, totalRows, ctx) {
    ...
    // First page
    if (start > 1) {
        nav.appendChild(mkBtn("1", tx("ui.browser.pagination.pageN", "Page {page}", { page: 1 }), false, current === 1, () => {
            ctx.setPageIndex(0);
            ctx.refreshTable();
        }));
    }
    ...
    for (let p = start; p <= end; p++) {
        nav.appendChild(mkBtn(String(p), tx("ui.browser.pagination.pageN", "Page {page}", { page: p }), false, p === current, () => {
    ...
    // Last page
    if (end < totalPagesLocal) {
        nav.appendChild(mkBtn(String(totalPagesLocal), tx("ui.browser.pagination.pageN", "Page {page}", { page: totalPagesLocal }), false, current === totalPagesLocal, () => {
