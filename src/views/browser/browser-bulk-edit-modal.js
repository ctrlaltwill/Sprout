...
export class BulkEditModal extends Modal {
    ...
    onOpen() {
        ...
        const tx = (token, fallback, vars) => t(ctx.interfaceLanguage, token, fallback, vars);
        // ── Modal chrome ──────────────────────────────────────────────────────
        ...
        const handleModalPointerDownBlur = (ev) => {
            ...
            if (!(active instanceof HTMLInputElement || active instanceof HTMLTextAreaElement))
                return;
            if (!active.classList.contains("learnkit-flag-editor-control"))
                return;
            const activeWrap = active.closest(".learnkit-flag-editor-wrap");
            if (activeWrap === null || activeWrap === void 0 ? void 0 : activeWrap.contains(target))
                return;
            active.blur();
        };
        ...
        const normalizedTypes = cards.map((card) => { var _a; return String((_a = card === null || card === void 0 ? void 0 : card.type) !== null && _a !== void 0 ? _a : "").toLowerCase(); });
        const canBulkToggleType = cards.length > 1 &&
            normalizedTypes.length > 0 &&
            normalizedTypes.every((type) => type === "basic" || type === "reversed");
        let selectedBulkType = normalizedTypes[0] === "reversed" ? "reversed" : "basic";
        ...
        if (hasNonCloze) {
            fields.splice(fields.findIndex((f) => f.key === "info"), 0, { key: "answer", label: answerLabel, editable: true });
        }
            ...
            if (typeof ResizeObserver !== "undefined") {
                const ro = new ResizeObserver(() => {
                    if (control instanceof HTMLTextAreaElement && wrap.classList.contains("learnkit-flag-editor--focused")) {
                        ...
                        if (renderedHeight > 0)
                            lastPreviewHeight = clampHeight(renderedHeight);
                    }
        ...
        const topKeys = ["id", "type", "stage", "due"];
        const answerPredicate = (card) => { var _a; return String((_a = card.type) !== null && _a !== void 0 ? _a : "").toLowerCase() !== "cloze"; };
        let mcqOriginalString = "";
        let buildMcqValue = null;
        // ── Group picker field builder ────────────────────────────
        const createGroupPickerField = (initialValue, cardsCount) => {
            ...
            if (cardsCount > 1) {
                ...
                overwriteNotice.textContent =
                    tx("ui.browser.bulkEdit.groups.overwriteHint", "Typing here will overwrite this field for every selected card; leave it blank to keep existing values.");
            ...
            const renderList = () => {
                ...
                if (raw && !exact)
                    addRow(tx("ui.browser.bulkEdit.groups.add", "Add \"{group}\"", { group: rawDisplay || rawTitle }), rawTitle || raw, true);
        ...
        // ── Shared-value helper ───────────────────────────────────
        const sharedValue = (col, predicate = () => true) => {
        ...
        const createFieldWrapper = (field) => {
            ...
            if (field.key === "type" && canBulkToggleType) {
                ...
                const makeTypeItem = (type, labelText) => {
                    ...
                    const apply = () => {
                        ...
                        for (const child of Array.from(menuList.children)) {
                            if (child instanceof HTMLElement) {
                                ...
                                const marker = child.querySelector(".size-2");
                                marker === null || marker === void 0 ? void 0 : marker.classList.toggle("invisible", !isChecked);
                            }
                        }
                        menu.setAttribute("aria-hidden", "true");
            ...
            if (field.editable && (field.key === "title" || field.key === "question" || field.key === "answer" || field.key === "info")) {
                ...
                if (field.key === "title")
                    textarea.placeholder = tx("ui.browser.bulkEdit.placeholder.title", "Enter a descriptive title for this flashcard");
                if (field.key === "question")
                    textarea.placeholder = isClozeOnly
                ...
                if (field.key === "answer")
                    textarea.placeholder = tx("ui.browser.bulkEdit.placeholder.answer", "Enter the answer to your question");
                if (field.key === "info")
                    textarea.placeholder = tx("ui.browser.bulkEdit.placeholder.info", "Optional: Add extra context or explanation shown on the back of the card");
                input = textarea;
            }
            else {
            ...
            if (cards.length > 1 && field.editable) {
                ...
                overwriteNotice.textContent = tx("ui.browser.bulkEdit.overwriteHint", "You have selected {count} {label}. Any input in this field will overwrite this field for all cards. To leave all cards in their current form, leave this field blank.", { count: cardCount, label: cardLabel });
                overwriteNotice.classList.add("learnkit-is-hidden", "learnkit-is-hidden");
            ...
            const modalFieldMin = field.editable && (field.key === "title" || field.key === "question" || field.key === "answer" || field.key === "info")
                ? fieldMinHeightPx(field.key)
                : 38;
            const modalFieldMax = field.editable && (field.key === "title" || field.key === "question" || field.key === "answer" || field.key === "info")
        ...
        // ── Footer (Cancel / Save) ────────────────────────────────
        this.modalEl.querySelectorAll(":scope > .lk-modal-footer.learnkit-bulk-edit-footer").forEach((node) => node.remove());
        ...
        save.addEventListener("click", () => {
            void (async () => {
                ...
                const hasTypeChange = canBulkToggleType && cards.some((card) => { var _a; return String((_a = card.type) !== null && _a !== void 0 ? _a : "") !== selectedBulkType; });
                ...
                try {
                    for (const card of cards) {
                        let updated = card;
                        if (canBulkToggleType && String((_b = updated.type) !== null && _b !== void 0 ? _b : "") !== selectedBulkType) {
                            updated = ctx.applyValueToCard(updated, "type", selectedBulkType);
                        }
                        for (const [key, value] of Object.entries(updates)) {
                            updated = ctx.applyValueToCard(updated, key, value);
                        }
 ...
 */
export function openBulkEditModal(app, cards, ctx) {
