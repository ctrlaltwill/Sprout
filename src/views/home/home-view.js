 ...
 */
import { ItemView, Notice, setIcon, TFile, TFolder } from "obsidian";
...
import { MS_DAY, localDayIndex, formatTimeAgo, scopeFromDeckPath, formatDeckLabel, formatPinnedDeckLabel, getDeckLeafName, } from "./home-helpers";
...
export class SproutHomeView extends ItemView {
    ...
    async onOpen() {
        ...
        const tabTitleEl = this.leaf.tabHeaderInnerTitleEl;
        if (tabTitleEl) {
            tabTitleEl.empty();
        ...
        if ((_e = (_d = (_c = this.plugin.settings) === null || _c === void 0 ? void 0 : _c.general) === null || _d === void 0 ? void 0 : _d.enableAnimations) !== null && _e !== void 0 ? _e : true) {
    ...
    async onClose() {
        ...
        this._heatmapRoot = null;
        (_d = this._titleStripEl) === null || _d === void 0 ? void 0 : _d.remove();
        this._titleStripEl = null;
        resetAOS();
        await Promise.resolve();
    }
    ...
    _applyWidthMode() {
        ...
        if (titleStrip)
            setCssProps(titleStrip, "--lk-home-max-width", maxWidth);
    }
    ...
    render() {
        ...
        const nameSetting = (_e = this.plugin.settings.general.userName) !== null && _e !== void 0 ? _e : "";
        ...
        const greeting = (_f = this._greeting) !== null && _f !== void 0 ? _f : pickHomeGreeting();
            ...
            : greeting.fallback;
        {
            ...
            nameInput.addEventListener("beforeinput", (event) => {
                ...
                const start = (_a = nameInput.selectionStart) !== null && _a !== void 0 ? _a : currentValue.length;
                const end = (_b = nameInput.selectionEnd) !== null && _b !== void 0 ? _b : currentValue.length;
            ...
            nameInput.addEventListener("paste", (event) => {
                ...
                if (!pasted)
                    return;
                const currentValue = nameInput.value;
                const start = (_c = nameInput.selectionStart) !== null && _c !== void 0 ? _c : currentValue.length;
                const end = (_d = nameInput.selectionEnd) !== null && _d !== void 0 ? _d : currentValue.length;
                const selectedLength = Math.max(0, end - start);
            ...
            this._nameObserver =
                typeof ResizeObserver !== "undefined"
                    ...
                    : null;
            if (this._nameObserver)
                this._nameObserver.observe(title);
        ...
        const events = (_j = (_h = (_g = this.plugin.store).getAnalyticsEvents) === null || _h === void 0 ? void 0 : _h.call(_g)) !== null && _j !== void 0 ? _j : [];
        const cards = (_m = (_l = (_k = this.plugin.store).getAllCards) === null || _l === void 0 ? void 0 : _l.call(_k)) !== null && _m !== void 0 ? _m : [];
        const states = (_o = this.plugin.store.data.states) !== null && _o !== void 0 ? _o : {};
        ...
        const activeStudyCards = cards.filter((card) => {
            ...
            if (isParentCard(card))
                return false;
            const source = String((_a = card.sourceNotePath) !== null && _a !== void 0 ? _a : "").trim();
        ...
        const scopeHasCards = (scope) => {
            var _a;
            if (!scope || typeof scope !== "object")
                return false;
            for (const card of activeStudyCards) {
                const path = String((_a = card.sourceNotePath) !== null && _a !== void 0 ? _a : "").trim();
                if (!path)
                    continue;
        ...
        for (const ev of sessionEvents) {
            const scope = ev === null || ev === void 0 ? void 0 : ev.scope;
            if (!scope || typeof scope !== "object")
                continue;
            const type = String(scope.type || "");
            const key = String(scope.key || "");
            const name = String(scope.name || "");
            if (!["vault", "folder", "note"].includes(type))
                continue;
            ...
            if (seenDecks.has(dedupeKey))
                continue;
            if (!scopeExists(scope))
                continue;
            if (!scopeHasCards(scope))
                continue;
        ...
        const pinnedDecks = (_p = this.plugin.settings.general.pinnedDecks) !== null && _p !== void 0 ? _p : [];
        ...
        for (const card of cards) {
            const id = String((_q = card === null || card === void 0 ? void 0 : card.id) !== null && _q !== void 0 ? _q : "");
            if (!id)
                continue;
            // Skip parent cards — only children (the ones actually studied) should be counted
            if (isParentCard(card))
                continue;
            const st = states === null || states === void 0 ? void 0 : states[id];
            if (!st || String((_r = st.stage) !== null && _r !== void 0 ? _r : "") === "suspended")
                continue;
            ...
            if (due <= nowMs) {
                ...
                totalDue += 1;
                const path = String((_s = card === null || card === void 0 ? void 0 : card.sourceNotePath) !== null && _s !== void 0 ? _s : "").trim();
                if (!path)
                    continue;
                dueCounts.set(path, ((_t = dueCounts.get(path)) !== null && _t !== void 0 ? _t : 0) + 1);
            }
            else if (due <= tomorrowMs) {
                // due tomorrow (currently unused)
            }
        }
        const getDueForPrefix = (prefix) => {
        ...
        for (const ev of reviewEvents) {
            ...
            const entry = (_u = dayMap.get(dayIndex)) !== null && _u !== void 0 ? _u : { count: 0, totalMs: 0 };
            entry.count += 1;
        ...
        const openStudyForScope = async (scope) => {
            try {
        ...
        const openStudyForDeckPath = async (path) => {
            await openStudyForScope(scopeFromDeckPath(path));
        };
        const openSettingsTab = async (tab) => {
            try {
        ...
        const makeDeckSection = (label, iconName) => {
        ...
        const makeStatCard = (label, value, note) => {
            ...
            if (note)
                card.createDiv({ cls: "text-xs text-muted-foreground", text: note });
            return card;
        };
        ...
        makeStatCard(tx("ui.home.stat.cardsOverdue", "Cards overdue"), String(totalOverdue), formatQueueTimeEstimate(totalOverdue));
        makeStatCard(tx("ui.home.stat.cardsDueToday", "Cards due today"), String(totalDueToday), formatQueueTimeEstimate(totalDueToday));
        ...
        const savePinnedDecks = async (decks) => {
        ...
        const renderPinnedDecks = () => {
            ...
            const commitPinnedReorder = async (fromIndex, toIndex) => {
                if (fromIndex === toIndex)
                    return;
                if (fromIndex < 0 || fromIndex >= currentPinned.length)
                    return;
                if (toIndex < 0 || toIndex >= currentPinned.length)
                    return;
        ...
        const renderSearchDropdown = () => {
            ...
            const filtered = allDecks
                .map(deck => {
                if (currentPinned.includes(deck))
                    return false;
        ...
        this._heatmapRoot.render(React.createElement(ReviewCalendarHeatmap, {
            revlog: reviewEvents,
            timezone,
            rangeDays: 0,
            filters: {},
        ...
        const renderTip = () => {
            ...
            tipActionBtn === null || tipActionBtn === void 0 ? void 0 : tipActionBtn.remove();
