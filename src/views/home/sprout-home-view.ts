...
import { AOS_DURATION, MAX_CONTENT_WIDTH_PX, VIEW_TYPE_HOME, VIEW_TYPE_REVIEWER } from "../../platform/core/constants";
...
export class SproutHomeView extends ItemView {
  ...
  render() {
    ...
    if (showGreeting) {
      ...
      const setGreetingText = (_name: string, firstOpen: boolean) => {
        ...
        const variants = isPhoneMobile
          ? [
            firstOpen
              ? tx("ui.home.greeting.welcomeMobileFirst", "Welcome to Sprout.")
              : tx("ui.home.greeting.welcomeMobileBack", "Welcome back to Sprout."),
          ]
          : firstOpen
            ? [tx("ui.home.greeting.welcomeFirst", "Welcome to Sprout, {name}")]
            : [
                tx("ui.home.greeting.welcomeBack", "Welcome back to Sprout, {name}"),
                tx("ui.home.greeting.kiaOra", "Kia ora {name}!"),
                tx("ui.home.greeting.gday", "G'day {name}!"),
              ];
        if (!isPhoneMobile) {
          if (hour >= 5 && hour < 12) variants.push(tx("ui.home.greeting.morning", "Good morning {name}"));
          if (hour >= 5 && hour < 12) variants.push(tx("ui.home.greeting.morning", "Good morning {name}"));
          else if (hour >= 12 && hour < 18) variants.push(tx("ui.home.greeting.afternoon", "Good afternoon {name}"));
          else variants.push(tx("ui.home.greeting.evening", "Good evening {name}"));
        }
      ...
      const typingText = tx("ui.home.typing.yourName", "your name");
      let typingIdx = 0;
      let typingDir: 1 | -1 = 1;
      const typingStep = () => {
        try {
      ...
      title.textContent = tx("ui.view.home.title", "Home");
    }
    ...
    makeStatCard(tx("ui.home.stat.cardsOverdue", "Cards overdue"), String(totalOverdue));
    makeStatCard(tx("ui.home.stat.cardsDueToday", "Cards due today"), String(totalDueToday));
    ...
    // Avg time/day card with trend badge
    {
      ...
      header.createDiv({ text: tx("ui.home.stat.dailyTime", "Daily time"), cls: "bc" });
    ...
    // Avg cards/day card with trend badge
    {
      ...
      statsRow.appendChild(card);
      const header = card.createDiv({ cls: "flex items-center justify-between text-sm text-muted-foreground" });
      header.createDiv({ text: tx("ui.home.stat.dailyCards", "Daily cards"), cls: "bc" });
